...
class Trustpilot extends Module
{
    ...
    public function hookDisplayOrderConfirmation($params)
    {
        ...
        if ($code > 250 && $code < 254) {
            Logger::addLog(
                'Trustpilot module: Skipping invitation sending from confirmation page. Trustpilot module is disabled.'
            );
            return;
        }
        ...
        $mapped_invitation_trigger =
            TrustpilotConfig::getInstance()->getFromMasterSettings('general')->mappedInvitationTrigger;
    ...
    // Let's delete this hook (after checking in prod)
    //if hookActionOrderHistoryAddAfter will cover all invitations plus missing ones
    ...
    public function hookActionOrderHistoryAddAfter($params)
    {
        $newOrderStatus = new OrderState(
            (int) $params['order_history']->id_order_state,
            (int) isset($params['cart']) ? $params['cart']->id_lang : null
        );
    ...
    public function sendBackendInvitation($params, $hook)
    {
        try {
            ...
            if (!$this->isModuleConfigured()) {
                Logger::addLog(
                    'Trustpilot module: Skipping invitation sending. Trustpilot module is not configured.',
                    2
                );
                return;
            }
            ...
            if ($code > 250 && $code < 254) {
                Logger::addLog(
                    'Trustpilot module: Skipping invitation sending from backend. Trustpilot module is disabled.'
                );
                return;
            }
            ...
            if (in_array(
                (string)$params['newOrderStatus']->id,
                $config->getFromMasterSettings('general')->mappedInvitationTrigger
            )) {
    ...
    public function logError($e, $description, $optional = array())
    {
        try {
            ...
            $exception = 'Trustpilot module: Unable to log error. Error: ' . $ex->getMessage();
