...
class TrustpilotPastOrders
{
        ...
        try {
            ...
            if (!is_null($key)) {
                ...
                foreach ($chunked_failed_orders as $failed_orders_chunk) {
                    ...
                    if ($code == 202) {
                        $collect_product_data = WITH_PRODUCT_DATA;
                        $batch['invitations'] = $this->getInvitationsByRefs(
                            $collect_product_data,
                            $failed_orders_chunk
                        );
                        $batch['type'] = $collect_product_data;
    ...
    {
        $date = new DateTime();
        $timestap = time() - (86400 * $period_in_days);
        $args = array(
            'date_created' => "> '" . ($date->setTimestamp($timestap)->format('Y-m-d H:i:s') ) . "'",
            'limit' => 20,
            'paged' => $pageId,
    ...
    private function getInvitationsByRefs($collect_product_data, $order_refs)
    {
        ...
        $query = "SELECT o.id_order FROM {$db_prefix}orders o
          WHERE o.reference IN (\"".implode('","', array_map('pSQL', $order_refs))."\")
        ";
    ...
    private function getPastOrdersIds($args)
    {
        ...
        $date_created = stripslashes( pSQL($args['date_created']) );
        $statuses = implode(',', array_map('intval', $args['past_order_statuses']));
        $db_prefix = _DB_PREFIX_;
        $query = "SELECT o.id_order FROM {$db_prefix}orders o
            WHERE o.current_state IN ({$statuses}) AND o.date_add $date_created";

        $query = $this->appendShopFiltersToQuery($query);
        $query = $query . " ORDER BY o.id_order LIMIT {$offset}, {$limit}";

        $orders = Db::getInstance()->ExecuteS($query);
        return $orders;
    }

    private function appendShopFiltersToQuery($query)
    {
        try {
            $id_shop = null;
            $id_shop_group = null;
            if (method_exists('Context', 'getContext')) {
                ...
                $id_shop_group = (int)Context::getContext()->shop->getContextShopGroupID();
            } elseif (method_exists('Shop', 'getContextShopID') && method_exists('Shop', 'getContextShopGroupID')) {
                $id_shop = (int)Shop::getContextShopID(true);
                $id_shop_group = (int)Shop::getContextShopGroupID(true);
            } else {
                TrustpilotConfig::getInstance()->getShopFromContext($id_shop_group, $id_shop);
            }

            if (!is_null($id_shop)) {
                $query = $query . ' AND `id_shop` = ' . pSQL($id_shop);
            }
            if (!is_null($id_shop_group)) {
                $query = $query . ' AND `id_shop_group` = ' . pSQL($id_shop_group);
            }
        } catch (\Throwable $e) {
            $message = 'Failed to fetch shop id for past orders sync';
            Logger::addLog($message . 'Error: ' . $e->getMessage(), 2);
            Module::getInstanceByName('trustpilot')->logError($e, $message);
        } catch (\Exception $e) {
            $message = 'Failed to fetch shop id for past orders sync';
