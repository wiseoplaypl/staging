...
<script>
    ...
    function rebuildLimitLimit() {
        ...
        if (nbOfItemsInFeed < getAllProductsNb) {
            if (nbOfItemsInFeed == 0 || nbOfItemsInFeed == "") {
                $('.getAllProductsNbInfo').html("{l s='Defined number of items in feed is:' mod='gmfeed'} " + "<span class='badge'>" + "&infin;" + "</span> " + "{l s='Module will export all products to one feed file' mod='gmfeed'}");
                $('#limit_page').val(0).change();
            } else {
                $('.partOfFeedLine').fadeOut();
                $('.getAllProductsNbInfo').html("{l s='Defined number of items in feed is:' mod='gmfeed'} " + "<span class='badge'>" + nbOfItemsInFeed + " </span> " + "{l s='Number of productsin your shop is: ' mod='gmfeed'}" + "<span class='badge'>" + getAllProductsNb + " </span> " + "{l s='To export all products you need to split export process to several files' mod='gmfeed'}" + ":<br/>");
                ...
                for (i = 0; i < Math.ceil(nbOfParts); i++) {
                    ...
                    $('.getAllProductsNbInfo').append('<div class="partOfFeedLine">' + "{l s='Part: ' mod='gmfeed'}" + " " + (i + 1) + " {l s='will contain products from' mod='gmfeed'}: " + startProduct + " {l s='to' mod='gmfeed'}: " + endProduct + ' <span class="btn btn-default generateThisPartOfFeed" data-part="' + (i + 1) + '">' + "{l s='generate this part of feed' mod='gmfeed'}" + '</span></div>');
                }
            }
        } else {
            $('.getAllProductsNbInfo').html("{l s='Defined number of items in feed is:' mod='gmfeed'} " + "<span class='badge'>" + "&infin;" + "</span> " + "{l s='Module will export all products to one feed file' mod='gmfeed'}");
    ...
    {/if}
        ...
        $(".syncGooleCategories").click(function (e) {
            ...
            e.preventDefault();
            $.ajax({
                url: "index.php?language_code=" + $(this).attr('data-language-code') + "&ajax=1&controller=AdminExportProductsFeedGoogle&action=downloadCategories&token=" + AjaxToken,
                beforeSend: function () {
        ...
        $(".syncGooleCategoriesCustom").click(function (e) {
            ...
            e.preventDefault();
            $.ajax({
                url: "index.php?language_code=" + $(this).attr('data-language-code') + "&selected_option=" + $(this).parent().find('select option:selected').val() + "&ajax=1&controller=AdminExportProductsFeedGoogle&action=downloadCategoriesCustom&token=" + AjaxToken,
                beforeSend: function () {
                    button.find('i').addClass('icon-spin').addClass('icon-refresh').removeClass('icon-check').removeClass('icon-bug').parent().removeClass('btn-danger').removeClass('btn-success');
                },
                success: function (result) {
