...
$('.ui-priority-primary').live('click', function () {
    $.ajax({
        url: ajax_url,
        type: 'POST',
        dataType: 'json',
...
$('[name=id_manufacturer], [name=condition], [name=date_add]').live('change', function (e) {
    e.preventDefault();
    self = this;
});
$('[name=location]').live('change', function (e) {
    console.log($this);
    e.preventDefault();
    self = this;
});
...
$('.table-product_td, .table.configuration td').live('click', function (e) {

    var this_var = $(this).find('.edit_field');

    if($(this).find('a.process-icon-link').length > 0) {
        return;
    }

    e.preventDefault();
    if(this_var.next().children('textarea, input[type=text]').is('.type_date'))
        positionCalendar(this);

    // if (!validateActiveField())
    //     return false;
    if (!this_var.is('.edit_category'))
    {
        if (this_var.next().find('[class="type_price"]').length)
        {
            this_var.attr('data-val', $(this).next().find('textarea, input[type=text]').val());
        }
        this_var.closest('td').find('.form_edit_field').show();
        this_var.hide();
    }
    else
    {
        var self = this;
        current_id_product = $(self).find('.edit_category').data('id');
...
$(document).ready(function () {
    $('.id_brand, .id_condition, .id_data_add, .location').change(function(){
    ...
    $('.id_brand, .id_condition, .id_data_add, .location').click(function(){
        var data_id= $(this).attr('data-id');
        $.ajax({
            url: ajax_url,
            type: 'POST',
    ...
    $('.form_edit_field [data-event-save=1]').live('save_field', function () {
        ...
        $.ajax({
            ...
            data: {
                ...
                shop: ($(self).data('shop') ? 1 : 0),
                id_shop: $('[data-field-name=id_shop]').val()
            },
            success: function(r) {
                if(r !== null)
                {
...
$('.table-product_td, .table.configuration td').live('click', updateFormField);
...
function updateFormField(event) {
    if(event.keyCode === undefined || event.keyCode == 13)
    {
        ...
        if (event.keyCode === undefined && $(this).closest('td').is('.active_field'))
            return false;

        if($('table.configuration div:visible').find('> textarea').data('fieldTable') == 'specific_price') {
            if ($('table.configuration div:visible').find('> textarea').val() == -1) {
                var field_specific_prive = $('table.configuration div:visible').find('> textarea').closest('.active_field').find('.edit_field');
                var specific_prive = '';
            }
        }

        $('.edit_field').closest('td.active_field').find('[data-event-save=1]').trigger('save_field');
        $('.edit_field').closest('td.active_field').find('input.location').val(
            $('.edit_field').closest('td.active_field').find('[data-event-save=1]').val()
        );
    //    $('.edit_field').closest('td.active_field').find('input.location').trigger( "click" );

        if (typeof specific_prive != 'undefined' && field_specific_prive.length) {
            field_specific_prive.text(specific_prive);
        }

        $('.edit_field').closest('td.active_field').find('.form_edit_field').hide();

        $('.edit_field').closest('td').removeClass('active_field');
        $('.tagify-container input').val('');
        $('.table-product_td, .table.configuration td').removeClass('active_field');
...
$('.stage_images, .close_form_images').live({
    click: function (e) {
        e.preventDefault();
        $('.stage_images, .form_images, .form_location, .stage_location').fadeOut(500);
    }
});
...
$('.set_cover').live('click', function (e) {
    ...
    if ($(this).find('img').attr('alt') == 'enabled.gif') {
        return false;
    }
...
$('.saveAdditionalSettingProduct').live('click', function () {
   // tinyMCE.triggerSave();
    var data = {};
    $('.autoload_rte').each(function (index, value) {
      data[value.id] += $('#' + value.id).html( tinymce.get(value.id).getContent() );
    })
    ...
    $('.form_popup').find('#redirect option:selected').each(function () {
        data += '&type_redirect=' + $(this).data('type');
    });
    $('.form_popup').find('#id_category option:selected').each(function () {
        data += '&redirect_category=' + $(this).val();
    });
    $('.form_popup').find('#id_product option:selected').each(function () {
        data += '&redirect_product=' + $(this).val();
    });
...
$('.saveDescription').live('click', function () {
    ...
    for (var i in languages) {
        var field_id = field + languages[i]['id_lang'];
        data[field_id] = tinymce.get(field_id).getContent();
    }

    data.id_product = current_id_product;
    data.ajax = 1;
    data.action = 'save_description';
    ...
    $.ajax({
        ...
        success: function (r)
        {
            if (r.hasError)
            {
                if($('.form_popup').is(".content_form h3")) {
                    $('.form_popup .content_form h3').after('<div class="form_popup_error alert alert-danger error">' + r.errors.join('<br>') + '</div>');
                } else {
                    $('.form_popup h2').after('<div class="form_popup_error alert alert-danger error">' + r.errors.join('<br>') + '</div>');
                }
            }
            else
            {
...
/* tab more advanced stock management*/
$('.advanced_stock_management').live('click', function(e)
{
    var val = 0;
...
/*end tab more advanced stock management*/
...
// product activate for 1.5
$('[src $="disabled.gif"], [src $="enabled.gif"]').live('click', function (e) {
    if (!$(this).closest('.form_images').length) {
        var self = this;
        var id_product = $(this).closest('a').attr('href').replace(/.*id\_product\=([0-9]+).*/, '$1');
        ...
        $.ajax({
            ...
            success: function () {
                if ($(self).attr('src').search(/disabled/) !== -1) {
                    $(self).attr('src', $(self).attr('src').replace('disabled', 'enabled'));
                }
                else {
                    $(self).attr('src', $(self).attr('src').replace('enabled', 'disabled'));
                }
            }
        });
    }
...
$('.activ .list-action-enable').live('click', function (e) {
    e.preventDefault();
    var self = this;
    var id_product = $(this).attr('href').replace(/.*id\_product\=([0-9]+).*/, '$1');

    if ($(this).hasClass('action-enabled')) {
        active_title = action_disabled_title;
        active_desc = action_disabled_desc;
    } else {
        active_title = action_enabled_title;
        active_desc = action_enabled_desc;
    }
...
$('.order .list-action-enable').live('click', function (e) {
    e.preventDefault();
    var self = this;
    var id_product = $(this).attr('href').replace(/.*id\_product\=([0-9]+).*/, '$1');
    $.ajax({
...
function validateActiveField()
{
    ...
    if ($('.active_field :input').length && !$('.active_field :input:visible').length)
        return true;
    var field = $('td.active_field [data-event-save="1"]');
...
$(function () {
    ...
    $('.saveMoreCombination').live('click', function (e) {
        ...
        data['low_stock_threshold'] = parseInt($('.form_edit_attributes').find('#low_stock_threshold').val());
...
$('.createCombination').live('click', function ()
{
    ...
    data['low_stock_threshold'] = $('[name=low_stock_threshold]').val();
    ...
    $.ajax({
        ...
        success: function (r) {
            ...
            $([document.documentElement, document.body]).animate({
                scrollTop: $(".ajax_form_edit_attributes").offset().top - 30
            }, 2000);
        }
    });
});
...
$(function () {
    $('#getCustomizationBlock').live('click', function () {
        var block = $('#customization');
        block.addClass('loading');
        var data = {id_product: id_product, action: 'get_customization_block', ajax: 1};
        data.text_fields = block.find('[name="text_fields"]').val();
        data.uploadable_files = block.find('[name="uploadable_files"]').val();
        $.ajax({
            url: ajax_url,
            method: 'post',
            data: data,
            ...
            success: function(r) {
                if (r.hasError == true) {
                    ...
                    block.html(r.html);
                    var current_lang_fields = block.find('.lang-'+id_language);
                    block.find('.translatable-field').hide()
                    current_lang_fields.show();
...
$('.com_location').live('click', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (r) {
             $('.stage_location, .form_location').fadeIn(500);
             $('.form_location .content_form').html(r.content);
             $('.form_location').setCenterPositionAbsoluteBlockGP();
        }
    });
});
...
(function ($) {
    $.widget("ui.tagify2", {
        options: {
            delimiters: [188],          // what user can type to complete a tag in char codes: [enter], [comma]
        ...
        _create: function() {
            ...
            this.tagInput = $("<input type='text2'>")
                .attr( 'placeholder', opts.addTagPrompt )
                .attr('id',opts.id)
                .attr('data_lang', opts.data_lang)
                .attr('data_ids', opts.data_ids)
                .keypress( function(e) {
                    var $this = $(this),
                        pressed = e.which;
                        ...
                        if (pressed == opts.delimiters[i]) {
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ajax: true,
                                    action: 'save_tags',
                                    id_product: opts.data_ids,
                                    id_lang: opts.data_lang,
                                    tag_list: self.tags
                                },
                                success: function (r) {
                                    console.log(r);
                                    if (r.hasError) {
                                        $.alert(r.errors + ':  ' + r.tag);
