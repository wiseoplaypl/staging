...
{block name="td_content"}
    {if (isset($params.table) && ($params.table == 'product_attribute,stock_available' || $params.table == 'stock_available') && $tr['depends_on_stock'] == 1 && Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT'))}
        <a data-id="{$tr.id_product|escape:'quotes':'UTF-8'}" data-id-pa="{$tr.id_product_attribute|escape:'quotes':'UTF-8'}" class="button btn btn-default viewAdvancedStockManagement" href="#" title="{l s='Advanced stock management' mod='dgridproductslite'}">
    ...
    {else}
        ...
        <div class="{if isset($params.image_c)}edit_image {/if}{if isset($params.need_edit) && $params.need_edit}edit_field{if $params.validate == 'category'} edit_category{/if}{if $ps_v < 1.6} v15{/if}{/if}"
                ...
                {if $params.type == 'edit_attributes'}<a data-id="{$tr['id_product_attribute']|intval}" class="edit_attributes" href="#"><span>{/if}
                ...
                {else}
                    ...
                    {/if}
                        ...
                        {elseif $key == 'additional_setting_combination'}
                            <a data-id="{$tr.id_product_attribute|escape:'quotes':'UTF-8'}" class="button btn btn-default viewMoreSettingCombination" href="#">{l s='More' mod='dgridproductslite'}</a>
                        {else}
                ...
                {if $params.type == 'edit_attributes'}</span></a>{/if}
        </div>
        {if isset($params.need_edit) && $params.need_edit}
            {if !$params.lang}
                {if $params.validate != 'category'}
                    <div class="form_edit_field{if $ps_v < 1.6} v15{/if}">
                        <textarea {if $params.validate == 'price' || $params.validate == 'float'}onkeypress="return isNumberKey(event)"{/if}
                                data-event-save="1"
                                ...
                                class="type_{if isset($params.validate) && $params.validate}{$params.validate|escape:'quotes':'UTF-8'}{else}{$params.type|escape:'quotes':'UTF-8'}{/if}">{if !empty($tr["`$key`_no_format"])}{$tr["`$key`_no_format"]|escape:'quotes':'UTF-8'}{else}{if isset($tr.$key) && $tr.$key}{if $params.validate == price}{$tr.$key|replace:' ':''|replace:',':'.'|floatval}{else}{$tr.$key|escape:'quotes':'UTF-8'}{/if}{/if}{/if}</textarea>
                    </div>
                {else}
                    <div class="box_categories" style="display: none">
                        <div class="box_categories_stage"></div>
                        <div class="box_categories_form">
                            <div class="title_product">{l s='Edit product' mod='dgridproductslite'}: {$tr.name|escape:'quotes':'UTF-8'}
                                <a class="cancel_change_category btn btn-default">{l s='Cancel' mod='dgridproductslite'}</a>
                            </div>
                            <div class="form-group">
                                {$tr.view_category_box|escape:'mail':'UTF-8'}
                            </div>
                            <div class="form-group">
                                <label class="control-label col-lg-4" for="id_category_default" title="{l s='The default category is the main category for your product, and is displayed by default.' mod='dgridproductslite'}">
                                    {l s='Default category' mod='dgridproductslite'}
                                </label>
                                <div class="col-lg-5">
