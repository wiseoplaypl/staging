...
<table class="table tableDnD" style="width: 100%;" id="imageTable">
	<thead>
		<th>{l s='Image' mod='dgridproductslite'}</th>
		<th>{l s='Position' mod='dgridproductslite'}</th>
		<th>{l s='Legend' mod='dgridproductslite'}</th>
		{if isset($id_product_attribute)}
			<th>{l s='In combination' mod='dgridproductslite'}</th>
		{else}
			<th>{l s='Cover' mod='dgridproductslite'}</th>
		{/if}
		<th>{l s='Actions' mod='dgridproductslite'}</th>
	</thead>
	<tbody class="dgp_images" id="imageList">
				...
				<tr id="{$image.id_image|intval}" class="dgp_image_{$image.id_image|intval}">
					...
					{if isset($image.legend)}
						...
						<div style="width: 30%; float: left;">
							{if count($languages)}
							<select class="custom-select" data-switcher-translate="true">
								{foreach from=$languages item=l}
					...
					<td>
						<a href="#" data-image="{$image.id_image|intval}" class="delete_image button btn btn-default">
							<i class="icon-trash"></i> {l s='Delete' mod='dgridproductslite'}
						</a>
					</td>
				</tr>
			{/foreach}
		...
		<tr class="none_images" {if is_array($images) && count($images)}style="display: none;"{else}style="display: block;"{/if}>
			<td colspan="4">{l s='Not images' mod='dgridproductslite'}</td>
		</tr>
		{/if}
	</tbody>
</table>
