...
<h2>
    {l s='Advanced stock management' mod='dgridproductslite'}
</h2>
{if is_array($warehouses) && count($warehouses)}
    <table class="table advanced_stock_management_table">
        <thead>
            ...
            <th>
                {l s='Warehouse' mod='dgridproductslite'}
            </th>
            <th>
                {l s='Quantity' mod='dgridproductslite'}
            </th>
            <th>

            </th>
            <th>
                {l s='Price' mod='dgridproductslite'}
            </th>
            <th>
                {l s='Action' mod='dgridproductslite'}
            </th>
            <th>
            ...
            {foreach from=$warehouses item=warehouse}
                    <tr>
                        ...
                        <td>
                            <div class="row">
                                <div class="col-lg-8">
                                    <select class="custom-select" name="locations[{$warehouse.id_warehouse|intval}][action]">
                                        <option value="1">{l s='Increase' mod='dgridproductslite'}</option>
                                        <option value="-1">{l s='Decrease' mod='dgridproductslite'}</option>
                                    </select>
                                </div>
                                <div class="col-lg-4">
                        ...
                        <td>
                            <input name="locations[{$warehouse.id_warehouse|intval}][price]" type="text" value="{$base_price_product|escape:'quotes':'UTF-8'}">
                        </td>
                        <td>
                            ...
                            <button type="button" data-id-product="{$id_product|intval}" data-id-product-attribute="{$id_product_attribute|intval}" class="btn btn-default applyQuantityAdvancedStock">
                                {l s='Apply changes' mod='dgridproductslite'}
                            </button>
                        </td>
                    </tr>
            {/foreach}
