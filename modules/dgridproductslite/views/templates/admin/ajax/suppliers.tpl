...
<h3>{l s='Suppliers' mod='dgridproductslite'}</h3>
<div class="row">
	<div class="col-lg-3">
		<label class="control-label label-right">{l s='Suppliers of the current product' mod='dgridproductslite'}</label>
	</div>
	<div class="col-lg-9">
		<table class="table">
			<thead>
			<td>{l s='Selected' mod='dgridproductslite'}</td>
			<td>{l s='Name supplier' mod='dgridproductslite'}</td>
			<td>{l s='On default' mod='dgridproductslite'}</td>
			</thead>
			<tbody>
	...
	<div class="col-lg-12">
		<div class="alert alert-info">
            {if $associated_suppliers|@count == 0}
                {l s='You must specify the suppliers associated with this product. You must also select the default product supplier before setting references.' mod='dgridproductslite'}
            {else}
                {l s='You can specify product reference(s) for each associated supplier.' mod='dgridproductslite'}
            {/if}
            {l s='Click "Save and Stay" after changing selected suppliers to display the associated product references.' mod='dgridproductslite'}
		</div>
	</div>
	<div class="col-lg-12">
		<label class="control-label ">{l s='Supplier reference(s)' mod='dgridproductslite'}</label>
	</div>
	<div class="col-lg-12">
		<div class="" id="accordion-supplier">
					...
					<div id="supplier-{$supplier->id}">
						<div class="panel-body">
							<table class="table">
								<thead>
								<tr>
									<th><span class="title_box">{l s='Product name'  mod='dgridproductslite'}</span></th>
									<th><span class="title_box">{l s='Supplier reference'  mod='dgridproductslite'}</span>
									</th>
									<th>
										<span class="title_box">{l s='Unit price tax excluded'  mod='dgridproductslite'}</span>
									</th>
									<th><span class="title_box">{l s='Unit price currency'  mod='dgridproductslite'}</span>
									</th>
								</tr>
								</thead>
								<tbody>
                                {foreach $attributes AS $index => $attribute}
									...
									<tr {if $index is odd}class="alt_row"{/if}>
										...
										<td>
											<select class="custom-select" name="product_price_currency_{$attribute['id_product']}_{$attribute['id_product_attribute']}_{$supplier->id_supplier}">
