...
<div class="panel form_edit_attributes">
    <h3>{l s='Edit attributes' mod='dgridproductslite'}</h3>
    <input type="hidden" name="id_product_attribute" value="{$combination->id|intval}">
    <div class="row">
        <label class="col-lg-3 control-label">{l s='Attribute' mod='dgridproductslite'}</label>
        <div class="col-md-3">
            <select class="custom-select" name="attribute_group">
    ...
    {if is_array($attribute_groups) && count($attribute_groups)}
        {foreach from=$attribute_groups item=attribute_group}
            <div class="row" data-group="{$attribute_group.id_attribute_group|intval}">
                <label class="control-label col-lg-3">{l s='Value' mod='dgridproductslite'}</label>
                <div class="col-md-4">
    ...
    <div class="row row_selected_attr">
        <div class="col-md-4 col-md-offset-3">
            <select class="" id="product_att_list" name="attribute_combination_list[]" multiple="multiple">
    ...
    <div class="row">
        <div class="col-lg-12">
            <button class="btn btn-danger cancelEditAttributes" type="button"><i class="icon-remove"></i>{l s='Cancel' mod='dgridproductslite'}</button>
            <button class="btn btn-success saveAttributes" type="button"><i class="icon-save"></i>{l s='Save' mod='dgridproductslite'}</button>
        </div>
    </div>
</div>
<script>
