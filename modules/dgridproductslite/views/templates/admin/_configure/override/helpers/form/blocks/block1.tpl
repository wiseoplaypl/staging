    ...
    <div class="form-group">
        ...
        {foreach $field as $input}
            {if $input.name == 'short_description'}
                {block name="field"}
                    {block name="input"}
                        {if $input.type == 'radio' && $input.name == 'short_description'}
                            ...
                            {/foreach}
                        {elseif $input.type == 'switch' && $input.name == 'short_description'}
                            <div class="col-lg-9">
    ...
    <div class="form-group">
        ...
        {foreach $field as $input}
            {if $input.name == 'description'}
                {block name="field"}
                    {block name="input"}
                        {if $input.type == 'radio' && $input.name == 'description'}
                            {foreach $input.values as $value}
                                <div class="radio {if isset($input.class)}{$input.class}{/if}">
                                    {strip}
                                        <label>
                                ...
                                {if isset($value.p) && $value.p}<p class="help-block">{$value.p}</p>{/if}
                            {/foreach}
                        {elseif $input.type == 'switch' && $input.name == 'description'}
                            <div class="col-lg-9">
                                <span class="switch prestashop-switch fixed-width-lg">
                                    {foreach $input.values as $value}
                                        <input type="radio" name="{$input.name}"{if $value.value == 1} id="{$input.name}_on"{else} id="{$input.name}_off"{/if} value="{$value.value}"{if $fields_value[$input.name] == $value.value} checked="checked"{/if}{if isset($input.disabled) && $input.disabled} disabled="disabled"{/if}/>
