...
{foreach $field as $input}
    ...
    {else}
        ...
        {/if}
        {block name="label"}
            {if isset($input.label)}<label class="conf_title">{$input.label} </label>{/if}
        {/block}
        {block name="field"}
            <div class="margin-form">
                ...
                {if isset($fields_value[$input.name].image) && $fields_value[$input.name].image}
                    <div id="image">
                        ...
                        <p align="center">{l s='File size'  mod='dgridproductslite'} {$fields_value[$input.name].size}{l s='kb'  mod='dgridproductslite'}</p>
                        <a href="{$current}&{$identifier}={$form_id}&token={$token}&deleteImage=1">
                            <img src="../img/admin/delete.gif" alt="{l s='Delete'  mod='dgridproductslite'}" /> {l s='Delete'  mod='dgridproductslite'}
                        </a>
                    </div><br />
                {/if}
                {/if}
                    ...
                    <select name="{$key}" class="{if isset($input.class)}{$input.class}{/if}">
                        ...
                        {if $key == 'months'}
                            ...
                            {foreach $select as $k => $v}
                                <option value="{$k}" {if $k == $fields_value[$key]}selected="selected"{/if}>{l s=$v mod='dgridproductslite'}</option>
                            {/foreach}
                        {else}
                            {foreach $select as $v}
