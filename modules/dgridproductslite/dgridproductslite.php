...
class DGridProductsLite extends Module
{
    public function __construct()
    {
        $this->name = 'dgridproductslite';
        ...
        parent::__construct();
        $this->displayName = $this->l('Quick edit products(Lite)');
        $this->description = $this->l('Edit information about products');
        $this->module_key = '671ca0b9af4637c764997cfac7d70027';

        if (Tools::getValue('controller') == 'AdminProducts' && Tools::getValue('action') == 'productsList') {
    ...
    public function installTab()
    {
        ...
        foreach ($languages as $lang) {
            $name[$lang['id_lang']] = $this->l('Grid Products Lite');
        }
        $tab->name = $name;
    ...
    public function getImageLang($smarty)
    {
        ...
        $path = $smarty['path'];
        $module_path = 'dgridproductslite/views/img/';
    ...
    public function adminFormShowColumn()
    {
        ...
        $helper_form->override_folder = 'override/';
        $helper_form->base_tpl = version_compare(_PS_VERSION_, '1.6', '<')
            ? 'form15.tpl' : 'form16.tpl';
        $helper_form->fields_value = $fields_value;
        $helper_form->token = Tools::getValue('token');
        $helper_form->default_form_language = $this->context->employee->id_lang;
    ...
    public function adminFormShowColumnComb()
    {
        ...
        $fields_form = array(
            ...
            array(
                'label' => $this->l('Impact tax excl'),
                'name' => $prefix . 'price'
            ),
            array(
                'label' => $this->l('Impact tax incl'),
            ...
            array(
                'label' => $this->l('More'),
                'name' => $prefix . 'additional_setting_product'
            ),
            array(
                'label' => $this->l('Unit price product'),
                'name' => $prefix . 'unit_price_product'
            ),
        );
    ...
    public function assignDocumentation()
    {
        ...
        $this->context->controller->addCSS($this->getLocalPath() . 'views/css/admin-theme1_7.css');
        $this->context->controller->addCSS($this->getLocalPath() . 'views/css/documentation.css');

        if (version_compare(_PS_VERSION_, '1.7.0', '>')) {
            $this->context->controller->addCSS(($this->_path) . 'views/css/new-theme.css', 'all');
        }
