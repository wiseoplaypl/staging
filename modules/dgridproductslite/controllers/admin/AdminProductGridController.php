...
if (!class_exists('TransModDGP')) {
    require_once(_PS_MODULE_DIR_ . 'dgridproductslite/classes/TransModDGP.php');
}
if (!class_exists('DGridTools')) {
    require_once(_PS_MODULE_DIR_ . 'dgridproductslite/classes/DGridTools.php');
}

if (!class_exists('PackRowSeoSa')) {
    require_once(_PS_MODULE_DIR_ . 'dgridproductslite/classes/More/PackRowSeoSa.php');
}

if (version_compare(_PS_VERSION_, '1.6', '<')) {
    require_once(_PS_MODULE_DIR_ . 'dgridproductslite/classes/CustomizationField.php');
}
...
class AdminProductGridController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        a.`id_product` = sa.`id_product` AND sa.id_shop = ' . pSQL($id_shop) . ')
        LEFT JOIN `' . _DB_PREFIX_ . 'category_lang` cl
         ON (sa.`id_category_default` = cl.`id_category` AND b.`id_lang` = cl.`id_lang` 
            AND cl.id_shop = a.id_shop_default)
        LEFT JOIN `' . _DB_PREFIX_ . 'shop` shop ON (shop.id_shop = ' . pSQL($id_shop) . ')
        ...
        $this->_select .= 'a.`reference`, sa.`active`'. ' AS `active`' .
            (version_compare(_PS_VERSION_, '1.7.3', '>=')
                ? "," . "a.low_stock_threshold" : " ") .
            (version_compare(_PS_VERSION_, '1.7.5', '>=')
                ? "," . ' sav.`location`' : '') . '
           , shop.`name` AS `shopname`, a.`id_product` as `id`,
        ' . (version_compare(_PS_VERSION_, '1.6.1.0', '>=') ? 'image_shop.`id_image`' :
                'MAX(image_shop.`id_image`)') . ' AS `id_image`,
         cl.`name` AS `name_category`, sa.`price`, 0 AS `price_final`, 
        pd.`nb_downloadable`, sav.`quantity` AS `sav_quantity`, sa.`active`,sa.`available_for_order`,
        IF(sav.`quantity`<=0, 1, 0) AS `badge_danger`';

        if (Tools::getValue('carriers')) {
            $this->_join .= ' LEFT JOIN `' . _DB_PREFIX_ . 'product_carrier` pc ON (pc.`id_product` = a.`id_product`)';
        }
        $this->_join .= ' LEFT JOIN `' . _DB_PREFIX_ . 'product_supplier` ps ON (ps.`id_product` = a.`id_product`)';
        ...
        if ($this->search_attributes = Tools::getValue('attributes')) {
            $this->_join .= '
            LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute` pa
            ON (a.`id_product` = pa.`id_product`)';
            ...
            ON (pac.`id_attribute` = attribute.`id_attribute`)';

            $this->_where .= ' AND attribute.`id_attribute` IN(' . pSQL($this->search_attributes) . ')';
        }
        if (Tools::getValue('custom_feature')) {
            $search_custom = Tools::getValue('custom_feature');
            $ids = Db::getInstance()->executeS('
                SELECT fp.`id_product`
                FROM `' . _DB_PREFIX_ . 'feature_product` fp
                WHERE fp.`id_feature_value` in (
                SELECT fvf.`id_feature_value` 
                FROM `' . _DB_PREFIX_ . 'feature_value_lang` fvf 
                WHERE fvf.`value` =' . $search_custom . ' 
                AND fvf.`id_lang` = '. $this->context->language->id .') 
            ');
            $ids_product = "";
            if ($search_custom) {
                foreach ($ids as $item) {
                    $ids_product = $ids_product . $item['id_product'] . ',';
                }
                $this->custom_feature = Tools::substr($ids_product, 0, -1);
                $this->_where .= 'AND a.`id_product` IN (' . pSQL($this->custom_feature) . ')';
            }
        }
        ...
        if (Tools::getValue('carriers')) {
            $this->search_carriers = Tools::getValue('carriers');
            if (Tools::strrpos($this->search_carriers, ',', 1)) {
                $this->_where .= ' AND pc.`id_carrier_reference` IN (' . pSQL($this->search_carriers) . ')';
            } else {
                $this->_where .= ' AND pc.`id_carrier_reference` = ' . (int)$this->search_carriers;
            }
        }
        ...
        if ($date_from) {
            $this->_where .= ' AND a.`date_add` >= "' . $date_from . '"';
        }

        if ($date_to) {
            $this->_where .= ' AND a.`date_add` <= "' . $date_to . '"';
        }
        $low = Tools::getValue('low');
        if ($low == 1) {
            $this->low = 1;
            $this->_where .= (version_compare(_PS_VERSION_, '1.7.0.0', '>=') ? "
            AND a.`low_stock_threshold` >= sav.`quantity`" : " ");
        }
        $order_by = Tools::getValue('order_by');
        $order_way = Tools::getValue('order_way');
        if (empty(Tools::getValue('order_by'))) {
            $order_by = Tools::getValue('productOrderby');
            $order_way = Tools::getValue('productOrderway');
        }
        if ($order_by && $order_way) {
            switch ($order_by) {
                ...
                case 'wholesale_price':
                    $this->_orderBy = 'sa.wholesale_price';
                    break;
                case 'condition':
                    $this->_orderBy = 'a.condition';
                    break;
                case 'brand':
                    $this->_join .= '
                    LEFT JOIN ' . _DB_PREFIX_ . 'manufacturer manufacturer ON (
                    a.id_manufacturer = manufacturer.id_manufacturer)';
                    $this->_orderBy = 'manufacturer.name';
                    break;
                case 'date_add':
                    $this->_orderBy = 'a.date_add';
                    break;
                case 'weight':
                    $this->_orderBy = 'a.weight';
                    break;
                case 'low_stock_threshold':
                    $this->_orderBy =  (version_compare(_PS_VERSION_, '1.7.3.0', '>=') ?
                        'a.low_stock_threshold' : " ");
                    break;
            }
        ...
        // Перевод названия столбцов сетки
        $this->l('Short desc');
        $this->l('Photo');
        $this->l('Cat');
        $this->l('Name');
        ...
        $this->l('Stock location');
        $this->l('Low stock threshold');
        $this->l('Weight');
        foreach ($this->fields_list as $key => $element) {
            $this->fields_list[$key]['title'] = $this->l($element['title']);
        }
...
//        $this->addRowAction('edit');
    ...
    public function processDelete()
    {
        ...
        $redirect_after_params = parse_url($this->redirect_after, PHP_URL_QUERY);
        foreach (${'_GET'} as $param => $value) {
            if (strpos($redirect_after_params, $param . '=') === false && $param != 'deleteproduct') {
                $this->redirect_after .= '&' . $param . '=' . $value;
            }
        }
    ...
    public function getList(
        ...
        if ($this->context->cookie->productgridproductOrderby == 'brand') {
            $this->context->cookie->productgridproductOrderby = '';
            $this->context->cookie->write('productgridproductOrderby');
        };

        parent::getList($id_lang, $this->_orderBy, $this->_orderWay, $start, $limit, $this->context->shop->id);
        ...
        foreach ($this->_list as &$item) {
            ...
            $item['price_final'] = round($product_tax_calculator->addTaxes($item['price']), 6);
            ...
            ).'&id_product='.$item['id_product'].'&updateproduct';

            //$specific_price = SpecificPrice::getIdsByProductId($item['id_product']);

            $select_shop = Db::getInstance()->executeS(
                'SELECT id_shop FROM ' . _DB_PREFIX_ . 'specific_price WHERE `id_product` = ' . (int)$item['id_product']
            );
            $shop_page = Shop::getContextShopID();

            if ($shop_page != null) {
                $specific_price = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                    'SELECT `id_specific_price`
			FROM `' . _DB_PREFIX_ . 'specific_price`
			WHERE `id_product` = ' . (int)$item['id_product'] . '
            AND (`id_shop` = ' . $shop_page . ' OR `id_shop` = ' . 0 . ')'
                );
            } else {
                $specific_price = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                    'SELECT `id_specific_price`
            ...
            foreach ($select_shop as $key => $itemr) {
                if ((int)$itemr['id_shop'] == (int)$shop_page || (int)$shop_page == null) {
                    $item['has_specific_price'] = true;
                }
            }
            ...
            $item['has_specific_price'] = (
            ($specific_price && $specific_price_group)
            || ($specific_price && ($shop_page == null))
            || ($specific_price && ($shop_page == 0))
                ? true : false);
            ...
            $item['locale'] = (bool)Db::getInstance()->getValue('SELECT id_product FROM '
                . _DB_PREFIX_ . 'product_attribute WHERE id_product = ' . (int)$item['id_product']);

            // находим референт продукта
            $product_sup_refer = Db::getInstance()->getValue('SELECT product_supplier_reference FROM '
                . _DB_PREFIX_ . 'product_supplier WHERE id_product = ' . (int)$item['id_product']);
// если пусто значит товар с комбинациями
            if ($product_sup_refer == "") {
                // находим комбинацию по умолчанию
                if (version_compare(_PS_VERSION_, '1.6.1.0', '<=')) {
                    $default_attr = Db::getInstance()->getValue('SELECT id_product_attribute FROM '
                        . _DB_PREFIX_ . 'product_attribute 
                        WHERE id_product = ' . (int)$item['id_product'] . ' and default_on = 1');
                } else {
                    $default_attr = Db::getInstance()->getValue('SELECT id_product_attribute FROM '
                        . _DB_PREFIX_ . 'product_attribute_shop 
                        WHERE id_product = ' . (int)$item['id_product'] . ' and default_on = 1');
                }
// считываем ее референс
                $product_sup_refer = Db::getInstance()->getValue('SELECT product_supplier_reference FROM '
                    . _DB_PREFIX_ . 'product_supplier 
                WHERE id_product = ' . (int)$item['id_product'] . ' and id_product_attribute = ' . (int)$default_attr);
            }
// полученный референс вставляем в сетку
            $item['product_supplier_reference'] = $product_sup_refer;

            if ($item['unit_price_ratio'] > 0) {
                $ratio = $item['unit_price_ratio'];
                $price = Db::getInstance()->getValue('SELECT price FROM '
                    . _DB_PREFIX_ . 'product_shop 
                WHERE id_product = ' . (int)$item['id_product']);
                $item['unit_price_ratio'] = round($price / $ratio, 6);
            }
...
//            'tagify', // not work for 1.5
        ...
        $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/style.css');
        $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/drag_option.css');
        $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/jquery-confirm.css');
        $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/admin.css');
        $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/select2.min.css');
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/drag_option.js');
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/images.js');
        if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/compatibility.js');
        }
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/dgridproductslite.js');
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/tree_custom.js');
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/vendor/jquery.select2.js');
        if (version_compare(_PS_VERSION_, '1.6', '<')) {
            $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/bootstrap-dropdown.js');
            $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/fix_confirm_link15.js');
        }
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/jquery-confirm.js');
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/typeahead.min.js');
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/admin.js');
        $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/pack.js');
        ...
        if ((float)_PS_VERSION_ < 1.6) {
            ...
            $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/admin-theme.css');
            $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/admin-theme2.css');
            $this->context->controller->addCSS(_MODULE_DIR_ . 'dgridproductslite/views/css/font-awesome.min.css');
            $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/tree.js');
            $this->context->controller->addJS(_MODULE_DIR_ . 'dgridproductslite/views/js/typeahead.min.js');
        }
        ...
        $config = Tools::jsonDecode(Configuration::get('SEOSA_DGRID_COLUMN'), true);
        ...
        $this->context->smarty->assign(array(
            ...
            'lenght_short_desc' => isset($config['lenght_short_desc']) ? $config['lenght_short_desc'] : '',
            'lenght_desc' => isset($config['lenght_desc']) ? $config['lenght_desc'] : '',
            'datetimepicker' => $this->renderDateTimePickerForSearch(),
            'attributes' => DGridTools::getAttributes($this->context->language->id, false),
            'search_attributes' => explode(',', $this->search_attributes),
            'low' => $this->low
        ));
    ...
    public function renderDateTimePickerForSearch()
    {
        $fields_form = array(
            'form' => array(
                'legend' => array(
    ...
    public function renderForm()
    {
        ...
        $product->price = 0;
        $default_category = Category::getRootCategory($this->context->language->id, $shop = null);
        $product->id_category_default = $default_category->id;
        $product->save();
        $cat = array();
        $cat[] = 2;
        $product->updateCategories($cat);
    ...
    public function ajaxProcessSaveField()
    {
        ...
        if ($name == 'price' && $criterion == 'id_product' || $name == 'price_final' && $criterion == 'id_product') {
            $duo =  Db::getInstance()->executeS('SELECT price , unit_price_ratio 
                                                 FROM '. _DB_PREFIX_ . 'product_shop WHERE id_product = ' . $id);
            $koefic = $duo[0]['price']/$duo[0]['unit_price_ratio'];
        }

        if ($table == 'product_supplier') {
            $result = Db::getInstance()->executeS(
            ...
            $data = Db::getInstance()->executes('SELECT * FROM '
                . _DB_PREFIX_ . 'product_supplier WHERE id_product = ' . (int)$id . ' and id_product_attribute =  0');

            if (!empty($result) && !empty($data)) {
                foreach ($result as $id_comp) {
                    Db::getInstance()->execute(
                        'INSERT INTO ' . _DB_PREFIX_ . 'product_supplier 
                         SET `id_product` = ' . (int)$id . ', 
                         `id_product_attribute` =  ' . (int)$id_comp['id'] . ',
                          `id_supplier` =  ' . $data[0]['id_supplier'] . ',
                          `product_supplier_reference` = "' . $value . '",
                          `product_supplier_price_te` = ' . $data[0]['product_supplier_price_te'] . ',
                          `id_currency` = ' . $data[0]['id_currency'] . '
                          ON DUPLICATE KEY UPDATE `id_product_attribute`= ' . (int)$id_comp['id']
                    );
                }
            } else {
                if (empty($data)) {
                    ...
                    Db::getInstance()->execute('
                         ...
                         `id_product_attribute` =  0,
                          `id_supplier` =  '.(int)$suppliers[0]['id_supplier'].',
                          `product_supplier_reference` = "' . $value . '",
                          `product_supplier_price_te` = 0,
                          `id_currency` = ' . 1);

                    Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'product
                SET id_supplier = '.(int)$suppliers[0]['id_supplier'].'
                WHERE id_product = ' . (int)$id);
                }

                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'product_supplier
                SET product_supplier_reference = "' . $value . '"
                WHERE id_product = ' . (int)$id . ' and id_product_attribute = 0');
            }
        }

        if ($criterion == 'id_specific_price') {
            $this->updateObjectField('SpecificPrice', $name, $id, $value, $id_shop);
        ...
        if ($name == 'price_final' && $criterion == 'id_product') {
            ...
            $value_r = $value/$koefic;
            Db::getInstance()->execute('UPDATE `'. _DB_PREFIX_ . 'product_shop` ps, `'. _DB_PREFIX_ . 'product` p 
            SET p.`unit_price_ratio` = '. $value_r .', ps.`unit_price_ratio` = '. $value_r .'
             WHERE  p.`id_product` = ps.`id_product` AND p.`id_product` = ' . $id);
        }

        if ($name == 'unit_price_ratio' && $criterion == 'id_product') {
            $price =  Db::getInstance()->getValue('SELECT price FROM '
                . _DB_PREFIX_ . 'product WHERE id_product = ' . $id);
            $name = 'unit_price_ratio';
            $value = $price/$value;
            Db::getInstance()->execute('UPDATE '
                . _DB_PREFIX_ . 'product_shop SET `unit_price_ratio` = '. $value .' WHERE id_product = ' . $id);
        }
        ...
        if ($table && !is_array($table)) {
            Db::getInstance()->update($table, array(
                ...
                . (Shop::isFeatureActive() && $shop
                && $this->context->shop->getContext() !== Shop::CONTEXT_ALL ? ' AND id_shop = ' . (int)$id_shop : ''));
        } elseif ($table) {
            foreach ($table as $t) {
        ...
        if ($criterion == 'id_product') {
            $id_shop = Shop::getContextShopID();
            $this->updateObjectField('Product', 'date_upd', $id, date('Y-m-d H:i:s'), $id_shop);

            if ($name == 'id_tax_rules_group') {
                Db::getInstance()->execute('
                UPDATE `' . _DB_PREFIX_ . 'product_shop`
                SET `id_tax_rules_group` = ' . (int)$value . ', `date_upd` =  ' . '"'. date("Y-m-d H:i:s") .'"'.'
        ...
        if ($name == 'price' && $criterion == 'id_product') {
            $price =  Db::getInstance()->getValue('SELECT price FROM '
                . _DB_PREFIX_ . 'product WHERE id_product = ' . $id);
            $value = $price/$koefic;

            Db::getInstance()->execute('UPDATE `'. _DB_PREFIX_ . 'product_shop` ps, `'. _DB_PREFIX_ . 'product` p 
            SET p.`unit_price_ratio` = '. $value .', ps.`unit_price_ratio` = '. $value .'
             WHERE  p.`id_product` = ps.`id_product` AND p.`id_product` = ' . $id);
        }
        if ($criterion == 'id_product_attribute') {
            die(Tools::jsonEncode(array(
            ...
            'product_final_price' => Product::getPriceStatic($id_product, true, $id)
                ...
                die(Tools::jsonEncode(array(
                'hasError' => false,
                'product_final_price' => Product::getPriceStatic($id)
                )));
        }
    }
    ...
    public function getHtmlCombinationsAndImagesByProduct($id_product)
    {
        ...
        pas.`available_date`, ps.`product_supplier_reference`
        FROM ' . _DB_PREFIX_ . 'product_attribute pa
        INNER JOIN ' . _DB_PREFIX_ . 'product_attribute_shop pas
        ...
        $fields = array(
            ...
            'additional_setting_combination' => array(
                ...
                'need_edit' => false,
                'orderby' => false,
                'class' => 'additional_setting_combination',
                'search' => false,
                'validate' => 'no'
            ),
        );
        ...
        foreach ($combinations as &$combination) {
            ...
            if ($results = Db::getInstance()->getvalue($sql, $use_cache = 0)) {
                $id_image = $results;
            } else {
    ...
    public function ajaxProcessGetImages()
    {
        ...
        $content = $this->context->smarty->fetch(
            _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/images.tpl'
        );
    ...
    public function ajaxProcessSetCover()
    {
        ...
        if (Validate::isLoadedObject($image)) {
            ...
            @unlink(_PS_TMP_IMG_DIR_ . 'product_mini_' . (int)$image->id_product . '_' .
            ...
            if ($image->save()) {
                ...
                if (file_exists(_PS_PROD_IMG_DIR_ . $image->getImgFolder() . $image->id . '.jpg')) {
                    ...
                    foreach ($types as $type) {
                        if (file_exists(_PS_PROD_IMG_DIR_ . $image->getImgFolder() . $image->id . '-' .
                            $type['name'] . '.jpg')) {
                            unlink(_PS_PROD_IMG_DIR_ . $image->getImgFolder() . $image->id . '-' .
                                $type['name'] . '.jpg');
                        }
                        ImageManager::resize(
                    ...
                    $content = $this->context->smarty->fetch(
                        _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/single_image.tpl'
                    );
    ...
    public function ajaxProcessGetFeatures()
    {
        if (!Module::isEnabled('seosaextendedfeatures')) {
            ...
            foreach ($features as $k => $tab_features) {
                ...
                foreach ($product_features as $key => $tab_products) {
                    if ($tab_products['id_feature'] == $tab_features['id_feature']) {
                        $features[$k]['current_item'] = $tab_products['id_feature_value'];
                        if ($tab_products['custom'] == 1) {
                            $product_features[$key]['custom_value'] =
                                FeatureValue::getFeatureValueLang($features[$k]['current_item']);
                        }
                    }
                }
            ...
            die(Tools::jsonEncode(array(
                'hasError' => false,
                'content' => $this->context->smarty->fetch(
                    _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/features.tpl'
                )
            )));
        } else {
            $id_product = Tools::getValue('id_product');
        ...
        if (Validate::isLoadedObject($product = new Product((int)Tools::getValue('id_product')))) {
            ...
            if (version_compare(_PS_VERSION_, '1.7', '>')) {
                ...
                foreach ($postdata as $key => $val) {
                    if (preg_match('/^feature-([0-9]+)/i', $key, $match)) {
                        $post17[$ind]['id_feature'] = $val;
                    } elseif (preg_match('/^feature_values_([0-9]+)/i', $key, $match)) {
                        $post17[$ind]['id_feature_value'] = $val;
                    } elseif (preg_match('/^custom_([0-9]+)/i', $key, $match)) {
                        $post17[$ind][$match[0]] = $val;
                    } elseif (preg_match('/^end_([0-9]+)/i', $key, $match)) {
                        $ind = $ind + 1;
                    }
                }

                $post10 = array_unique($post17, SORT_REGULAR);
                foreach ($post10 as $key => $el) {
                    $test = "";
                    foreach ($el as $name => $it) {
                        if ($name != 'id_feature') {
                            $test = $test . $it;
                        }
                    }
                ...
                foreach ($post10 as $key => $element) {
                    if (!empty($element['id_feature_value']) && $element['id_feature_value'] != 0) {
                        $product->addFeaturesToDB($element['id_feature'], $element['id_feature_value'], 0);
                    } else {
                        $id_value = $product->addFeaturesToDB($element['id_feature'], 0, 1);
                        foreach ($languages as $language) {
                            foreach ($element as $k => $cust) {
                                if (preg_match('/^custom_([0-9]+)/i', $k, $match)
                                    && $language['id_lang'] == $match[1]) {
                                    if (!empty($element['custom_' . $language['id_lang']])) {
                                            $product->addFeaturesCustomToDB(
                                                $id_value,
                                                (int)$match[1],
                                                $element['custom_' . $language['id_lang']]
                                            );
                                    } else {
                                            $product->addFeaturesCustomToDB($id_value, (int)$match[1], "");
                                    }
                                }
                            }
                        }
                    ...
                    if (preg_match('/^feature_([0-9]+)_value/i', $key, $match)) {
                        if ($val) {
                            ...
                            if ($default_value = $this->checkFeatures($languages, $match[1])) {
                                ...
                                foreach ($languages as $language) {
                                    if ($cust = Tools::getValue('custom_' . $match[1] . '_' .
...
//        }
        ...
        die(Tools::jsonEncode(array(
            'hasError' => false,
            'content' => $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/seo.tpl'
            )
        )));
    }
    ...
    public function ajaxProcessGetCategories()
    {
        ...
        if (empty($selected_categories)) {
            $id_cat_def = Category::getRootCategory($this->context->language->id, $shop = null);
            $selected_categories = array($id_cat_def->id => array(
                "id_category" => ".$id_cat_def->id.",
                "name" => $id_cat_def->name,
                "link_rewrite" => $id_cat_def->link_rewrite,
            ));
        }
        if (empty($categories)) {
            $categories = array($id_cat_def->id);
        }
        $this->context->smarty->assign(array(
            'product' => new Product($id_product, false, $this->context->language->id),
        ...
        die(Tools::jsonEncode(array(
            'hasError' => false,
            'content' => $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/categories.tpl'
            )
        )));
    }
    ...
    public function ajaxProcessSetOrder()
    {
        $id_product = Tools::getValue('id_product');
        $product = new Product($id_product);
        if (Validate::isLoadedObject($product)) {
            $this->updateObjectField(
                ...
                'available_for_order',
                $product->id,
                ($product->available_for_order ? 0 : 1)
            );
            $st = ($product->available_for_order ? 0 : 1);
            $sql = 'UPDATE ' . _DB_PREFIX_ . 'product p SET p.`available_for_order`= ' . (int)$st . ' 
        ...
        if (Validate::isLoadedObject($product)) {
            ...
            if ($product->unit_price_ratio > 0) {
                $price = Db::getInstance()->getValue('
                         SELECT price from `' . _DB_PREFIX_ . 'product` where id_product = 1');

                $unit_price = $price / $product->unit_price_ratio;
            }
            ...
            if (Configuration::get('PS_CUSTOMIZATION_FEATURE_ACTIVE')) {
                $customization_fields = $this->getCustomizationFields($product);

                $this->context->smarty->assign(array(
                    'file_fields' => $customization_fields['file_fields'],
                    'text_fields' => $customization_fields['text_fields']
                ));
            }
            ...
            foreach ($attributes as $attribute) {
                if (!isset($attribute['attribute_designation'])) {
                    continue;
                }
            ...
            $pack_row = PackRowSeoSa::getPackDataForMore($product);
            // redirect
            $idShop = Shop::getContextShopID();

            $category_mini = array();
            $select_prod = array();
            $category_mini = Db::getInstance()->executeS('
SELECT `id_category`, `name` 
FROM `' . _DB_PREFIX_ . 'category_lang` 
WHERE `id_shop` = ' . $idShop . ' and `id_lang` = ' . $this->context->language->id);
            $select_prod = Db::getInstance()->executeS('
SELECT `id_product`, `name` 
FROM `' . _DB_PREFIX_ . 'product_lang` 
WHERE `id_shop` = ' . $idShop . ' and `id_lang` = ' . $this->context->language->id);
            if (version_compare(_PS_VERSION_, '1.7', '>')) {
                $item_type = Db::getInstance()->executeS('SELECT `redirect_type`, `id_type_redirected` FROM `' .
                    _DB_PREFIX_ . 'product_shop` WHERE `id_product` = ' . $product->id . ' and `id_shop` = ' . $idShop);
            } else {
                $item_type = 0;
            }
            $this->context->smarty->assign(array(
                'item_type' => $item_type,
                'product_mini' => $select_prod,
            ...
            die(Tools::jsonEncode(array(
                'hasError' => false,
                'content' => $this->context->smarty->fetch(
                    _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/more.tpl'
                )
            )));
        }
    }

    private function getCustomizationFields($product)
    {
        $file_fields = array();
        $text_fields = array();
        ...
        return array('file_fields' => $file_fields, 'text_fields' => $text_fields);
    }
    ...
    public function ajaxProcessGetDescription()
    {
        ...
        if (Validate::isLoadedObject($product)) {
            ...
            die(Tools::jsonEncode(array(
                'hasError' => false,
                'content' => $this->context->smarty->fetch(
                    _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/description.tpl'
                )
            )));
        }
    }
    ...
    public function ajaxProcessSaveDescription()
    {
        ...
        $description = array();
        foreach ($this->getLanguages() as $l) {
            $function = Product::$definition['fields'][$name]['validate'];
            $value = Tools::getValue($name . '_' . $l['id_lang']);
            ...
            if (Tools::strlen(strip_tags($value)) > $limit && $name == 'description_short') {
                $errors[] = sprintf(
                    $this->l('The number of characters in the Short Description  %s exceeds the limit %d now %d'),
                    $l['name'],
                    $limit,
            ...
            if (Tools::strlen(strip_tags($value)) > $limit_d && $name == 'description') {
                $errors[] = sprintf(
                    $this->l('The number of characters in the Description  %s exceeds the limit %d now %d'),
                    $l['name'],
                    $limit_d,
                    Tools::strlen(strip_tags($value))
                );
    ...
    public function ajaxProcessSaveMore()
    {
        ...
        $low_stock_threshold =  Tools::getValue('low_stock_threshold');
        ...
        $type_redirect = Tools::getValue('type_redirect');
        $redirect_category = (int)Tools::getValue('redirect_category');
        $redirect_product = (int)Tools::getValue('redirect_product');
        $additional_delivery_times = Tools::getValue('delivery_time');
        $delivery_in_stock = Tools::getValue('delivery_in_stock');
        $delivery_out_stock = Tools::getValue('delivery_out_stock');
        $id_lang_delivery_time = Tools::getValue('id_lang_delivery_time');
        ...
        foreach ($this->getLanguages() as $l) {
            ...
            if (Tools::strlen(strip_tags($description_short[$l['id_lang']])) > $limit) {
                $errors[] = sprintf(
                    $this->l('The number of characters in the Short Description  %s exceeds the limit %d now %d'),
                    $l['name'],
                    $limit,
                    Tools::strlen(strip_tags($description_short[$l['id_lang']]))
                );
            }

                $limit_d =  21000;

            if (Tools::strlen(strip_tags($description[$l['id_lang']])) > $limit_d) {
                $errors[] = sprintf(
                    $this->l('The number of characters in the Description  %s exceeds the limit %d now %d '),
                    $l['name'],
                    $limit_d,
                    Tools::strlen(strip_tags($description[$l['id_lang']]))
                );
            }
        }

//        if (!call_user_func(
//            array('Validate', Product::$definition['fields']['additional_delivery_times']['validate']),
//            $additional_delivery_times
//        )) {
//            $errors[] = $this->l('delivery time wrong');
//        }
        ...
        if ($ps_stock_management && !call_user_func(
            array(
                'Validate',
                Product::$definition['fields']['available_date']['validate']
            ),
            $available_date
        )
        ) {
        ...
        if (!call_user_func(
            array(
                'Validate',
                Product::$definition['fields']['wholesale_price']['validate']
            ),
            $wholesale_price
        )
        ) {
            ...
            if (Validate::isLoadedObject($product)) {
                if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
                    ...
                    StockAvailable::setProductDependsOnStock(
                        (int)$product->id,
                        Tools::getValue('depends_on_stock'),
                        $this->context->shop->id
                    );
                }
                StockAvailable::setProductOutOfStock($product->id, (int)Tools::getValue('out_of_stock'));
                ...
                $product->show_condition = $show_condition;
                ...
                if (version_compare(_PS_VERSION_, '1.7', '>')) {
                    $product->redirect_type = $type_redirect;
                    if ($type_redirect != '404') {
                        if ($redirect_category != '') {
                            $product->id_type_redirected = (int)$redirect_category;
                        }
                        if ($redirect_product != '') {
                            $product->id_type_redirected = (int)$redirect_product;
                        }
                    } else {
                        $product->id_type_redirect = 0;
                    }
                    //             dump($product->id_type_redirected);
                    Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'product_shop` 
                        SET redirect_type = ' . '"' . $type_redirect . '"' . ' , 
                        id_type_redirected = ' . $product->id_type_redirected . ' 
                        WHERE id_product=' . $id_product);
                    Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'product` 
                        SET `additional_delivery_times` = ' . (int)$additional_delivery_times . ' 
                        WHERE `id_product` = ' . $id_product);
                    Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'product_lang` 
                        SET `delivery_in_stock` = ' . '"' . $delivery_in_stock . '"' . ', 
                        `delivery_out_stock` = ' . '"' . $delivery_out_stock . '"' . '
                        WHERE `id_product` = ' . $id_product . ' AND `id_lang` = ' . $id_lang_delivery_time);
                }
                ...
                $un = (float)$product->unit_price > 0 ? $product->price / $product->unit_price : 0 ;
                $un = round($un, 6);
                Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'product` 
                        SET `unit_price_ratio` = ' . $un . ' 
                ...
                PackRowSeoSa::updatePackItems($product);
    ...
    public function updateSuppliers(Product $product)
    {
        ...
        foreach ($attributes as $attribute) {
            foreach ($associated_suppliers as $supplier) {
                if (Tools::isSubmit(
                    'supplier_reference_' . $product->id . '_' . $attribute['id_product_attribute'] . '_'
                        . $supplier->id_supplier
                ) || (Tools::isSubmit('product_price_' . $product->id . '_' . $attribute['id_product_attribute'] . '_'
                        . $supplier->id_supplier)
                        && Tools::isSubmit('product_price_currency_' . $product->id . '_'
                            . $attribute['id_product_attribute'] . '_' . $supplier->id_supplier)
                    )
                ) {
                    $reference = pSQL(Tools::getValue('supplier_reference_'
                        . $product->id . '_' . $attribute['id_product_attribute']
                        . '_' . $supplier->id_supplier, ''));

                    $price = (float)str_replace(
                        array(' ', ','),
                        array('', '.'),
                        Tools::getValue(
                            'product_price_' . $product->id . '_'
                            . $attribute['id_product_attribute'] . '_' . $supplier->id_supplier,
                            0
                        )
                    );
                    ...
                    $id_currency = (int)Tools::getValue(
                        'product_price_currency_' . $product->id . '_'
                        . $attribute['id_product_attribute'] . '_' . $supplier->id_supplier,
                        0
                    );
                    ...
                    $product_supplier_id = (int)ProductSupplier::getIdByProductAndSupplier(
                        $product->id,
                    ...
                    if (!$product_supplier_id) {
                        ...
                        if ($product->id_supplier == $supplier->id_supplier) {
                            if ((int)$attribute['id_product_attribute'] > 0) {
                                ...
                                $product->wholesale_price = (float)Tools::convertPrice($price, $id_currency);
                ...
                } elseif (Tools::isSubmit('supplier_reference_' .$product->id . '_' . $attribute['id_product_attribute']
                    . '_' . $supplier->id_supplier)) {
                    if ((int)$attribute['id_product_attribute'] > 0) {
                        $product_supplier = new ProductSupplier();
                        $product_supplier->id_product = $product->id;
        ...
        if ($new_default_supplier != $product->id_supplier) {
            Db::getInstance()->getValue(
                ...
                SET `id_supplier` = ' . $new_default_supplier . '
    ...
    public function ajaxProcessGetSpecificPrice()
    {
        ...
        if (Validate::isLoadedObject($product)) {
            ...
            $ids_shop = Shop::getContextListShopID();
            foreach ($specific_prices as $key => $spec) {
                if ($spec['id_shop'] && isset($ids_shop[0]) && $spec['id_shop'] != $ids_shop[0]) {
                    unset($specific_prices[$key]);
                }
            }
            $tmp = array();
            ...
            $this->context->smarty->assign(array(
                ...
                'js_mod_dir' => _MODULE_DIR_ . 'dgridproductslite/views/js/',
            ...
            die(Tools::jsonEncode(array(
                'hasError' => false,
                'content' => $this->context->smarty->fetch(
                    _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/specific_price.tpl'
                )
            )));
        }
    }
        ...
        if (!count($errors)) {
            if ($product->productAttributeExists(Tools::getValue('attribute_combination_list'))) {
                ...
                $id_product_attribute = $product->addCombinationEntity(
                    ...
                    '',
                    Tools::getValue('low_stock_threshold')
                );
    ...
    public function ajaxProcessEditAttributes()
    {
        ...
        die(Tools::jsonEncode(array(
            'hasError' => false,
            'content' => $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/edit_attributes.tpl'
            )
        )));
    }
    ...
    public function ajaxProcessMoreSettingCombination()
    {
        ...
        die(Tools::jsonEncode(array(
            'hasError' => false,
            'content' => $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/more_combination.tpl'
            )
        )));
    }
    ...
    public function ajaxProcessSaveMoreCombination()
    {
        ...
        if (Validate::isLoadedObject($combination)) {
            ...
            $low_stock_threshold= (int)Tools::getValue('low_stock_threshold');
    ...
    public function initContent()
    {
        if ($this->display != 'edit' && $this->display != 'add') {
            ...
            $this->tpl_list_vars['category_tree'] = $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/tree.tpl'
            );
            $this->tpl_list_vars['base_url'] = preg_replace(
                '#&id_category=[0-9]*#',
                '',
                self::$currentIndex
            ) . '&token=' . $this->token;
    ...
    public function ajaxProcessAdvancedStockManagement()
    {
        ...
        $this->context->smarty->assign(
            array(
                ...
                'id_product' => $id_product,
                'id_product_attribute' => $id_product_attribute,
                'base_price_product' => Product::getPriceStatic(
                    (int)$id_product,
                    false,
                    0,
                    6,
    ...
    public function ajaxProcessSaveAdvancedStockManagement()
    {
        ...
        die(Tools::jsonEncode(array(
            'hasError' => false,
            'message' => $this->l('Settings appplied successfully!')
        )));
    }

    public function updateObjectField($class_name, $field, $id, $value, $id_shop = null)
    {
        if (is_null($id_shop)) {
        ...
        foreach ($only_one as $item) {
            $item_arr = explode('-', $item);
            if ($fix_only_one = $class_name == $item_arr[0]
                && $field == $item_arr[1]) {
                break;
            }
        }
        if ($class_name == "SpecificPrice") {
            $fix_only_one = true;
        }
        ...
        $prod_attr = array("width", "height", "weight", "depth");

        if (in_array($field, $prod_attr)) {
            $sql2 = 'UPDATE ' . _DB_PREFIX_ . $definition['table'];
            $sql2 .= ' SET `' . pSQL($field) . '` = "' . ObjectModel::formatValue($value, $definition_field['type'])
                . '"';
            $sql2 .= ' WHERE `' . $definition['primary'] . '` = ' . (int)$id;
            if ($multi_shop_active) {
                $sql2 .= ' AND `id_shop` IN('
                    . (count($ids_shop) ? implode(',', array_map('intval', $ids_shop)) : 'NULL') . ')';
            }
        ...
        if (!$lang && $shop) {
            if ($field == 'condition') {
                $sql_shop = 'UPDATE ' . _DB_PREFIX_ . $definition['table'];
            } else {
    ...
    public function ajaxProcessGetCustomizationBlock()
    {
        $errors = array();
        $id_product = (int)Tools::getValue('id_product');
        $product = new Product($id_product);
        if (!Validate::isLoadedObject($product)) {
            $errors = $this->l('No object');
        }
        $product->text_fields = Tools::getValue('text_fields');
        $product->uploadable_files = Tools::getValue('uploadable_files');
        ...
        if (!$product->createLabels(
            (int)$product->uploadable_files - $files_count,
            (int)$product->text_fields - $text_count
        )
        ) {
            $errors[] = Tools::displayError('An error occurred while creating customization fields.');
        }
        if (!count($this->errors) && !$product->updateLabels()) {
            $errors[] = Tools::displayError('An error occurred while updating customization fields.');
        }
        $product->customizable = ($product->uploadable_files > 0 || $product->text_fields > 0) ? 1 : 0;
        if (($product->uploadable_files != $files_count || $product->text_fields != $text_count)
            && !count($this->errors) && !$product->update()) {
            $errors[] = Tools::displayError('An error occurred while updating the custom configuration.');
        }
        ...
        die(Tools::jsonEncode(array(
            ...
            'html' => $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/customization.tpl'
            )
        )));
    }

    public function getLinkProductForList($echo)
    {
            $product = new Product((int)$echo);
            $url = $this->getPreviewUrl($product);
        $this->context->smarty->assign(array(
            'url_product_link' => $url,
            'id_product' => $echo
        ));
        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/product_link.tpl');
    }

    public function getPreviewUrl(Product $product, $preview = true)
    {
        $id = Tab::getIdFromClassName('AdminProducts');
        $token = Tools::getAdminToken('AdminProducts' . (int)$id . (int)$this->context->employee->id);
        ...
        if (version_compare(_PS_VERSION_, '1.7.0.0', '<')) {
            ...
            $category_link = Category::getLinkRewrite($product->id_category_default, $this->context->language->id);
        } else {
           $link_rewrite = $this->getFieldValue(
                $product,
                'link_rewrite',
                $this->context->language->id
            );
           $category_link =  Category::getLinkRewrite(
        ...
        if (!$product->active  && $preview) {
            ...
            $preview_url .= ((strpos($preview_url, '?') === false) ? '?' : '&')
                . 'adtoken=' . $token . '&ad=' . $admin_dir . '&id_employee=' . (int)$this->context->employee->id;
        }
    ...
    {
        if (isset($this->tabAccess['delete']) && $this->tabAccess['delete']) {
            if (is_array($this->boxes) && !empty($this->boxes)) {
                $object = new $this->className();
                if (isset($object->noZeroObject) &&
                    // Check if all object will be deleted
                    (count(call_user_func(array($this->className, $object->noZeroObject))) <= 1
                        || count(${'_POST'}[$this->table . 'Box'])
                        == count(call_user_func(array($this->className, $object->noZeroObject))))) {
                    $this->errors[] = Tools::displayError('You need at least one object.')
                        . ' <b>' . $this->table . '</b>
 <br />' . Tools::displayError('You cannot delete all of the items.');
                } else {
                    $success = 1;
                            ...
                            if (!count($this->errors)) {
                                if ($product->delete()) {
                                    PrestaShopLogger::addLog(
                                        sprintf(
                                            $this->l('%s deletion', 'AdminTab', false, false),
                                            $this->className
                                        ),
                                        1,
                                        null,
    ...
    public function ajaxProcessGetlocation()
    {
        $id_product = Tools::getValue('id_product');
        $id_product_attribute = Tools::getValue('id_product_attribute');
       ...
       SELECT  GROUP_CONCAT(al.`name`) as names,sa.`location`, pac.`id_product_attribute` 
       FROM `' . _DB_PREFIX_ . 'product_attribute_combination` pac 
	   LEFT JOIN `' . _DB_PREFIX_ . 'attribute_lang` al 
	   ON pac.`id_attribute` = al.`id_attribute` AND al.`id_lang` = ' . $id_lang . '
	   ...
	   IN (select pa.`id_product_attribute` 
	   FROM `' . _DB_PREFIX_ . 'product_attribute` pa 
	   ...
	   Group BY pac.`id_product_attribute`
        ...
        $content = $this->context->smarty->fetch(
            _PS_MODULE_DIR_ . 'dgridproductslite/views/templates/admin/ajax/combination_locale.tpl'
        );
    ...
    public function ajaxProcessGetlocale()
    {
        $id_product = (int)Tools::getValue('id_product');
        $id_attr = (int)Tools::getValue('id_product_attribute');
        $locale_text = Tools::getValue('locale_text');

        Db::getInstance()->execute('
        UPDATE ' . _DB_PREFIX_ . 'product_attribute pa
        left join ' . _DB_PREFIX_ . 'stock_available sa 
        ON sa.`id_product_attribute` = ' . (int)$id_attr . ' AND sa.`id_product` = ' . (int)$id_product . '
        set pa.`location` = ' . '"' . $locale_text . '"' . ', sa.`location` = ' . '"' . $locale_text . '"' . '  
        where pa.`id_product` = ' . $id_product . ' AND pa.`id_product_attribute` = ' . $id_attr . ' 
        ');
    ...
    public function ajaxProcessSaveTags()
    {
        $id_product = (int)Tools::getValue('id_product');
        $id_lang = (int)Tools::getValue('id_lang');
        $tag_list = Tools::getValue('tag_list');
        foreach ($tag_list as $el) {
            if (preg_match("/[!<;>;?=+#\"{}_$%]/", $el)) {
                die(Tools::jsonEncode(array(
                    'hasError' => true,
                    'errors' => $this->l('Forbidden characters in tag'),
                    'tag' => $el
                )));
                return true;
            }
        }
        $tag_list_no_empty = array_filter(
            $tag_list,
            function ($value) {
                return $value !== '';
            }
        );
        $tags_list_format = implode(",", $tag_list_no_empty);
        self::deleteTagsForProduct($id_product, $id_lang);
        Tag::addTags($id_lang, $id_product, $tags_list_format, $separator = ',');
        die(Tools::jsonEncode(array(
            'hasError' => false,
        )));
    }

    public static function deleteTagsForProduct($id_product, $id_lang)
    {
        $tags_removed = Db::getInstance()->executeS(
            'SELECT id_tag FROM '._DB_PREFIX_.'product_tag 
            WHERE id_product='.(int)$id_product . ' AND `id_lang` ='. $id_lang
        );
        $result = Db::getInstance()->delete('product_tag', 'id_product = '.(int)$id_product . ' AND `id_lang` = '
            . (int)$id_lang);
        Db::getInstance()->delete('tag', 'NOT EXISTS (SELECT 1 FROM '._DB_PREFIX_.'product_tag
        												WHERE '._DB_PREFIX_.'product_tag.id_tag = '
            ._DB_PREFIX_.'tag.id_tag)');
        $tag_list = array();
...
//        }
    ...
    public function ajaxProcessGetAttribut()
    {
        $idLang = Tools::getValue('id_lang');
        $temp = FeatureValue::getFeatureValuesWithLang((int)$idLang, (int)Tools::getValue('id'), false);
        $id_select = str_replace(array('form_features_', '_value'), "", Tools::getValue('id_select'));
        $name = 'feature_' . Tools::getValue('id');
        die(Tools::jsonEncode(array(
            'hasError' => false,
            'content' => $temp,
