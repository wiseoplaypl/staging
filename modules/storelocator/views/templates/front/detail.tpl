...
{capture name=path}{if isset($store.name) && !empty($store.name)}{$store.name|escape:'htmlall':'UTF-8'}{else}{l s='Store number' mod='storelocator'} {$store.id_store|escape:'htmlall':'UTF-8'}{/if}{/capture}
...
<div class="row{if $_ps_ver > 0} ps_onesix_specific{/if}" id="fmmsl_single_store_maparea">
...
{/if}
<br />
<a href="{$link->getPageLink('stores', true)|escape:'htmlall':'UTF-8'}" class="btn btn-default button button-small" title="{l s='Back to Stores' mod='storelocator'}"><span>{l s='Back to Stores' mod='storelocator'}</span></a>
