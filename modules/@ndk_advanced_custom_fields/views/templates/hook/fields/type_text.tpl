...
<div class="form-group ndkackFieldItem field-type-{$field.type}" data-iteration="{$field_iteration}"  data-id="{$field.target|escape:'htmlall'}" data-view="{$field.target_child|escape:'htmlall'}" data-name="{$field.name|escape:'htmlall'}" data-name="{$field.name|lower|escape:'htmlall'}" data-admin-name="{$field.admin_name|lower|escape:'htmlall'}" data-field="{$field.id_ndk_customization_field|escape:'htmlall'}" data-quantity-link="{$field.quantity_link}" {foreach from=$field.options key=k item=v} data-{$k}="{$v}"{/foreach}>
	...
	<div class="fieldPane clearfix">
		...
		{if $field.configurator == 1}
			<textarea id="ndkcsfield_{$field.id_ndk_customization_field|intval}" {$smarty.capture.dataPattern nofilter} data-lines="{$field.nb_lines|escape:'htmlall'}" data-max="{$field.maxlength}"  data-message="{l s='Informe' mod='ndk_advanced_custom_fields'} {$field.name|escape:'htmlall'}"  name="ndkcsfield[{$field.id_ndk_customization_field|intval}]" class="{if $field.is_visual == 1}visual-effect {/if} form-control {if $field.configurator == 1} textzone{/if} ndktextarea {if $field.required == 1} required_field{/if}"  data-group="{$field.id_ndk_customization_field|intval}" data-zindex="{$field.zindex|escape:'htmlall'}" data-price="{$fieldPrice|escape:'htmlall'}" data-ppcprice="{$field.price_per_caracter|escape:'htmlall'}"  data-id="{$field.target|escape:'htmlall'}" data-view="{$field.target_child|escape:'htmlall'}" data-dragdrop="{$field.draggable|intval}" 
			data-resizeable="{$field.resizeable|intval}" 
			data-rotateable="{$field.rotateable|intval}" data-path="{$field.svg_path|escape:'html'}"  data-blend="{$field.color_effect}" {if $field.is_mask_image}data-mask-image="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/mask/{$field.id_ndk_customization_field|intval}.jpg"{/if}></textarea>
		{else}
		<div class="clearfix clear bordered">
			<textarea id="ndkcsfield_{$field.id_ndk_customization_field|intval}" data-max="{$field.maxlength}" {if $field.maxlength > 0}maxlength="{$field.maxlength}"{/if} {$smarty.capture.dataPattern nofilter} data-message="{l s='Informe' mod='ndk_advanced_custom_fields'} {$field.name|escape:'htmlall'}"  name="ndkcsfield[{$field.id_ndk_customization_field|intval}]" class="{if $field.required == 1} required_field{/if} form-control simpleText ignoreMaxLength {if $field.is_visual == 1}visual-text{/if}"  data-group="{$field.id_ndk_customization_field|intval}" data-zindex="{$field.zindex|escape:'htmlall'}" data-price="{$fieldPrice|escape:'htmlall'}" data-ppcprice="{$field.price_per_caracter|escape:'htmlall'}" placeholder="{$field.name|escape:'htmlall'} {if $fieldPrice > 0}{l s='cost : ' mod='ndk_advanced_custom_fields'}{convertPrice price=$fieldPrice}{/if}"  data-id="{$field.target|escape:'htmlall'}" data-view="{$field.target_child|escape:'htmlall'}" data-dragdrop="{$field.draggable|intval}" 
			data-resizeable="{$field.resizeable|intval}" 
			data-rotateable="{$field.rotateable|intval}"
			data-path="{$field.svg_path|escape:'html'}" 
			data-lines="{$field.nb_lines|escape:'htmlall'}" 
			 data-blend="{$field.color_effect}" {if $field.is_mask_image}data-mask-image="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/mask/{$field.id_ndk_customization_field|intval}.jpg"{/if}></textarea>
