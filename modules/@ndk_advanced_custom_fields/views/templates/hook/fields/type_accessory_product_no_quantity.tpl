	...
	>{$field.name|escape:'htmlall'}
		...
		<div class="fieldPane clearfix">
			...
			<div class="minmaxBlock">
				...
				<div class="total_weight_container total_row">
					{l s='Total quantity' mod='ndk_advanced_custom_fields'} <span class="total_qtty">0</span>
				</div>
				{/if}
			...
			{if $field.quantity_min > 0}
				<p data-name="ndkcsfield[{$field.id_ndk_customization_field|intval}]" class=" alert-danger clear clearfix quantity_error_down  {if $field.quantity_min > 0}required_field{/if}" val="">{l s="You must add a minimum of " mod='ndk_advanced_custom_fields'}{$field.quantity_min} {l s='quantities' mod='ndk_advanced_custom_fields'}</p>
			{/if}
			{if $field.weight_min > 0}
				<p data-name="ndkcsfield[{$field.id_ndk_customization_field|intval}]" class=" alert-danger clear clearfix weight_error_down  {if $field.weight_min > 0}required_field{/if}" val="">{l s="You must add a minimum of " mod='ndk_advanced_custom_fields'}{$field.weight_min} {Configuration::get('PS_WEIGHT_UNIT')}</p>
			{/if}
			</div>
			<!-- bloc tags -->
			...
			{include file='module:ndk_advanced_custom_fields/views/templates/hook/values/type_accessory_product_no_quantity.tpl'}
			</ul>
