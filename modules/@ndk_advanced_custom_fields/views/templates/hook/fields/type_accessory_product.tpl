	...
	>{$field.name|escape:'htmlall'}
		...
		<div class="fieldPane clearfix">
			...
			{if $field.weight_max > 0}
				<p class="weight_error_up alert-danger clear clearfix">{l s="You can't add more than " mod='ndk_advanced_custom_fields'}<span>{$field.weight_max} {Configuration::get('PS_WEIGHT_UNIT')}</span></p>
			{/if}
			...
			<div class="clearfix clear" id="main-{$field.id_ndk_customization_field|intval}">
				<div class="clearfix clear col-xs-12 clearfix visu-tools"></div>
			
			<!-- bloc tags -->
			<ul class="ndk_accessory_list simple_accessory_product clearfix clear">
				{include file='module:ndk_advanced_custom_fields/views/templates/hook/values/type_accessory_product.tpl'}
			</ul>
