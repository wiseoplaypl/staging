		...
		<div class="fieldPane clearfix">
			...
			<div class="clearfix clear" id="main-{$field.id_ndk_customization_field|intval}">
				...
				{if $field.quantity_max > 0}
					<p class="max-limit">{l s='You have reached the maximum of  %d underwear' sprintf=[$field.quantity_max]  mod='ndk_advanced_custom_fields'}</p>
				{/if}
				{if $field.quantity_min > 0}
					<p data-name="ndkcsfield[{$field.id_ndk_customization_field|intval}]" class=" alert-danger clear clearfix quantity_error_down required_field" val="">{l s='You have to add a minimum of  %d underwear' sprintf=[$field.quantity_min]  mod='ndk_advanced_custom_fields'}</p>
				{/if}
				...
				{*IMAGES*}
					...
					<div class="ndkhiddenimglibrary">
						...
						{include file='module:ndk_advanced_custom_fields/views/templates/hook/values/type_underwear.tpl'}	
			...
			<button 
				...
				data-min-item="{$field.quantity_min}"
...
<script type="text/javascript">
	...
	{if $field.alignments !=''}
		...
		{/foreach}
	{/if}
	 typeText['model'] = "{l s='Model' mod='ndk_advanced_custom_fields' js='1'}";
	 typeText['size'] = "{l s='Size' mod='ndk_advanced_custom_fields' js='1'}";
	 typeText['zone'] = "{l s='Zone' mod='ndk_advanced_custom_fields' js='1'}";
	 typeText['name'] = "{l s='Text' mod='ndk_advanced_custom_fields' js='1'}";
	
</script>
	
{addJsDefL name=designerRemoveText}{l s='remove' mod='ndk_advanced_custom_fields' js='1'}{/addJsDefL}

{addJsDefL name=underwearText}{l s='Underwear' mod='ndk_advanced_custom_fields' js='1'}{/addJsDefL}
{addJsDefL name=designerImgText}{l s='> Item (image)' mod='ndk_advanced_custom_fields' js='1'}{/addJsDefL}
{addJsDefL name=designerTextText}{l s='> Item (text)' mod='ndk_advanced_custom_fields' js='1'}{/addJsDefL}
{addJsDefL name=designerValue}{l s='see picture' mod='ndk_advanced_custom_fields' js='1'}{/addJsDefL}
