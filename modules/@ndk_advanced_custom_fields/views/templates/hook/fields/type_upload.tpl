...
<div class="form-group ndkackFieldItem field-type-{$field.type}" data-iteration="{$field_iteration}"  data-id="{$field.target|escape:'htmlall'}" data-view="{$field.target_child|escape:'htmlall'}" data-name="{$field.name|escape:'htmlall'}" data-field="{$field.id_ndk_customization_field|escape:'htmlall'}" {if $field.is_mask_image}data-mask-image="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/mask/{$field.id_ndk_customization_field|intval}.jpg"{/if} data-quantity-link="{$field.quantity_link}" {foreach from=$field.options key=k item=v} data-{$k}="{$v}"{/foreach}>
		<label class="toggler {if !$field.is_picto}toggler-default-picto{/if} lazyload"
		{if $field.is_picto} 
		{if $ndkccpwebp}
		{include file="module:ndk_custom_product_page/views/templates/hook/srcset-background-by-path.tpl" path='img/scenes/ndkcf/pictos/'|cat:$field.id_ndk_customization_field|cat:'.jpg' sizes=[['width' => 100, 'screen' => 4000]]}
		{else}
		style="background-image: url('{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/pictos/{$field.id_ndk_customization_field|intval}.jpg');"{/if}
		{/if}
	>{$field.name|escape:'htmlall'} {if $field.show_price == 1}{if $fieldPrice > 0}{l s='cost : ' mod='ndk_advanced_custom_fields'} {convertPrice price=$fieldPrice}{/if}{/if}
		...
		<div class="fieldPane clearfix clear clearfix">
			...
			{/if}
				...
				{/if}
				<input type="file" data-group="{$field.id_ndk_customization_field|intval}" id="" class="upload_ndk_visu form-control" data-action="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}modules/ndk_advanced_custom_fields/uploadNdkcsfields.php" name="ndk_tmp_name" data-blend="{$field.color_effect}"/>
...
{* 						{if $field.is_mask_image}data-mask-image="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/mask/{$field.id_ndk_customization_field|intval}.jpg"{/if} *}
						 data-price="{$fieldPrice|escape:'htmlall'}" data-id="{$field.target|intval}" data-view="{$field.target_child|intval}"/>
