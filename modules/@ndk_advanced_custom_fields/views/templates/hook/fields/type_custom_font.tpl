...
<script>
	...
	window['ndkcfCustomFont_{$field.id_ndk_customization_field}'] = [];
</script>
	...
	{foreach from=$field.values item=value}
		{assign var="letters" value="|"|explode:$value.value}
			...
			window['ndkcfCustomFont_{$field.id_ndk_customization_field}']['{$letter}'] = '{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/{$value.id}{if $value.issvg}.svg{else}.jpg{/if}';
