				 ...
				 <div class="clear clearfix accessory-infos">
					 ...
					 {if 1 == 1 || Product::isAvailableWhenOutOfStock($product.out_of_stock)}
						 ...
						 {if $value.step_quantity !=''}
							...
							{/foreach}
						 {/if}
							 <input type="number" name="ndkcsfield[{$field.id_ndk_customization_field|intval}][quantityProd][{$value.id|intval}|{$value.id_product_value|intval}|0]" {if $value.set_quantity == 1}data-qtty-available="{$value.quantity|intval}" {/if}  data-qtty-max="{$value.quantity_max|intval}"  data-qtty-min="{$value.quantity_min|intval}" type="text" class="ndk-accessory-quantity {if $value.price > 0 || $field.price > 0}price_overrided{/if}" id="ndk-accessory-quantity-{$value.id|intval}" 
							 value="0"
							 data-default-value="{$value.default_value|intval}" 
