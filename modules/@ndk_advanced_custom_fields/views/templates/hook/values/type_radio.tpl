...
*}
			...
			{foreach from=$field.values item=value}
				...
				<span class="radio">
					<input data-message="{l s='Informe' mod='ndk_advanced_custom_fields'} {$field.name|escape:'htmlall'}"  id="radio_{$field.id_ndk_customization_field|escape:'htmlall'}_{$value.id|escape:'htmlall'}" type="radio" name="ndkcsfield[{$field.id_ndk_customization_field|escape:'htmlall'}]" class=" {if $field.required == 1} required_field{/if} ndk-radio not_uniform{if $field.is_visual == 1}visual-effect {/if} {if $field.required == 1} required_field{/if}" data-group="{$field.id_ndk_customization_field|intval}" data-src="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/thumbs/{$value.id|intval}-thickbox_default.jpg" data-zindex="{$field.zindex|escape:'htmlall'}" 
					...
					data-hide-field="{if $value.influences_restrictions|strpos:"all" !== false}1{else}0{/if}" data-id-value="{$value.id|intval}" 
					 value="{$value.value|escape:'htmlall'}" data-price="{if $valuePrice > 0}{$valuePrice|escape:'htmlall'}{else}{$fieldPrice|escape:'htmlall'}{/if}" data-id="{$field.target|escape:'htmlall'}" data-view="{$field.target_child|escape:'htmlall'}"/>
					<label for="radio_{$field.id_ndk_customization_field|escape:'htmlall'}_{$value.id|escape:'htmlall'}">{$value.value|escape:'htmlall'}
