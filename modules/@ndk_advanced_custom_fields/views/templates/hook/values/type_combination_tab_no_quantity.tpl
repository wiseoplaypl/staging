			...
			{foreach from=$field.values item=value}
				...
				<li class="clearfix accessory-ndk filterTag tagged {foreach from=$tags item=tag}{$tag|replace:' ':'-'} {/foreach}" data-tags="{foreach from=$tags item=tag}{$tag}|{/foreach}"  data-group="{$field.id_ndk_customization_field|intval}"  data-zindex="{$field.zindex|escape:'htmlall'}" 
				  ...
				  <div class="col-md-4 accessory-img-block">
					  <img class="img-responsive" src="{$link->getImageLink($product.link_rewrite, $product.cover_image_id, 'home_default')|escape:'html'}"/>
				  </div>
								 ...
								 <div class="combColumn">
									 <span class="ndkcf_col_title" title="{$col.attribute_name}">
										 {if $col.is_color_group == 1}
											 <span class="ndkcf_col_action color_square color-ndk {if $field.is_visual == 1}visual-effect {/if}" 
											 qtty_total_attr="0" 
											 data-dragdrop="{$field.draggable|intval}" 
											 data-resizeable="{$field.resizeable|intval}" 
											 data-rotateable="{$field.rotateable|intval}"
											 ...
											 style="background: {if $img_color_exists}url('{$theme_col_dir}{$col.id_attribute|intval}.jpg'){else}{$col.attribute_color|escape:'htmlall'}{/if};">{$col.attribute_name}<span class="color_counter" >0</span></span>
										 {else}
											 {$col.attribute_name}
										 {/if}
									 </span>
									 ...
									 {if StockAvailable::getQuantityAvailableByProduct($value.id_product_value, $id_combination) > 0 || Product::isAvailableWhenOutOfStock($product.out_of_stock)}
										 <div class="combRow accessory-ndk-no-quantity clearfix" data-group="{$field.id_ndk_customization_field|intval}" >
										 ...
										 {else}
											 <div class="combRow accessory-ndk-no-quantity clearfix combRow_oos">
												 {if isset($row.attribute_name)}
														 {$row.attribute_name}
													 {else}
														 {l s='Choose' mod='ndk_advanced_custom_fields'}
