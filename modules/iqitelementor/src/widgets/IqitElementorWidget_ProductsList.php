...
class IqitElementorWidget_ProductsList
{
    ...
    public function getForm()
    {
        ...
        return [
            ...
            'products_ids' => [
                ...
                'placeholder' => IqitElementorWpHelper::__( 'Product name or ref', 'elementor' ),
    ...
    public function parseOptions($optionsSource, $preview = false)
    {
        ...
        if ($optionsSource['view'] == 'grid' || $optionsSource['view'] == 'grid_s'){
            ...
            $cls_fix_classes[] = 'desktop-swiper-cls-fix-' . IqitElementorWpHelper::absint( $optionsSource['slides_to_show'] ) . ' swiper-cls-row-fix-' . IqitElementorWpHelper::absint( $optionsSource['items_per_column'] );
            $cls_fix_classes[] = 'tablet-swiper-cls-fix-' . IqitElementorWpHelper::absint( $optionsSource['slides_to_show_tablet'] );
            $cls_fix_classes[] = 'mobile-swiper-cls-fix-' . IqitElementorWpHelper::absint( $optionsSource['slides_to_show_mobile']);

            $return['cls_fix_classes'] = implode( ' ', $cls_fix_classes);
        }
    ...
    public function calculateGrid($nb):int {
        $nb = (int) $nb;
        if ($nb == 0) {
            $nb = 1;
        }
