    ...
    protected function __construct(SymfonyRequest $request = null)
    {
        ...
        if (Configuration::get('purls_suppliers') == 1) {
            ...
            if (count($languages_foreach) > 1) {
                ...
                $this->default_routes['supplier_rule_'.Context::getContext()->language->id] = array(
                    'controller' => 'supplier',
                    'rule' => Configuration::get('PURLS_ROUTE_supplier_rule'),
                    'keywords' => array(
                        'id' => array('regexp' => '[0-9]+'),
                        'rewrite' => array(
                            'regexp' => '[_a-zA-Z0-9-\pL]*',
        ...
        if (Configuration::get('purls_manufacturers') == 1) {
            ...
            if (count($languages_foreach) > 1) {
                ...
                $this->default_routes['manufacturer_rule_'.Context::getContext()->language->id] = array(
                    'controller' => 'manufacturer',
                    'rule' => Configuration::get('PURLS_ROUTE_manufacturer_rule'),
                    'keywords' => array(
                        'id' => array('regexp' => '[0-9]+'),
                        'rewrite' => array(
                            'regexp' => '[_a-zA-Z0-9-\pL]*',
        ...
        if (Configuration::get('purls_cms') == 1) {
            ...
            if (count($languages_foreach) > 1) {
                ...
                $this->default_routes['cms_rule_'.Context::getContext()->language->id] = array(
                    'controller' => 'cms',
                    'rule' => Configuration::get('PURLS_ROUTE_cms_rule'),
                    'keywords' => array(
                        'id' => array('regexp' => '[0-9]+'),
                        'rewrite' => array(
                            'regexp' => '[_a-zA-Z0-9-\pL]*',
                ...
                $this->default_routes['cms_category_rule_'.Context::getContext()->language->id] = array(
                    'controller' => 'cms',
                    'rule' => Configuration::get('PURLS_ROUTE_cms_category_rule'),
                    'keywords' => array(
                        'id' => array('regexp' => '[0-9]+'),
                        'rewrite' => array(
                            'regexp' => '[_a-zA-Z0-9-\pL]*',
