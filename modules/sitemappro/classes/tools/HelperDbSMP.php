...
class HelperDbSMP
{
    ...
    public function createTable($table, $definition_fields)
    {
        $sql = array();
        foreach ($definition_fields as $field_name => $field) {
            $sql[] = $this->definitionFieldToSQL($field_name, $field);
        }
        $this->execute($this->getTemplateTableSql($table), $sql);
    }

    public function upgradeTable($table, $definition_fields = array())
    {
        $list_fields = $this->getFieldsFromDatabase($table);
        $this->dropNotExistsColumns($table, $list_fields, $definition_fields);
        $this->upgradeColumnsTable($table, $definition_fields, $list_fields);
    }
    ...
    public function truncateTable($table, $where = '')
    {
