...
(function () {
    ...
    function TabContainer(tab)
    {
        ...
        this.tab = $(tab);
        this.init = function () {
            _this.tab.find('> .js-tabs > ul.js-nav-tabs > li').click(function () {
                _this.tab.find('> .js-tabs > .js-nav-tabs > li').removeClass('active');
                $(this).addClass('active');
                _this.tab.find('> .tabs_content_wrap > .tabs_content > div').hide();

                if ($(this).data('tab') != 'all_tabs') {
                    _this.tab.find('[id="'+$(this).data('tab')+'"]').show();
                } else {
                    _this.tab.find('> .tabs_content_wrap > .tabs_content > div').show();
                }
            });
            _this.tab.find('> .tabs_content_wrap > .tabs_content > div').hide();
            //_this.tab.find('> .tabs_content > div:first').show();
            _this.tab.find('> .js-tabs > ul > li:first').trigger('click');

        };
        this.set = function(id)
        {
            _this.tab.find('.js-nav-tabs > li').removeClass('active');
