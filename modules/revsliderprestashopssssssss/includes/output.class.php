...
class RevSliderOutput {
	 ...
	 */
	public static function putSlider($sliderID,$putIn="", $gal_ids = array(), $settings = array(), $order = array(),$lang = 'all'){
		...
		$output->putSliderBase($sliderID, $gal_ids, false, $settings, $order,$lang);
	...
	private function filterOneSlide($slides){
            $oneSlideID = null;
                    if(isset($this->oneSlideData['slideid'])){
		...
		if($this->previewMode == true && $this->oneSlideMode == true){
			...
			if(!empty($gal_ids) && $gal_ids[0]){ //add slides from the images
					...
					$isSlidesFromPosts = $this->slider->isSlidesFromPosts();
                                        var_dump($isSlidesFromPosts);die();
					$gi = 0;
		...
		foreach($slides as $slide){
			...
			if($isThumbsActive == true || $is_special_nav == true){
				...
				if($thumb_do == 'slider'){ //use the slider settings for width / height
					...
					$th_width = intval($this->slider->getParam('previewimage_width', $this->slider->getParam('thumb_width', 150)));
					$th_height = intval($this->slider->getParam('previewimage_height', $this->slider->getParam('thumb_height', 100)));
					
					//$th_width = intval($this->slider->getParam('thumb_width', 100));
					//$th_height = intval($this->slider->getParam('thumb_height', 50));
					if($th_width == 0) $th_width = 150;
					if($th_height == 0) $th_height = 100;
...
<?php	
 ...
 <?php do_action('communicate_revapi'); ?>
		<?php
		if($js_to_footer && $this->previewMode == false && $markup_export == false){
...
//		}
	 ...
	 * @param $data - mixed, can be ID ot Alias.
	 */
	public function putSliderBase($sliderID, $gal_ids = array(), $markup_export = false, $settings = array(), $order = array(),$lang='all'){
		...
		try{
			...
			$slider_type = $this->slider->getParam("slider-type");
                        // var_dump($slider_type);die();
			// set wrapper height
			$wrapperHeigh = 0;
                       // var_dump($this->slider->getParam("height"));die();
			$wrapperHeigh += (int)$this->slider->getParam("height");
