...
$_SESSION['SDSRF']["verify"] = "RESPONSIVEfilemanager";
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_GET['lang'])) {
        ...
        if (array_key_exists($lang, $languages)) {
            $_SESSION['SDSRF']['language'] = $lang;
        }
    } elseif (isset($_SESSION['SDSRF']['language']) && $_SESSION['SDSRF']['language'])
        $lang = strip_tags($_SESSION['SDSRF']['language']);
    if (array_key_exists($lang, $languages)) {
        $_SESSION['SDSRF']['language'] = $lang;
    }
}
...
if (isset($_GET['fldr']) && !empty($_GET['fldr']) && strpos($_GET['fldr'], '../') === FALSE && strpos($_GET['fldr'], './') === FALSE && strpos($_GET['fldr'], '..\\') === FALSE && strpos($_GET['fldr'], '.\\') === FALSE) {
    $subdir = rawurldecode(trim(strip_tags($_GET['fldr']), "/") . "/");
    $_SESSION['SDSRF']["filter"] = '';
} else {
    $subdir = '';
}
...
if (!isset($_SESSION['SDSRF']["subfolder"])) {
    $_SESSION['SDSRF']["subfolder"] = '';
}
$rfm_subfolder = '';

if (!empty($_SESSION['SDSRF']["subfolder"]) && strpos($_SESSION['SDSRF']["subfolder"], '../') === FALSE && strpos($_SESSION['SDSRF']["subfolder"], '..\\') === FALSE && strpos($_SESSION['SDSRF']["subfolder"], './') === FALSE && strpos($_SESSION['SDSRF']["subfolder"], "/") !== 0 && strpos($_SESSION['SDSRF']["subfolder"], '.') === FALSE) {
    $rfm_subfolder = $_SESSION['SDSRF']['subfolder'];
}
...
<html xmlns="https://www.w3.org/1999/xhtml">
    <head>
        ...
        <link rel="shortcut icon" href="<?php echo $assets_dir; ?>img/ico/favicon.ico">
        <link href="<?php echo $assets_dir; ?>css/font-awesome.min.css" rel="stylesheet" type="text/css" />
        <link href="<?php echo $assets_dir; ?>css/style.css" rel="stylesheet" type="text/css" />
        <link href="<?php echo $assets_dir; ?>js/jPlayer/skin/blue.monday/jplayer.blue.monday.css" rel="stylesheet" type="text/css">
<!--[if lt IE 8]><style>
.img-container span, .img-container-mini span {
        ...
        <script src="<?php echo $assets_dir; ?>js/plugins.js"></script>
        <script src="<?php echo $assets_dir; ?>js/jPlayer/jquery.jplayer/jquery.jplayer.js"></script>
        <script src="<?php echo $assets_dir; ?>js/modernizr.custom.js"></script>
        <?php
        if ($aviary_active) {
            if (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) {
                ?>
                <script src="https://dme0ih8comzn4.cloudfront.net/imaging/v2/editor.js"></script>
        <?php } elseif (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https' || !empty($_SERVER['HTTP_X_FORWARDED_SSL']) && $_SERVER['HTTP_X_FORWARDED_SSL'] == 'on') { ?>
                <script src="https://dme0ih8comzn4.cloudfront.net/imaging/v2/editor.js"></script>
            <?php } else { ?>
...
?>
            ...
            var ps_token = "<?php echo Tools::getAdminTokenLite('AdminRevolutionsliderFmanager'); ?>";
        </script>
        <script src="<?php echo $assets_dir; ?>js/include.js"></script>
    </head>
    <body>

        <!--        
                <input type="text" id="sds_send" value="123">
        ...
        <input type="hidden" id="cur_dir_show_url" value="<?php echo $cur_dir_show_url; ?>" />
        ...
        <input type="hidden" id="clipboard" value="<?php echo ((isset($_SESSION['SDSRF']['clipboard']['path']) && trim($_SESSION['SDSRF']['clipboard']['path']) != null) ? 1 : 0); ?>" />
        ...
        <?php if ($upload_files) { ?>
            ...
            <div class="uploader">
                <div class="pull-right">
                    <button id="sds_return" class="btn close-uploader"><i class="fa fa-close"></i></button>
                </div>
                <div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab1">
                            <form action="<?php echo $dialog_php; ?>" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
...
// Should lazy loading be enabled
                                ...
                                <div class="row-fluid">
                                    <div class="span6">
                                        ...
                                        <?php if ($create_folders) { ?>
                                            <button class="tip btn new-folder" title="<?php echo trans('New_Folder') ?>"><i class="add-folder-png"></i><?php echo trans('New_Folder') ?></button>
                                        <?php } ?>
            ...
            <div class="row-fluid">
                <?php 
                $link = $dialog_php_reload . $get_params;
                ?>
                <ul class="breadcrumb">
                    ...
                    <?php } ?>
                    <li class="pull-right"><a id="refresh" class="btn-small" href="<?php echo $dialog_php_reload; ?><?php echo $get_params . $subdir . "&" . uniqid() ?>"><i class="fa fa-refresh"></i></a></li>
            ...
            <!-- breadcrumb div end -->
            <div class="row-fluid ff-container" style="display: none;">
                <div class="span12">
                    ...
                    <?php } else { ?>
                        ...
                        <ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
                            ...
                            foreach ($files as $file_array) {
                                ...
                                ?>" 
                                    <?php if (($filter != '' && stripos($file, $filter) === false)) echo ' style="display:none;"'; ?>>
                                    ...
                                    ?>">
                                        ...
                                        <?php } ?>
                                        <a class="folder-link" href="<?php echo $dialog_php_reload; //$dialog_php;  ?>?<?php echo $get_params . rawurlencode($src) . "&" . ($callback ? 'callback=' . $callback . "&" : '') . uniqid() ?>">
                                            <div class="img-precontainer">
                                                <div class="img-container directory"><span></span>
                                                    <img class="directory-img"  src="<?php echo $assets_dir_img; ?><?php echo $icon_theme; ?>/folder<?php
                                                    if ($file == "..") {
                                                        echo "_back";
                                                    }
                                                    ?>.png" />
                                        ...
                                        <?php } else { ?>
                                            ...
                                            <div class="box">
                                                ...
                                                ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="<?php echo $dialog_php; ?>?<?php echo $get_params . rawurlencode($src) . "&" . uniqid() ?>"><?php echo $file; ?></a></h4>
                                            </div>
                            ...
                            foreach ($files as $nu => $file_array) {
                                ...
                                if ((!($_GET['type'] == 1 && !$is_img) && !(($_GET['type'] == 3 && !$is_video) && ($_GET['type'] == 3 && !$is_audio))) && $class_ext > 0) {
                                    ...
                                    <li class="ff-item-type-<?php echo $class_ext; ?> file"  data-name="<?php echo $file; ?>" <?php if (($filter != '' && stripos($file, $filter) === false)) echo ' style="display:none;"'; ?>><?php
                                        ...
                                        ?>">
                                            <?php
                                                        $source_file_url = $config['source_path_orginal'] . $subdir . $file;
                                                        $sds_ex = explode('.', $file);
                                                        $sds_file = $config['assets_dir_icon'] . $sds_ex[1] . '.jpg'; 
                                            ?>
                                            <a href="javascript:void('')" data-source="<?php echo $source_file_url; ?>" class="link" data-file="<?php echo $file; ?>" data-function="<?php echo $apply; ?>">
                                                <div class="img-precontainer">
                                                    ...
                                                    <div class="img-container">
                                                        ...
                                                        <?php
                                                        $sds_img = $config['thumbs_path_orginal'] . $subdir . $file;
                                                        $sds_ex = explode('.', $file);
                                                        $sds_file = $config['assets_dir_icon'] . $sds_ex[1] . '.jpg';
                
                                                        $thumb_file_dir = get_upload_dir_from_url($sds_img);
                                                        
                                                        if(!file_exists($thumb_file_dir)){
                                                            $sds_img = $source_file_url;
                                                        }
                                                        
                                                        if ($is_img ) { ?>
                                                            <div style="background-image: url(<?php echo $sds_img; ?>);"></div>
                                                        <?php } else { ?>
                                                            <div style="background-image: url(<?php echo $sds_file; ?>);"></div>
                                                        <?php }
                                                        ?>
                                                    </div>
                                                </div>
                                                <div class="img-precontainer-mini <?php if ($is_img) echo 'original-thumb' ?>">
                                                    <div class="filetype <?php echo $file_array['extension'] ?> <?php if (in_array($file_array['extension'], $editable_text_file_exts)) echo 'edit-text-file-allowed' ?> <?php
                                                        if (!$is_icon_thumb) {echo "hide";}
                                                    ?>">
                                                        <?php echo $file_array['extension'] ?>                  
                                                    </div>
                                                    <div class="img-container-mini">
                                                        ...
                                                        if ($is_img) {
                                                            ...
                                                            $sds_img = $config['source_path_orginal'] . $subdir . $file;
                                                        }
                                                        
                                                        ?>

                                                        <?php if ($mini_src != "") { ?>
                                                                        <!--<img class="<?php echo $show_original_mini ? "original" : "" ?><?php echo $is_icon_thumb_mini ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : "" ?>" <?php echo $lazy_loading_enabled ? "data-original" : "src" ?>="<?php $mini_src; ?>">-->
                                                            <img class="<?php echo $show_original_mini ? "original" : "" ?><?php echo $is_icon_thumb_mini ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : "" ?>" src="<?php echo $sds_img; ?>">
                                                        <?php } ?>
                                                    </div>
                                                </div>
                                                <?php if ($is_icon_thumb) { ?>
            ...
            <?php echo $filename; ?></h4>
                                            ...
                                            <figcaption>
                                                <form action="<?php echo $sds_force_download_php; ?>" method="post" class="download-form" id="form<?php echo $nu; ?>">
                                                    ...
                                                    <?php if ($is_img && $src_thumb != "" && $file_array['extension'] != "tiff" && $file_array['extension'] != "tif") { ?>
                                                        <a class="tip-right preview" title="<?php echo trans('Preview') ?>" data-url="<?php echo $config['current_path_host'] . $subdir . $file; ?>" data-toggle="lightbox" href="#previewLightbox"><i class=" icon-eye-open"></i></a>
                                                    <?php } elseif (($is_video || $is_audio) && in_array($file_array['extension'], $jplayer_ext)) { ?>
                                                        ...
                                                        ?>"
                                                           title="<?php echo trans('Preview') ?>" data-url="<?php echo $sds_ajax_calls_php; ?>&action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
            ...
            <?php } elseif (in_array($file_array['extension'], array('dwg', 'dxf', 'hpgl', 'plt', 'spl', 'step', 'stp', 'iges', 'igs', 'sat', 'cgm', 'svg'))) { ?>
                                                        <a class="tip-right file-preview-btn" title="<?php echo trans('Preview') ?>" data-url="<?php echo $sds_ajax_calls_php; ?>&action=cad_preview&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
                                                           href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
            <?php } elseif ($preview_text_files && in_array($file_array['extension'], $previewable_text_file_exts)) { ?>
                                                        <a class="tip-right file-preview-btn" title="<?php echo trans('Preview') ?>" data-url="<?php echo $sds_ajax_calls_php; ?>&action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
                                                           href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
            <?php } elseif ($googledoc_enabled && in_array($file_array['extension'], $googledoc_file_exts)) { ?>
                                                        <a class="tip-right file-preview-btn" title="<?php echo trans('Preview') ?>" data-url="<?php echo $sds_ajax_calls_php; ?>&action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
            ...
            <?php } elseif ($viewerjs_enabled && in_array($file_array['extension'], $viewerjs_file_exts)) { ?>
                                                        <a class="tip-right file-preview-btn" title="<?php echo trans('Preview') ?>" data-url="<?php echo $sds_ajax_calls_php; ?>&action=get_file&sub_action=preview&preview_mode=viewerjs&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
                                                           href="docs.google.com;" ><i class=" icon-eye-open"></i></a>
...
<?php endforeach; ?>
        ...
        <div id="loading_container" style="display:none;">
            ...
            <img id="loading_animation" src="<?php echo $assets_dir_img; ?>storing_animation.gif" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%"/>
        </div>
        <!-- loading div end -->
    ...
    function displayResult(element)
    {
       document.getElementById(element).value = 'hi';
        ...
        <script>
            ...
            setTimeout(function() { autoLoad(); }, 1000);
