...
if (!function_exists('trans')) {
    // language
    if (!isset($_SESSION['SDSRF']['language']) || file_exists('lang/' . basename($_SESSION['SDSRF']['language']) . '.php') === false || !is_readable('lang/' . basename($_SESSION['SDSRF']['language']) . '.php')
    ) {
...
//            $languages = include 'lang/languages.php';
//        } else {
            $languages = RS_PLUGIN_PATH.'/include/filemanager/lang/languages.php';
       // }

        if (array_key_exists($_SESSION['SDSRF']['language'], $languages)) {
            $lang = $_SESSION['SDSRF']['language'];
        } else {
            response(trans('Lang_Not_Found') . AddErrorLocation())->send();
            exit;
        }
    }
//    if (file_exists(JPATH_COMPONENT . '/lang/' . $lang . '.php')) {
//        $lang_vars = include JPATH_COMPONENT . '/lang/' . $lang . '.php';
//    } else {
        $lang_vars = include  RS_PLUGIN_PATH.'/include/filemanager/lang/' . $lang . '.php';
...
function folder_info($path, $count_hidden = true) {
    ...
    $hidden_folders = array(); //edit by shahin
    $hidden_files = array();
...
function image_check_memory_usage($img, $max_breedte, $max_hoogte) {
    if (file_exists($img)) {
        ...
        if($last == 'g')
            ...
            $memory_limit = $v_memory_limit_int*1073741824;
        if($last == 'm')
            //$v_memory_limit_int = $v_memory_limit_int*1024*1024;
            $memory_limit= $v_memory_limit_int*1048576;
        if($last == 'k')
            $memory_limit = $v_memory_limit_int*1024;
...
function get_upload_dir_from_url($pathImage){
   $dir =  RS_PLUGIN_URL.'uploads/';
    $url = RS_PLUGIN_PATH.'/uploads/';
    $urlImage = str_replace($url,$dir, $pathImage);
    return($urlImage); 
}
/**
