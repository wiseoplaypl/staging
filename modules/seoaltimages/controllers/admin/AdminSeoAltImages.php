...
class AdminSeoAltImagesController extends ModuleAdminController {
	
	public function __construct()
    {
        $this->className = 'SeoAltImagesModel';
        $this->table = 'seoaltimages';
        $this->deleted = false;
        $this->identifier = 'id_seoaltimages';
        $this->lang = false;
        $this->bootstrap = true;
		...
		$this->fields_list = array(
            'id_seoaltimages' => array('title' => $this->l('ID'), 'align' => 'center', 'class' => 'fixed-width-xs'),
            'title' => array('title' => $this->l('Title'), 'align' => 'center'),
			'rule' => array('title' => $this->l('Rule'), 'align' => 'center'),
            'status' => array('title' => $this->l('Enabled'), 'align' => 'center', 'type' => 'bool', 'callback' => 'getStatus')
        );
    ...
    {
		require_once($this->module->getLocalPath().'classes/SeoAltImagesModel.php');
        parent::init();
    }
	...
	public function renderForm()
    {
		...
		$id = (int)Tools::getValue('id_seoaltimages');
		$id_lang = (int)$this->context->language->id;
		$this->display = ($id > 0) ? '&updateseoaltimages' : '&addseoaltimages';
		$shops = Shop::getShops(true, null, false);
		$languages = Language::getLanguages();
		$categories = Category::getSimpleCategories($id_lang);
		$url = $this->context->link->getAdminLink('AdminSeoAltImages', true);
		$collection = ($id > 0) ? new SeoAltImagesModel($id) : array();
		if ($id > 0) {
			$collection->products = $collection->getRelativeProducts($id);
			$collection->shops = $collection->getRelativeShops($id);
			$collection->categories = $collection->getRelativeCategories($id);
			$collection->languages = $collection->getRelativeLangs($id);
		}
		//dump($collection);exit;
		$this->context->smarty->assign(array(
			'ps_version' => (int)$ps_version,
            'action' => self::$currentIndex.'&token='.$this->token.$this->display,
			'id' => $id,
			'shops' => $shops,
			'languages' => $languages,
	...
	public function postProcess()
    {
		...
		$trigger = Tools::getValue('action');
		if (Tools::isSubmit('submitSeoAlt')) {
			$seo_alt_value = Tools::getValue('seo_alt_value');
			$seo_alt_state = Tools::getValue('control_seo_alt');
			$seo_alt_title = Tools::getValue('seo_alt_title');
			$type = (int)Tools::getValue('seo_alt_type');
			$shops = Tools::getValue('shops');
			$languages = Tools::getValue('langs');
			$categories = Tools::getValue('category');
			$products = Tools::getValue('related_products');
			$id = (int)Tools::getValue('id_seoaltimages');
			//dump($id);exit;
			if (empty($seo_alt_value)) {
                $this->errors[] = $this->l('Please fill Alt rule.');
			}
			elseif (empty($shops)) {
                $this->errors[] = $this->l('Please select a shop or multiple shops.');
			}
			elseif ($type == 1 && empty($categories)) {
                $this->errors[] = $this->l('Please select a category or multiple categories.');
			}
			elseif ($type == 2 && empty($products)) {
                $this->errors[] = $this->l('Please select a product or multiple products.');
			}
			else {
				$seo->rule = $seo_alt_value;
				$seo->status = $seo_alt_state;
				$seo->title = $seo_alt_title;
				$seo->type = $type;
				if ($id > 0) {
					$seo_cls = new SeoAltImagesModel($id);
					$seo_cls->rule = $seo_alt_value;
					$seo_cls->status = $seo_alt_state;
					$seo_cls->title = $seo_alt_title;
					$seo_cls->type = $type;
					$seo_cls->update();
					Db::getInstance()->delete(
						'seoaltimages_categories',
						'`id_seoaltimages` = '.(int)$id
					);
					Db::getInstance()->delete(
						'seoaltimages_products',
						'`id_seoaltimages` = '.(int)$id
					);
					Db::getInstance()->delete(
						'seoaltimages_shop',
				...
				$seo->saveShops($id, $shops);
				$seo->saveLangs($id, $languages);
				if ($type == 1) {
					$seo->saveCategories($id, $categories);
				}
				elseif ($type == 2) {
					$seo->saveProducts($id, $products);
				}
				$action = $action.'&conf=3';
				Tools::redirectAdmin($action);
			}
		}
		if ($trigger == 'getSearchProducts') {
