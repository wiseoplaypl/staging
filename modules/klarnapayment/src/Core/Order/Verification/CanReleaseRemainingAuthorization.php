...
class CanReleaseRemainingAuthorization implements OrderActionVerificationInterface
{
    ...
    public function verify(Order $order): bool
    {
        ...
        if (!$this->hasRemainingAuthorization($order)) {
            throw CouldNotVerifyOrderAction::noRemainingAuthorization();
        }
    ...
    private function hasRemainingAuthorization(Order $order): bool
    {
        $orderAmount = $order->getOrderAmount();
        $capturedAmount = $order->getCapturedAmount();

        return $orderAmount > $capturedAmount && $capturedAmount > 0;
    }
}
