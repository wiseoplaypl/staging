...
class CustomerProvider
{
    ...
    public function get(int $customerId): Customer
    {
        ...
        $apiCustomer->setType($this->getCustomerType($customer));
    ...
    private function getCustomerType(\Customer $customer): string
    {
        $cart = $this->context->getCart();

        if (
            !$this->configuration->getByEnvironmentAsBoolean(Config::KLARNA_PAYMENT_B2B_ENABLE)
            || !$cart
        ) {
