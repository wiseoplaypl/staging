...
class ValidateOrderAction
{
    ...
    public function run(ValidateOrderData $data): ?\Order
    {
        ...
        try {
            ...
            if (!$this->globalShopContext->isSharedCustomers()) {
                $criteria['id_shop'] = $this->globalShopContext->getShopId();
            }

            /** @var \Customer|null $customer */
            $customer = $this->customerRepository->findOneBy($criteria);
        } catch (\Exception $exception) {
            throw CouldNotValidateOrder::unknownError($exception);
        }

        if (!$customer) {
            throw CouldNotValidateOrder::failedToFindCustomer(
                $data->getCustomerId(),
