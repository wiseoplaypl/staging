...
use KlarnaPayment\Module\Core\Order\Verification\OrderStatusMappingValidator;
...
class AdminKlarnaPaymentSettingsController extends ModuleAdminController
{
    ...
    private const KLARNA_PAYMENT_B2B_ENABLE = 'KLARNA_PAYMENT_B2B_ENABLE';
    ...
    private const KLARNA_PAYMENT_SHARE_SHOPPING_DATA = 'KLARNA_PAYMENT_SHARE_SHOPPING_DATA';
    ...
    public function initContent(): void
    {
        ...
        $this->content .= $this->displayCloudSyncSettings();
    ...
    public function postProcess(): bool
    {
        ...
        if ($tools->isSubmit('submit_additional_settings')) {
            ...
            $isMerchantDirect = $merchantTypeService->isDirect();
            ...
            if ($isMerchantDirect) {
                ...
                $configuration->setByEnvironment(
                    Config::KLARNA_PAYMENT_B2B_ENABLE,
                    $tools->getValueAsInteger(self::KLARNA_PAYMENT_B2B_ENABLE . '_')
                );
            }

            if (!$isMerchantDirect) {
                $shareShoppingData = (int) $request->get(self::KLARNA_PAYMENT_SHARE_SHOPPING_DATA . '_');
                $configuration->setByEnvironment(
                    Config::KLARNA_PAYMENT_SHARE_SHOPPING_DATA,
                    $shareShoppingData
                );
            }
    ...
    public function displayAdditionalSettings(): string
    {
        ...
        if ($isMerchantDirect) {
            ...
            $this->fields_value[self::KLARNA_PAYMENT_B2B_ENABLE . '_'] =
                $configuration->getByEnvironmentAsBoolean(Config::KLARNA_PAYMENT_B2B_ENABLE);
            ...
            $this->context->smarty->assign([
                'klarnapayment' => [
                    'text' => $this->module->l('Enable B2B mode to send customers company details to Klarna.', self::FILE_NAME),
                ],
            ]);

            $helpIconB2B = $this->context->smarty->fetch(
                $this->module->getLocalPath() .
                '/views/templates/admin/settings/help-icon.tpl'
            );
            ...
            $inputs = [
                ...
                [
                    ...
                    'class' => 'klarna-admin-checkbox',
                    'type' => 'checkbox',
                    'name' => self::KLARNA_PAYMENT_B2B_ENABLE,
                    'values' => [
                        'query' => [
                            [
                                'id' => '',
                                'name' => $this->module->l('Enable B2B mode', self::FILE_NAME) . ' ' . $helpIconB2B,
                                'val' => '1',
                            ],
                        ],
                        'id' => 'id',
        ...
        if (!$isMerchantDirect) {
            $this->fields_value[self::KLARNA_PAYMENT_SHARE_SHOPPING_DATA . '_'] =
                $configuration->getByEnvironment(Config::KLARNA_PAYMENT_SHARE_SHOPPING_DATA);

            $inputs[] = [
                'form_group_class' => 'klarna-admin-checkbox-form-group klarna-share-data-title',
                'label' => 'Shopping data',
                'class' => 'klarna-admin-checkbox',
                'type' => 'checkbox',
                'name' => self::KLARNA_PAYMENT_SHARE_SHOPPING_DATA,
                'desc' => $this->module->l('Sharing your customer\'s shopping data with Klarna improves the customer\'s experience and makes disputes easier to handle.', self::FILE_NAME),
                'values' => [
                    'query' => [
                        [
                            'id' => '',
                            'name' => $this->module->l('Share shopping data with Klarna', self::FILE_NAME),
                            'val' => '1',
                        ],
                    ],
                    'id' => 'id',
    ...
    private function displayAutoCaptureSettings(): string
    {
        ...
        $selectedStatuses = $this->getOrderStatusesFromInput($moduleOrderStatuses);

        /** @var OrderStatusMappingValidator $orderStatusMappingValidator */
        $orderStatusMappingValidator = $this->module->getService(OrderStatusMappingValidator::class);

        if ($orderStatusMappingValidator->hasDuplicateMappings($selectedStatuses)) {
            $this->addDuplicateOrderStatusWarningBox($input);
        }
    ...
    private function getOrderStatusesFromInput(array $moduleOrderStatuses): array
    {
        $selectedStatuses = [];
        foreach (array_keys($moduleOrderStatuses) as $orderStatusKey) {
            $selected = $this->fields_value[$orderStatusKey];
            if ($selected) {
    ...
    private function addDuplicateOrderStatusWarningBox(array &$input): void
    {
        $this->context->smarty->assign('warningBox', [
            'message' => $this->module->l('Duplicate order statuses are not recommended. Please ensure each Klarna status is mapped to a unique PrestaShop order status to avoid unexpected behavior.', self::FILE_NAME),
        ]);
        ...
        $this->fields_form = [
            ...
            'buttons' => [
                [
                    ...
                    'class' => 'btn btn-lg pull-right form-btn',
                    'type' => 'submit',
                    'name' => 'submit_cloud_sync_dependencies',
                ],
            ],
        ];
    ...
    private function displayCloudSyncSettings(): string
    {
        /** @var LoggerInterface $logger */
        $logger = $this->module->getService(LoggerInterface::class);
        ...
        try {
            ...
            if ($mboInstaller->areDependenciesMet()) {
                return $this->displayPsAccounts();
            } else {
                return $this->displayCloudSyncDependencies($mboInstaller);
            }
        } catch (\Exception $e) {
    ...
    {
        /** @var MerchantCredentialsHandler $merchantCredentialsHandler */
        $merchantCredentialsHandler = $this->module->getService(MerchantCredentialsHandler::class);
        ...
        try {
            $merchantCredentialsHandler->handle($apiKey);
        } catch (\Exception $e) {
            $logger->info(
    ...
    private function isMerchantPartner(): bool
    {
        /** @var MerchantTypeService $merchantTypeService */
        $merchantTypeService = $this->module->getService(MerchantTypeService::class);

        $merchantType = $merchantTypeService->getMerchantType();

        return MerchantType::PARTNER === $merchantType;
    }
}
