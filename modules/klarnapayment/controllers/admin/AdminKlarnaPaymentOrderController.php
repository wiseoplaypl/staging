...
use KlarnaPayment\Module\Core\Order\Processor\ReleaseRemainingAuthorizationProcessor;
...
class AdminKlarnaPaymentOrderController extends ModuleAdminController
{
    ...
    public function postProcess(): bool
    {
        ...
        if ($tools->isSubmit('release-remaining-authorization')) {
            try {
                $this->releaseRemainingAuthorization($internalOrderId, $order);
            } catch (CouldNotReleaseRemainingAuthorization $exception) {
                $errors[$internalOrderId] = $this->module->l('Failed to release remaining authorization, check logs for more information.', self::FILE_NAME);

                $cookie->set(Config::KLARNA_PAYMENT_ORDER_MANAGEMENT_COOKIE_ERRORS, json_encode($errors));

                $logger->error('Failed to release remaining authorization.', [
                    'context' => [
                        'id_internal' => $internalOrderId,
                        'id_external' => $externalOrderId,
                    ],
     ...
     * @throws KlarnaPaymentException
     */
    private function releaseRemainingAuthorization(int $internalOrderId, Order $order): void
    {
        /** @var PrestaShopCookie $cookie */
        $cookie = $this->module->getService(PrestaShopCookie::class);

        /** @var ReleaseRemainingAuthorizationProcessor $releaseRemainingAuthorizationProcessor */
        $releaseRemainingAuthorizationProcessor = $this->module->getService(ReleaseRemainingAuthorizationProcessor::class);

        $releaseRemainingAuthorizationProcessor->processAction($order);

        $cookie->set(Config::KLARNA_PAYMENT_ORDER_MANAGEMENT_COOKIE_RELEASED, true);
    }
}
