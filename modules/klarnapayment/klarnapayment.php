...
use KlarnaPayment\Module\Infrastructure\Hook\Action\ActionOrderEditedHook;
...
class KlarnaPayment extends PaymentModule
{
    ...
    public function hookDisplayAdminOrder(array $params): string
    {
        ...
        // Check if order update to Klarna failed
        ...
        if ($klarnaPaymentOrder && $klarnaPaymentOrder->failed_to_update_klarna) {
            $warnings = [
                'message' => $this->l('Failed to update order with Klarna. Please try to update the order manually in the Klarna Merchant Portal'),
            ];
        }
    ...
    public function hookDisplayAdminOrderSide(array $params): string
    {
        ...
        if ($klarnaPaymentOrder && $klarnaPaymentOrder->failed_to_update_klarna) {
            $warnings = [
                'message' => $this->l('Failed to update order with Klarna. Please try to update the order manually or contact support.'),
            ];
        }
    ...
    public function hookActionOrderEdited(array $params): void
    {
        /** @var LoggerInterface $logger */
        $logger = $this->getService(LoggerInterface::class);
        ...
        try {
            ...
            /** @var ActionOrderEditedHook $actionOrderEditedHook */
            $actionOrderEditedHook = $this->getService(ActionOrderEditedHook::class);

            $actionOrderEditedHook->run($order);
        } catch (\Throwable $exception) {
            $logger->error('Failed to update order authorization with Klarna', [
                'context' => [
                    'order_id' => $order->id,
                ],
                'exceptions' => ExceptionUtility::getExceptions($exception),
    ...
    public function hookActionCarrierProcess(array $params): void
    {
        ...
        try {
            ...
            $klarnaExpressCheckoutRepository = $this->getService(KlarnaExpressCheckoutRepositoryInterface::class);

            /** @var \KlarnaExpressCheckout|null $klarnaExpressCheckout */
            $klarnaExpressCheckout = $klarnaExpressCheckoutRepository->findOneBy(['id_cart' => $cart->id]);

            if (
                !Validate::isLoadedObject($klarnaExpressCheckout)
                || !$klarnaExpressCheckout->is_kec
                || !$klarnaExpressCheckout->carrier_reference_id
                || !empty($klarnaExpressCheckout->client_token)
            ) {
                return;
            }

            /** @var CarrierRepositoryInterface $carrierRepository */
            $carrierRepository = $this->getService(CarrierRepositoryInterface::class);

            /** @var Carrier|null $carrier */
            $carrier = $carrierRepository->findOneBy(['id_reference' => $klarnaExpressCheckout->carrier_reference_id]);
            ...
            $updateCartCarrierHandler = $this->getService(UpdateCartCarrierHandler::class);
            ...
            $logger->error('Failed to set KEC carrier in hookActionCarrierProcess', [
                'context' => [
                    'cart_id' => $cart->id,
                ],
                'exceptions' => ExceptionUtility::getExceptions($exception),
    ...
    public function hookDisplayProductPriceBlock(array $params): string
    {
        ...
        $klarnaExpressCheckoutContent = $klarnaExpressCheckoutPresenter->present();
     ...
     * @return bool
     */
    public function addCheckboxCurrencyRestrictionsForModule(array $shopsList = []): bool
    {
        if (false === version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            return true;
        }
