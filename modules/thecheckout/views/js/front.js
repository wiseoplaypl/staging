...
// order-review block: monitor changes in delivery/invoice address and copy them to the review block
const monitorAddressChanges = (addressType) => {
    const targetNode = document.querySelector(`#thecheckout-address-${addressType} .inner-area`);

    if (!targetNode) {
        console.error('monitorAddressChanges - Target node not found');
        return; // Exit if targetNode is not found
    }

    targetNode.addEventListener('input', function(event) {
        if (event.target.matches('input, select')) {
            copyAddressesToOrderReview_debounced();
        }
    });

    // we need to use also change(), as from google-places-autocomplete input did not get triggered
    targetNode.addEventListener('change', function(event) {
        // console.log('!change detected on element', event.target);
        if (event.target.matches('input, select')) {
            copyAddressesToOrderReview_debounced();
        }
    });
...
const copyAddressToOrderReview = (sourceAddressType, reviewBlockAddressType) => {
    var inputs = Array.from(document.querySelectorAll(`#thecheckout-address-${sourceAddressType} .form-fields input, #thecheckout-address-${sourceAddressType} .form-fields select`))
        .filter(input => {
            return input.offsetParent !== null; // Filter out the invisible elements
        });
    var newMarkup = '';
    var firstName = '';
    var lastName = '';
    ...
    inputs.forEach(function(input) {
        var label = input.parentElement.querySelector('.field-label').textContent;
        var value;

        if (input.tagName.toLowerCase() === 'select' && input.options.length > 0 && input.selectedIndex > -1 && input.value !== '') {
            value = input.options[input.selectedIndex].textContent;
        } else {
            value = input.value;
        }

        // merge firstname and lastname into one field, use the label of firstname if it exists, otherwise use the label of lastname
        if (value.trim() !== '') {
            if (input.name === 'firstname') {
                ...
                lastName = value;
                firstNameLabel = (firstNameLabel ? firstNameLabel : label); // fallback solution if firstname is not shown
            } else {
                newMarkup += '<div class="field"><div class="label">' + label + '</div><div class="value">' + value + '</div></div>';
            }
        }
    });
    ...
    if (firstName || lastName) {
        newMarkup = '<div class="field"><div class="label">' + firstNameLabel + '</div><div class="value">' + firstName + ' ' + lastName + '</div></div>' + newMarkup;
    }

    document.querySelector(`.review-container .${reviewBlockAddressType}-address .fields`).innerHTML = newMarkup;
};

let copyAddresses_debounceTimer;

const copyAddressesToOrderReview_debounced = () => {
    clearTimeout(copyAddresses_debounceTimer);
    copyAddresses_debounceTimer = setTimeout(copyAddressesToOrderReview_now, 200);
}

const copyAddressesToOrderReview_now = () => {
    if (tcHasSecondAddress) {
        // If there is a second address, copy delivery address to review delivery block and invoice to invoice block
        copyAddressToOrderReview('delivery', 'delivery');
        copyAddressToOrderReview('invoice', 'invoice');
    } else {
        // If there is no second address, decide based on tcIsInvoiceAddressPrimary
        if (tcIsInvoiceAddressPrimary === '1') {
            // If the invoice address is primary, copy it to both blocks
            copyAddressToOrderReview('invoice', 'invoice');
            copyAddressToOrderReview('invoice', 'delivery');
        } else {
            // If the delivery address is primary, copy it to both blocks
            copyAddressToOrderReview('delivery', 'invoice');
            copyAddressToOrderReview('delivery', 'delivery');
        }
    }
}

let copyCustomerToOrderReview_debounceTimer;
const copyCustomerToOrderReview_debounced = () => {
    clearTimeout(copyCustomerToOrderReview_debounceTimer);
    copyCustomerToOrderReview_debounceTimer = setTimeout(copyCustomerToOrderReview_now, 200);
}

const copyCustomerToOrderReview_now = () => {
    // this shall work even for logged-in customers, fields are hidden, but still present in DOM
    const emailField = document.querySelector('#thecheckout-account input[name="email"]');
    const firstNameField = document.querySelector('#thecheckout-account input[name="firstname"]');
    const lastNameField = document.querySelector('#thecheckout-account input[name="lastname"]');

    const firstName = firstNameField.value.trim();
    const lastName = lastNameField.value.trim();
    const email = emailField.value.trim();

    const reviewFields = document.querySelector('.review-container .customer-section .fields');

    reviewFields.textContent = `
      ${firstName ? firstName + ' ' : ''} 
      ${lastName ? lastName + ' - ' : ''} 
      ${email}
    `.trim();
...
const monitorCustomerChanges = () => {
    const fieldNames = ['email', 'firstname', 'lastname'];

    fieldNames.forEach(fieldName => {
        const field = document.querySelector(`#thecheckout-account input[name="${fieldName}"]`);
        field.addEventListener('input', copyCustomerToOrderReview_debounced);
    });
}

const copyShippingToOrderReview = () => {
    const selectedOption = document.querySelector('.delivery-option-row input[type="radio"]:checked');

    if (selectedOption) {
        const selectedRow = selectedOption.closest('.delivery-option-row');

        const logo = selectedRow.querySelector('.delivery-option-logo').innerHTML;
        const nameAndDelay = selectedRow.querySelector('.name-and-delay').innerHTML;

        const extraContent = selectedRow.nextElementSibling; // this shall be .row.carrier-extra-content

        const reviewContent = `
            <div class="delivery-option-detail">
                <div class="delivery-option-logo">${logo}</div>
                <div class="name-and-delay">${nameAndDelay}</div>
            </div>
            <div class="carrier-extra-content">${extraContent ? extraContent.innerHTML : ''}</div>
        `;

        const reviewContainer = document.querySelector('.review-container .shipping-section .inner .fields');
        reviewContainer.innerHTML = reviewContent;
    }
};

const monitorShippingChanges = () => {
    prestashop.on('thecheckout_updateDeliveryOption', copyShippingToOrderReview);
}

const copyPaymentToOrderReview = () => {
    const selectedOption = document.querySelector('.payment-options .tc-main-title.selected');
    if (selectedOption) {
        const paymentOption = selectedOption.querySelector('.payment-option label').innerHTML;
        const additionalInfoContainer = selectedOption.querySelector('.additional-information');
        const additionalInfo = additionalInfoContainer && window.getComputedStyle(additionalInfoContainer).display !== 'none' ? additionalInfoContainer.innerHTML : '';

        const reviewContent = `
            <div class="payment-option-detail">
                ${paymentOption}
            </div>
            <div class="payment-additional-info">${additionalInfo}</div>
        `;

        const reviewContainer = document.querySelector('.review-container .payment-section .fields');
        reviewContainer.innerHTML = reviewContent;
    }
};

const monitorPaymentChanges = () => {
    prestashop.on('thecheckout_updatePaymentOption', copyPaymentToOrderReview);
}

const initOrderReviewBlock = () => {
    // skip, if #thecheckout-order-review block has .hidden class (= is not visible and it's useless to monitor for changes)
    if (!$('#thecheckout-order-review').length || $('#thecheckout-order-review').hasClass('hidden')) {
        return;
    }
    copyAddressesToOrderReview_debounced();
    copyCustomerToOrderReview_debounced();

    monitorAddressChanges('invoice');
    monitorAddressChanges('delivery');
    monitorCustomerChanges();
    monitorShippingChanges();
    monitorPaymentChanges();

    // tcHasSecondAddress is set in front.tpl and kept up-to-date here in front.js based on user's actions (expanded/collapsed second address)
    // this is necessary to know which source address shall be copied into which destination order-review block
    prestashop.on('thecheckout_changeSecondAddress', function(data) {
        tcHasSecondAddress = !data.isCollapsing;
    })
}
...
$(document).ready(function () {
    ...
    $('body').on('click', '#tc_save_account', function (e) {
        checkEmail('form.account-fields', $(this), function (jsonData) {
            if (!jsonData.hasErrors) {
                ...
                // prestashop.on('thecheckout_checkEmail', function(data) { console.log('email checked!', data.reason); })
                prestashop.emit('thecheckout_checkEmail', {
                    reason: 'tc_save_account',
                });
            }
        });
    });
    ...
    $('body').on('change', '.checkout-block input.-error:not(.smartform-phone-number,.smartform-email), .checkout-block select.-error', function () {
    ...
    $('body').on('click', '[data-link-action=x-forced-email-continue]', function () {
        ...
        if (!tc_helper_validateEmail(enteredEmail)) {
            ...
            checkEmail('.overlay-email', $(this), function (jsonData) {
                if (jsonData.hasErrors) {
                    if ('undefined' !== typeof jsonData.errors && 'undefined' !== typeof jsonData.errors['email']) {
    ...
    $('body').on('change', '[data-link-action=x-i-am-business]', function () {
        ...
        if ($(this).prop('checked')) {
            ...
            $(this).closest('.inner-wrapper').find('.business-fields-container').show();
        } else {
            ...
            $(this).closest('.inner-wrapper').find('.business-fields-container').hide();
        }
    ...
    $('body').on('input', '.checkout-block .text input, .checkout-block .tel input:not(.smartform-phone-number)', function () {
        $self = $(this);
    ...
    $('body').on('click', '.summary-customization > .expand-customization', function () {
        $(this).closest('.summary-customization').toggleClass('details-shown');
        return false;
    });
    ...
    // prevent shipping form submit on Enter keypress in pickup point search field
    $('#thecheckout-shipping').on('keypress', 'input[type=text]', function(e) {
        if (e.keyCode === 13) {
            e.preventDefault();
        }
    });
...
function handleWindowResize(win) {
    ...
    if (winWidth <= tcMobileViewThreshold && !tcIsMobileView) {
        ...
        // prestashop.on('thecheckout_windowResize', function(data) { console.log('windowResize container changed!', data); })
        prestashop.emit('thecheckout_windowResize', {
...
function openLoginForm() {
    $('.checkout-block #login-form [name=email]').val($('#thecheckout-account [name=email]').val());
    $('.offer-login').addClass('expanded');
    $('.checkout-block #login-form').fadeIn();
    scrollToElement($('.checkout-block #login-form').closest('.checkout-block'));
    return false;
}
...
function printContextNotices(blockSel, notices, isError = false) {
    const errorClass = isError ? 'error-notice' : '';
    $.each(notices, function (index, value) {
        $(blockSel + ' [name=' + index + ']').addClass('-notice');
        $(blockSel + ' [name=' + index + ']').after('<div class="field notice-msg ' + errorClass + '">' + value + '</div>');
    });
}

function findStepByBlockName(blockSel) {
    var foundStep = 0;
    step_error_loop:
    if (typeof tc_steps !== 'undefined') {
        ...
        for (const blockNameFull of selectorBlocks) {
            ...
            for (let i = tc_steps.length - 1; i >= 0; i--) {
                let tc_step = tc_steps[i];
                if (tc_step.blocks.includes(blockName)) {
                    foundStep = tc_step.step;
...
function printContextErrors(blockSel, errors, triggerElement, dontShowGlobal) {
    ...
    var switchToStep = findStepByBlockName(blockSel); // 0 = no switch
...
function updateQuantityFromInput(el) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            ...
            // Certain themes need to update shopping cart widget, they react on updateCart event;
            // However, some other themes may do over-work updating also cart summary, so it shall be disabled for some themes
            // Change with custom JS: let tc_disableUpdateCartEmitEvent = true;
            var disableUpdateCartEmit = (typeof tc_disableUpdateCartEmitEvent !== 'undefined') ? tc_disableUpdateCartEmitEvent : false;
            if (!disableUpdateCartEmit) {
...
function displayStaticCustomerInfoAndNav(customerSignInArea)
{
    if ('undefined' !== typeof customerSignInArea) {
        if ('undefined' !== typeof customerSignInArea.staticCustomerInfo) {
            ...
            $('body').removeClass('is_logged_0').addClass('is_logged_1');
        }
...
function parseShaimVatNumberErrors(errors) {
    let shaim_vatnumber_notices = [];
    let notice_status_error = false;
    let modifiedVatNumberErrors = errors.vat_number.filter(error => {
        if (error.startsWith('shaim_vatnumber:')) {
            let errorCode = error.split(':')[1];
            if (![ '-1', '0', '1' ].includes(errorCode)) {
                // probably the 'home' country or unknown status code (both is -2)
                return false;
            }
            let errMsg = errorCode
            if (typeof i18_shaim_vatnumber_notices !== 'undefined' && i18_shaim_vatnumber_notices[errorCode]) {
                errMsg = i18_shaim_vatnumber_notices[errorCode];
            }
            shaim_vatnumber_notices.push(errMsg);
            if (errorCode !== '1') {
                notice_status_error = true;
            }
            return false;
        }
        return true;
    ...
    return {
        errors: { ...errors, vat_number: modifiedVatNumberErrors },
        shaim_vatnumber_notices: shaim_vatnumber_notices.join(', '),
        notice_status_error: notice_status_error
    };
}
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            ...
            if ("undefined" !== typeof jsonData.invoice && null !== jsonData.invoice) {
                ...
                if (jsonData.invoice.hasErrors && jsonData.invoice.errors.vat_number && jsonData.invoice.errors.vat_number.length) {
                    removeError(blockSel + ' .field.notice-msg');
                    let result = parseShaimVatNumberErrors(jsonData.invoice.errors);
                    jsonData.invoice.errors = result.errors;
                    jsonData.invoice.hasErrors = Object.values(jsonData.invoice.errors).some(error => Array.isArray(error) && error.length > 0);
                    let shaim_vatnumber_notices = {}
                    shaim_vatnumber_notices.vat_number = result.shaim_vatnumber_notices;
                    // Ignore errors/notices, if VAT number field is empty
                    if ($('#thecheckout-address-invoice [name=vat_number]').val() && result.shaim_vatnumber_notices !== '') {
                        printContextNotices(blockSel, shaim_vatnumber_notices, result.notice_status_error);
                    }
                }
            ...
            if ("undefined" !== typeof jsonData.delivery && null !== jsonData.delivery) {
                ...
                if (jsonData.delivery.hasErrors && jsonData.delivery.errors.vat_number && jsonData.delivery.errors.vat_number.length) {
                    removeError(blockSel + ' .field.notice-msg');
                    let result = parseShaimVatNumberErrors(jsonData.delivery.errors);
                    jsonData.delivery.errors = result.errors;
                    jsonData.delivery.hasErrors = Object.values(jsonData.delivery.errors).some(error => Array.isArray(error) && error.length > 0);
                    let shaim_vatnumber_notices = {}
                    shaim_vatnumber_notices.vat_number = result.shaim_vatnumber_notices;
                    // Ignore errors/notices, if VAT number field is empty
                    if ($('#thecheckout-address-delivery [name=vat_number]').val() && result.shaim_vatnumber_notices !== '') {
                        printContextNotices(blockSel, shaim_vatnumber_notices, result.notice_status_error);
                    }
                }
...
function signIn() {
    ...
    $.ajax({
        ...
        data: "&ajax_request=1&action=signIn&" +
            $('.checkout-block #login-form').serialize() +
            dynamicToken(),
        success: function (jsonData) {
...
function updateShippingPrices(shippingBlockHtml) {
    if ('undefined' !== typeof shippingBlockHtml && null !== shippingBlockHtml) {
...
function updateShippingBlock(shippingModulesList, html, checksum, triggerElementName) {
    if ('undefined' !== typeof html && null !== html && shippingBlockChecksum != checksum) {
        ...
        if ((typeof tc_initCarrierJs !== 'undefined' ? tc_initCarrierJs : true) &&
            $(deliveryOptionSelector).length && !forceRefreshShipping) {
            if (!payment.isConfirmationTrigger(triggerElementName)) {
                $(deliveryOptionSelector).prop('checked', false).trigger('click');
            } else {
                // for confirmation click, only emit thecheckout_updateDeliveryOption event to notify other components (namely shaim_shipping_modules),
                // but do not trigger element click
...
function selectPaymentOption(optionId, paymentFee) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            // Hook: prestashop.on('thecheckout_updatePaymentOption', function() { console.log('payment option updated!'); })
            prestashop.emit('thecheckout_updatePaymentOption', {
                reason: 'update',
            });
...
function updatePaymentBlock(paymentModulesList, html, checksum, triggerElementName) {
    if ('undefined' !== typeof html && null !== html && (paymentBlockChecksum != checksum || 'thecheckout-confirm' == triggerElementName)) {
        ...
        paymentBlockElement.find('input[type=text], select, input[name="issuer"], input[name="transferGateway"], form input[type=radio]:checked, form input[type=checkbox]:checked, textarea, input[type=date], input[type=tel]').each(function () {
        ...
        $.each(payment_fields_values, function (index, value) {
            if ($(index).is(':radio') || $(index).is(':checkbox')) {
                $(index+'[value="'+value+'"]').prop('checked', true);
            } else {
                $(index).val(value);
            }
...
function updateCartSummaryBlock(html, checksum, isVirtualCart) {
    if ('undefined' !== typeof html && null !== html && cartSummaryBlockChecksum != checksum) {

        // console.log('checksum', checksum, html);
...
function updateAddressBlock(addressType, html, htmlAddressDropdown) {
    if ('undefined' !== typeof html && null !== html) {
    ...
    if ('undefined' !== typeof htmlAddressDropdown && null !== htmlAddressDropdown) {
        if ("invoice" === addressType) {
...
function selectDeliveryOption(deliveryForm) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            // Hook: prestashop.on('thecheckout_updateDeliveryOption', function() { console.log('delivery option updated!'); })
            prestashop.emit('thecheckout_updateDeliveryOption', {
                reason: 'update',
            });
            $('#thecheckout-shipping .error-msg').hide();
