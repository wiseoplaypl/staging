 ...
 * support@mypresta.eu
 */
include_once('../modules/pinterestfeed/model/google.php');
include_once('../modules/pinterestfeed/model/pms.php');
include_once('../modules/pinterestfeed/pinterestfeed.php');

class AdminExportProductsFeedPinterestController extends ModuleAdminController
{
    ...
    public function ajaxProcess()
    {
        if (Tools::getValue('action') == 'downloadCategories' && Tools::getValue('language_code', 'false') != 'false') {
            ...
            if (file_exists('../modules/pinterestfeed/google/' . Tools::getValue('language_code') . '.txt') && (bool)$download != 0) {
                @unlink('../modules/pinterestfeed/google/' . Tools::getValue('language_code') . '.txt');
            }

            if ((bool)$download != 0 && @file_put_contents('../modules/pinterestfeed/google/' . Tools::getValue('language_code') . '.txt', $download)) {
                echo 1;
            } else {
                echo 0;
            }
        ...
        if (Tools::getValue('action') == 'downloadCategoriesCustom' && Tools::getValue('language_code', 'false') != 'false') {
            ...
            $this->context->smarty->assign('pinterestfeed_categories', $categories);
            $this->context->smarty->assign('pinterestfeed_google_categories', $google_categories);

            echo $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'pinterestfeed/views/script-fancybox.tpl') . $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'pinterestfeed/views/categories-fancybox.tpl');
            die();
        } elseif (Tools::getValue('action') == 'searchCategory') {
            $contents = Tools::file_get_contents('../modules/pinterestfeed/google/' . Tools::getValue('language_code') . '.txt');
    ...
    public function renderConfigurationForm()
    {
        ...
        $inputs = array(
            ...
            array(
                ...
                'desc' => $this->l('Decide if you want to save feed to file or download it immediately once feed is generated') .
                    '<br/>' .
                    $this->l('Option when active will save feed to file located in module directory:') . '<span class="savedFeedUrl" class="font-weight:bold">' . Context::getContext()->shop->getBaseURL(true, true) . 'modules/pinterestfeed/</span><span class="SavedFeedUrlFile"></span><br/>' .
            ...
            array(
                ...
                'desc' => $this->l('If you want to change format of product name - select possible options.') . $this->l('This option is here because Pinterest does not allow to use CAPITALIZED TEXT for product names. So you can reformat the name of products with this option. Do it if your default names of items do not follow Pinterest shopping requirements.'),
            ...
            array(
                ...
                'value' => ';',
                'desc' => $this->l('The character to separate the fields in CSV file') . '. ' . $this->l('Usually pipe "|" - Pinterest allows to use it as a column delimiter')
            ),
            array(
                'type' => 'radio',
        ...
        $helper->token = Tools::getAdminTokenLite('AdminExportProductsFeedPinterest');
    ...
    public function loadScript()
    {
        ...
        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'pinterestfeed/views/script.tpl');
    }
    ...
    public function heading()
    {
        ...
        " . $this->l('This form generates a .csv or .xml file that is ready to import for Pinterest "products" purposes.') . "<br/>
        ...
        <div class='alert alert-info'>
            ...
            <div class='hide panel clearfix'>
                ...
                <div><span> " . $shop->getUrl(true) . "modules/pinterestfeed/feed.php?</span><span class='feedurl' style='word-wrap: break-word;'></span></div><br/><hr/><br/>
                <h3>" . $this->l('Non-secured URL') . "</h3>
                <div><span> " . $shop->getUrl(false) . "modules/pinterestfeed/feed.php?</span><span class='feedurl' style='word-wrap: break-word;'></span></div>    
            </div>
        </div>
        <div class='panel'>
            ...
            <input type='text' class='deserializeUrl' value='" . (Tools::isSubmit('updatepms') && !Tools::isSubmit('newfeed') ? (Tools::getValue('id_pms', 'false') != 'false' ? '?' . pms::getConfig(Tools::getValue('id_pms')) : '') : '') . "'/>
        </div>
        " . $this->loadScript();
    }
    ...
    public function renderExcludeForm()
    {
        $this->context->smarty->assign(array(
            ...
            'pinterestfeed_products' => (Configuration::get('gm_pinterestfeed_products', 'false') != 'false' && Configuration::get('gm_pinterestfeed_products', 'false') != '' && Configuration::get('gm_pinterestfeed_products', 'false') != NULL ? explode(",", Configuration::get('gm_pinterestfeed_products', 'false')) : array()),
            'this' => $this
        ));

        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'pinterestfeed/views/exclude-products.tpl');
    }
    ...
    public function renderGoogleCategories()
    {
        ...
        foreach (Language::getLanguages(true) AS $language) {
            if (file_exists('../modules/pinterestfeed/google/' . $language['language_code'] . '.txt')) {
        ...
        $this->context->smarty->assign('link', $this->context->link);
        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'pinterestfeed/views/google_lang_links.tpl');
    }

    public function getConfigFieldsValues()
    {
        return array(
            'pinterestfeed_products' => (Configuration::get('gm_pinterestfeed_products') != false ? explode(",", Configuration::get('gm_pinterestfeed_products')) : array()),
            ...
            'export_id_pms' => (Configuration::get('gm_export_id_pms') != false ? Configuration::get('gm_export_id_pms') : false),
        );
    }
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('submitExport')) {
            if (Tools::getValue('export_shortlink') == 1) {
                ...
                if ((int)Tools::getValue('export_id_pms')) {
                    $pms = new pms(Tools::getValue('export_id_pms'));
                    $pms->name = Tools::getValue('export_shortlinkName', $this->l('Unnamed Feed'));
                    $conf = '';
                    $post = array();
            ...
            if (Tools::getValue('remember_settings') == 1) {
                foreach ($this->getConfigFieldsValues() AS $gmkey => $gmvalue) {
                    if ($gmkey == 'pinterestfeed_products') {
                        if (Tools::getValue('pinterestfeed_products', 'false') != 'false') {
                            Configuration::updateValue('gm_pinterestfeed_products', implode(',', Tools::getValue('pinterestfeed_products')));
                        }
    ...
    {
        if (Tools::getValue('deletepms', 'false') != 'false') {
            $this->context->controller->confirmations[] = $this->l('Feed removed');
            $pms = new pms(Tools::getValue('id_pms'));
            $pms->delete();
        }
        ...
        return $helper->generateList(pms::getAll(), $this->helper_fields_list);
    }
    ...
    public function returnAlert($msg)
    {
        ...
        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'pinterestfeed/views/returnAlert.tpl');
    }
