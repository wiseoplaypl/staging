! function() {
    ...
    function b(a) {
        ...
        var b, c = {},
            d = a.match(/(?:[\?](?:[^=]+)=(?:[^&#]*)(?:[&](?:[^=]+)=(?:[^&#]*))*(?:[#].*)?)|(?:[#].*)/);
        if (null === d) return {};
        b = d[0].substr(1).split(/[&#=]/);
        for (var e = 0; e < b.length; e += 2) c[decodeURIComponent(b[e])] = decodeURIComponent(b[e + 1] || "");
        return c
    }
    ...
    function c(a) {
        ...
        var b = "",
            c = 0,
            d = Object.keys(a.params);
        if (0 === d.length) return "";
        for (d.sort(), a.hasParams || (b += "?" + d[0] + "=" + a.params[d[0]], c += 1); c < d.length; c += 1) b += "&" + d[c] + "=" + a.params[d[c]];
        return b
    }
    ...
    function d(a) {
        ...
        if (!a.match(/^(\d+[smhdw]?)+$/)) return 0;
        a = a.replace(/([smhdw])/g, " $1 ").trim(), b = a.split(" ");
        for (var e = 0; e < b.length; e += 2) c += parseInt(b[e], 10) * d[b[e + 1] || "s"];
        return c
    }
    ...
    function f() {
        this.provider = "canalplus", this.defaultFormat = "embed", this.formats = {
    ...
    e.prototype.parseProvider = function(a) {
        ...
        var c, d = this,
            e = d.parseProvider(a),
            f = d.plugins[e];
        if (e && f && f.parse) return c = f.parse.apply(f, [a, b(a)]), c && (c = d.removeEmptyParameters(c), c.provider = f.provider), c
    }, e.prototype.bind = function(a) {
        if (this.plugins[a.provider] = a, a.alternatives)
            for (var b = 0; b < a.alternatives.length; b += 1) this.plugins[a.alternatives[b]] = a
    }, e.prototype.create = function(b) {
        var c = b.videoInfo,
            d = b.params,
            e = this.plugins[c.provider];
        if (d = "internal" === d ? c.params : d || {}, e && (b.format = b.format || e.defaultFormat, e.formats.hasOwnProperty(b.format))) return e.formats[b.format].apply(e, [c, a(d)])
    };
    var m = new e;
    f.prototype.parseParameters = function(a) {
        ...
        if (d.params = c.parseParameters(b), d.id) return d
    }, f.prototype.createEmbedUrl = function(a, b) {
        var d = "http://player.canalplus.fr/embed/";
        return b.vid = a.id, d += c({
            params: b
        })
    }, m.bind(new f), g.prototype.parseUrl = function(a) {
        var b = a.match(/(?:embed|view)\/([a-zA-Z\d]+)/i);
        return b ? b[1] : void 0
        ...
        var b = a.match(/(?:\/video|ly)\/([A-Za-z0-9]+)/i);
        return b ? b[1] : void 0
    }, h.prototype.parse = function(a, b) {
        var c = this,
            d = {
                mediaType: this.mediaTypes.VIDEO,
                params: c.parseParameters(b),
                id: c.parseUrl(a)
            };
        return d.id ? d : void 0
        ...
        var c = b.channel || b.utm_content || a.channel;
        return delete b.utm_content, delete b.channel, c
    }, i.prototype.parseUrl = function(a, b, c) {
        var d;
        if (d = a.match(/(clips\.)?twitch\.tv\/(\w+)(?:\/(?:(.)\/(\d+)|(\w+)))?/i), b.channel = d ? d[2] : void 0, d && d[3] && d[4]) b.id = d[3] + d[4];
        else if (c.video) b.id = c.video, delete c.video;
        else if (d && d[1] && d[5]) b.id = d[5], b.isClip = !0;
        else if (c.clip) {
            var e = c.clip.split("/");
            b.channel = e[0], b.id = e[1], b.isClip = !0, delete c.clip
        }
        return b
    }, i.prototype.parseMediaType = function(a) {
        var b;
        return a.channel ? a.id && a.isClip ? (b = this.mediaTypes.CLIP, delete a.isClip) : b = a.id && !a.isClip ? this.mediaTypes.VIDEO : this.mediaTypes.STREAM : a.id && (b = this.mediaTypes.EMBEDVIDEO, delete a.channel), b
    }, i.prototype.parseParameters = function(a) {
        return a.t && (a.start = d(a.t), delete a.t), a
    }, i.prototype.parse = function(a, b) {
        var c = this,
            d = {};
        return d = c.parseUrl(a, d, b), d.channel = c.parseChannel(d, b), d.mediaType = c.parseMediaType(d), d.params = c.parseParameters(b), d.channel || d.id ? d : void 0
    }, i.prototype.createLongUrl = function(a, b) {
        var d = "";
        if (a.mediaType === this.mediaTypes.STREAM) d = "https://twitch.tv/" + a.channel;
        else if (a.mediaType === this.mediaTypes.VIDEO) {
            var e = this.seperateId(a.id);
            d = "https://twitch.tv/" + a.channel + "/" + e.pre + "/" + e.id, b.start && (b.t = b.start + "s", delete b.start)
        } else a.mediaType === this.mediaTypes.CLIP && (d = "https://clips.twitch.tv/" + a.channel + "/" + a.id);
        return d += c({
            params: b
        })
    }, i.prototype.createEmbedUrl = function(a, b) {
        var d = "https://player.twitch.tv/";
        return a.mediaType === this.mediaTypes.STREAM ? b.channel = a.channel : a.mediaType === this.mediaTypes.VIDEO || a.mediaType === this.mediaTypes.EMBEDVIDEO ? (b.video = a.id, b.start && (b.t = b.start + "s", delete b.start)) : a.mediaType === this.mediaTypes.CLIP && (d = "https://clips.twitch.tv/embed", b.clip = a.channel + "/" + a.id), d += c({
            params: b
        })
    }, m.bind(new i), j.prototype.parseUrl = function(a) {
        var b = a.match(/(?:\/(?:channels\/[\w]+|(?:(?:album\/\d+|groups\/[\w]+)\/)?videos?))?\/(\d+)/i);
        return b ? b[1] : void 0
        ...
        var c = {
            mediaType: this.mediaTypes.VIDEO,
            params: this.parseParameters(b),
            id: this.parseUrl(a)
        };
        return c.id ? c : void 0
    }, j.prototype.createUrl = function(a, b, d) {
        var e = a + b.id,
            f = d.start;
        return delete d.start, e += c({
            params: d
        }), f && (e += "#t=" + f), e
        ...
        var b = a.match(/(?:(?:v|vi|be|videos|embed|shorts)\/(?!videoseries)|(?:v|ci)=)([\w\-]{11})/i);
        return b ? b[1] : void 0
        ...
        if (a.params.list && (a.list = a.params.list, delete a.params.list), a.id && !a.params.ci) a.mediaType = this.mediaTypes.VIDEO;
        else if (a.list) delete a.id, a.mediaType = this.mediaTypes.PLAYLIST;
        else {
            if (!a.params.ci) return;
            delete a.params.ci, a.mediaType = this.mediaTypes.SHARE
        }
        return a
        ...
        return d.params = c.parseParameters(b, d), d = c.parseMediaType(d)
    }, k.prototype.createShortUrl = function(a, b) {
        var c = "https://youtu.be/" + a.id;
        return b.start && (c += "#t=" + b.start), c
    }, k.prototype.createLongUrl = function(a, b) {
        var d = "",
            e = b.start;
        return delete b.start, a.mediaType === this.mediaTypes.PLAYLIST ? (b.feature = "share", d += "https://youtube.com/playlist") : a.mediaType === this.mediaTypes.VIDEO ? (b.v = a.id, d += "https://youtube.com/watch") : a.mediaType === this.mediaTypes.SHARE && (b.ci = a.id, d += "https://www.youtube.com/shared"), a.list && (b.list = a.list), d += c({
            params: b
        }), a.mediaType !== this.mediaTypes.PLAYLIST && e && (d += "#t=" + e), d
    }, k.prototype.createEmbedUrl = function(a, b) {
        var d = "//youtube.com/embed";
        return a.mediaType === this.mediaTypes.PLAYLIST ? b.listType = "playlist" : (d += "/" + a.id, "1" === b.loop && (b.playlist = a.id)), a.list && (b.list = a.list), d += c({
            params: b
        })
    }, k.prototype.createImageUrl = function(a, b, c) {
        var d = a + b.id + "/",
            e = c.imageQuality || this.defaultImageQuality;
        return d + e + ".jpg"
        ...
        var b = a.match(/(?:(?:embed|sid)\/|v_show\/id_|VideoIDS=)([a-zA-Z0-9]+)/);
        return b ? b[1] : void 0
        ...
        var d = "http://player.youku.com/player.php/sid/" + a.id + "/v.swf";
        return d += c({
            params: b
        }), d
