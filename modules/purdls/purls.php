...
class purls extends Module
{
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'lang' => true,
                        'desc' => $this->l('Route to cms page.') . ' ' . $this->l('Available keywords: ') . '{id}, {categories:/}, {rewrite}* <span class="label label-danger">'. (Configuration::get('purls_cms') == 0 ? $this->l('To edit this field you need to activate option to remove id from this type of url'):'').'</span>',
                    ),
                    array(
                        'type' => 'text',
    ...
    {
        if (!parent:: install() || !$this->registerHook('Header') || !$this->registerHook('ModuleRoutes') || !$this->registerHook('actionAdminControllerSetMedia')) {
            return false;
        }
        return true;
    }
    ...
    public function hookModuleRoutes()
    {
        ...
        $supplier_rule = rtrim(Configuration::get('PS_ROUTE_supplier_rule'), '/');
        if ($supplier_rule == 'NULL' || $supplier_rule == false) {
            $supplier_rule = 'supplier/{rewrite}';
        };
        $manufacturer_rule = rtrim(Configuration::get('PS_ROUTE_manufacturer_rule'), '/');
    ...
    public static function getAnchor($id, $id_product_attribute, $with_id = false, $with_group = false)
    {
        ...
        foreach ($attributes as &$a) {
            ...
            $anchor .= '/' . ($with_id && isset($a['id_attribute']) && $a['id_attribute'] ? ($with_group ? $a['id_attribute_group'] . '|':'') . (int)$a['id_attribute'] . $sep : '') . $a['group'] . $sep . $a['name'];
        }
                ...
                if (is_array($attributes)) {
                    if (count($attributes) > 0) {
                        ...
                        foreach ($attributes as $combination) {
                            $json[str_replace("#/", "", self::getAnchor(Tools::getValue('id_product'), (int)$combination['id_product_attribute'], false))] = str_replace("#/", "", self::getAnchor(Tools::getValue('id_product'), (int)$combination['id_product_attribute'], true, true));
                            $json[str_replace("#/", "", self::getAnchor(Tools::getValue('id_product'), (int)$combination['id_product_attribute'], true))] = str_replace("#/", "", self::getAnchor(Tools::getValue('id_product'), (int)$combination['id_product_attribute'], true, true));
                        }
