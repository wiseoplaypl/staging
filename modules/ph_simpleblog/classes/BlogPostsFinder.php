...
class BlogPostsFinder
{
    ...
    public function findPosts()
    {
        ...
        if ($this->getIdCategory() > 0) {
            ...
            if ($childrens && sizeof($childrens)) {
                $child_categories = [$this->getIdCategory()];
        ...
        $sql->orderBy('sbp.is_featured > 0 DESC, '.$this->getOrderBy() . ' ' . $this->getOrderWay());
    ...
    private function getPostProperties($posts, $id_lang)
    {
        ...
        foreach ($posts as &$row) {
            ...
            $row['tags'] = isset($tags[$id_lang]) && isset($tags[$id_lang]) ? $tags[$id_lang] : [];
            
            if ((int) $row['id_simpleblog_author'] > 0) {
                $row['author'] = new SimpleBlogPostAuthor((int) $row['id_simpleblog_author'], $id_lang);
            }
