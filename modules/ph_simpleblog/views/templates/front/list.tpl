...
<div class="ph_simpleblog simpleblog-{if $is_category}category{else}home{/if}">
	...
	{if Configuration::get('PH_BLOG_DISPLAY_CATEGORY_CHILDREN')}
		...
		{else}
			{assign var="subcategories" value=SimpleBlogCategory::getCategories($cookie->id_lang, true, true)}
		{/if}

		{if $subcategories && $subcategories|count > 0}
			<div class="blogSubcat">
				<ul class="blogSubcat__list">
					{foreach from=$subcategories item=category}
						<li class="blogSubcat__item">
							<a
								class="blogSubcat__link btn btn-default"
								href="{$category.url}"
								>	
								{$category.name}
							</a>
	...
	{if isset($posts) && count($posts)}
		...
		<div class="{if Configuration::get('PH_BLOG_MASONRY_LAYOUT')}blog-masonry-list{else}row{/if} simpleblog-posts">
