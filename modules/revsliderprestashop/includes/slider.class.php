...
class RevSliderSlider extends RevSliderFunctions {
	...
	public function init_by_id($sid, $show_error = true){
		...
		if(empty($slider_data) && !RevLoader::is_admin() && $show_error === true){
	...
	public function init_by_alias($alias, $show_error = true){
		...
		if(empty($slider_data) && !RevLoader::is_admin() && $show_error === true){
			throw new Exception('Slider with alias '. RevLoader::sanitize_text_field( RevLoader::esc_attr($alias)).' not found.');
		}
	...
	public function init_by_data($data){
		$data = RevLoader::apply_filters('revslider_slider_init_by_data', $data);
		...
		RevLoader::do_action('revslider_slider_init_by_data_post', $this);
	}
	...
	public function is_posts(){
		...
		return (in_array($source, array('post', 'posts', 'specific_posts', 'specific_post', 'current_post', 'woocommerce', 'woo','woo_specific','ps_category'), true)) ? true : false;
	}
...
//		$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s ORDER BY slide_order ASC LIMIT 0,1", array($this->get_id())), ARRAY_A);
		$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s ORDER BY slide_order ASC ", array($this->get_id())), ARRAY_A);
...
//		$record = $wpdb->get_row($wpdb->prepare("SELECT `alias` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s LIMIT 0,1", array($slider_id)), ARRAY_A);
		$record = $wpdb->get_row($wpdb->prepare("SELECT `alias` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s LIMIT ", array($slider_id)), ARRAY_A);
	...
	private function delete_all_slides(){
		...
		RevLoader::do_action('revslider_slider_delete_all_slides', $this->id);
		RevLoader::do_action('revslider_slider_deleteAllSlides', $this->id);
	}
	...
	public function delete_static_slide(){
		...
		RevLoader::do_action('revslider_slider_delete_static_slide', $this->id);
	}
	...
	public function update_title($new_title){
		...
		$new_title = stripslashes(RevLoader::esc_html($new_title));
		if(!empty($new_title)){
...
//		$record = $wpdb->get_row("SELECT `id` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." ORDER BY `id` DESC LIMIT 0,1", ARRAY_A);
		$record = $wpdb->get_row("SELECT `id` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." ORDER BY `id` DESC ", ARRAY_A);
	...
	public function get_slide_children($slide_id){
		...
		if(!isset($slides[$slide_id])){
			$this->throw_error(RevLoader::__('Slide not found in the main slides of the slider. Maybe it', 'revslider'));
		}
	...
	private function duplicate_slider($title = false, $prefix = false){
		...
		if($title === false){
			...
			$new_title = ($prefix !== false) ? RevLoader::sanitize_text_field($title.' '.$this->get_val($params, 'title')) : $title;
			$new_alias = ($prefix !== false) ? RevLoader::sanitize_title($title.' '.$this->get_val($params, 'title')) : RevLoader::sanitize_title($title);
	...
	public function save_slider($slider_id, $data){
		...
		if($this->check_alias($alias)){
			$this->throw_error(RevLoader::__('A Slider with the given alias already exists', 'revslider'));
		}
	...
	public function get_slides_for_export(){
		...
		return RevLoader::apply_filters('revslider_get_slides_for_export', RevLoader::apply_filters('revslider_getSlidesForExport', $export));
	}
	...
	public function get_static_slide_for_export(){
		...
		return RevLoader::apply_filters('revslider_getStaticSlideForExport', $static_slide);
	}
	...
	public function get_parent_slides($published, $gal_ids, $lang){
		...
		RevLoader::apply_filters('revslider_get_parent_slides_pre', $lang, $published, $gal_ids, $this);
		...
		RevLoader::apply_filters('revslider_get_parent_slides_post', $parent_slides, $published, $gal_ids, $this);
...
//		$record = $wpdb->get_row($wpdb->prepare("SELECT slide_order FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %d ORDER BY slide_order DESC LIMIT 0,1", $this->id), ARRAY_A);
		$record = $wpdb->get_row($wpdb->prepare("SELECT slide_order FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %d ORDER BY slide_order DESC ", $this->id), ARRAY_A);
	...
	public function copy_slide_to_slider($data){
		...
		if(empty($add_to_slider))
			return RevLoader::__('Slide could not be duplicated', 'revslider');
		...
		return ($response === false) ? RevLoader::__('Slide could not be duplicated', 'revslider') : true;
	}
	...
	public function get_used_fonts($full = false){
		...
		return RevLoader::apply_filters('revslider_getUsedFonts', $gf);
	}
	...
	//  */
	// public function get_slides_from_posts_1($published = false, $gal_ids = array(), $lang = 'all'){
	...
	// 		break;
	// 		default:
	// 			$this->throw_error(RevLoader::__('This Source Type must be from posts.', 'revslider'));
	// 		break;
	// 	}
	...
	public function get_slides_from_posts($published = false, $gal_ids = array(), $lang = 'all'){
		...
		switch($source){
			...
			*/
			...
			break;
			case 'woo_specific':
				$posts = $this->get_products_from_product_ids($published);
			break;
			case 'ps_category':
				$posts = $this->get_categories_by_id($published);
			break;
		...
		if(!empty($posts)){
			...
			$this->throw_error(RevLoader::__('No Category Id Selected.', 'revslider'));
			return;
		}
	...
	public function get_related_posts(){
		...
		$get_relateds		= RevLoader::apply_filters('revslider_get_related_posts', $query, $post_id);
    ...
    return true;
		...
		$args	= RevLoader::apply_filters('revslider_get_popular_posts', $args, $post_id);
	...
	public function get_latest_posts($max_posts = false){
		...
		$args	= RevLoader::apply_filters('revslider_get_latest_posts', $args, $post_id);
		$posts	= get_posts($args);
	...
	public function get_slides_from_stream($published = false){
		...
		switch($sourcetype){
			...
			break;
			default:
				$this->throw_error(RevLoader::__('Sorry, this Social Stream cannot be displayed.', 'revslider'));
			break;
		}
		
		if($max_posts < 0) $max_posts *= -1;
		
		$posts = RevLoader::apply_filters('revslider_pre_mod_stream_data', $posts, $sourcetype, $this->id);
		...
		$posts = RevLoader::apply_filters('revslider_post_mod_stream_data', $posts, $sourcetype, $this->id);
		
		if(empty($posts)) $this->throw_error(RevLoader::__('Sorry, this Social Stream cannot be displayed.', 'revslider'));
	...
	private function get_posts_by_categories($published = false){
		...
		return RevLoader::apply_filters('revslider_get_posts_by_categories', $post, $this);
	}
	 ...
	 */
	private function get_products_from_categories_1($published = false){
		$slider_id	= $this->get_id();
	...
	private function get_categories_by_id($published = false){
		...
		//$taxonomies	= $data['tax'];
		$taxonomies	= '';
		$image_type	= $this->get_param(array('source', 'woo', 'image_type'), 'home_default');
		...
		//$addition	= array_merge($addition, RevSliderWooCommerce::get_meta_query($this->get_params()));
		//return $this->get_posts_by_category($cat_ids, $sort_by, $sort_dir, $max_posts, $post_types, $taxonomies, $addition);
		return $this->get_category_by_cat_id($cat_ids, $sort_by, $sort_dir, $max_posts, $post_types, $taxonomies, $addition);
	}

	private function get_products_from_product_ids($published = false){

		$slider_id	= $this->get_id();
		$product_ids	= $this->get_param(array('source', 'woo', 'product'));
		...
		//return $this->get_posts_by_category($cat_ids, $sort_by, $sort_dir, $max_posts, $post_types, $taxonomies, $addition);
		return $this->get_posts_by_category_custom($product_ids, $sort_by, $sort_dir, $max_posts, $post_types, $taxonomies, $addition);
	...
	private function get_products_from_categories($published = false){
		...
		//return $this->get_posts_by_category($cat_ids, $sort_by, $sort_dir, $max_posts, $post_types, $taxonomies, $addition);
		return $this->get_posts_by_category_custom($cat_ids, $sort_by, $sort_dir, $max_posts, $post_types, $taxonomies, $addition);
	}
	

	public static function get_category_by_cat_id($catIDs, $sortBy, $direction){
			...
			if (@RevsliderPrestashop::getIsset($categoriesid) && !empty($categoriesid)) {
					foreach ($categoriesid as $catid) {
							$results_temp = self::getAllCategory($catid, $sortBy, $direction);
...
public static function getAllCategory($id_category, $order = 'ASC', $limit = null){
		...
		{
			$row['id_image'] = (file_exists(_PS_CAT_IMG_DIR_.$row['id_category'].'.jpg')) ? $row['id_category'] : Language::getIsoById($id_lang).'-default';
		}
...
public static function get_posts_by_category_custom($catIDs, $sortBy, $direction, $numPosts = -1, $postTypes = "any", $taxonomies = "category", $arrAddition = array())
{
		if ($numPosts == -1) {
				$numPosts = null;
		}
...
//                }
		...
		$get_product = 'by_cat';
		if(isset($arrAddition['product']) &&  $arrAddition['product'] == 'specific'){
			$get_product = 'by_product_id';
		}

		$i = 0;
		if (@RevsliderPrestashop::getIsset($categoriesid) && !empty($categoriesid)) {
				foreach ($categoriesid as $catid) {
						$results_temp = self::getAllProducts($catid, $sortBy, $direction, $numPosts, $get_product);
						foreach ($results_temp as $temp) {
								$results[$i] = $temp;
								$i++;
						}
...
public static function getAllProducts($id_category, $order_by = null, $order_way = null, $limit = null, $get_product = 'by_cat')
{
		...
		$product_or_cat = '`id_category`';
		if($get_product != 'by_cat'){
			$product_or_cat = '`id_product`';
		...
		$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, MAX(product_attribute_shop.id_product_attribute) id_product_attribute, product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, pl.`description`, pl.`description_short`, pl.`available_now`,
				...
				Shop::addSqlAssociation('image', 'i', false, 'image_shop.cover=1') . '
																				...
																				WHERE product_shop.`id_shop` = ' . (int) $context->shop->id . '
																								AND cp.'.$product_or_cat.' = ' . (int) $id_category
...
public function getSliderImgSizeTypeSettings($slider_id)
{
		$this->validate_numeric($slider_id, "Slider ID");
		global $wpdb;
		try {
			$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %d", $slider_id), ARRAY_A);
			} catch (Exception $e) {
			$this->throw_error("Slider with ID: $slider_id Not Found");
		}
		$slider_data_settings = json_decode($slider_data['params']);

		if(isset($slider_data_settings->source->woo->image_type)){
			return $slider_data_settings->source->woo->image_type;
		}else{
			return 'home_default';
		}
	...
	public function get_overview_data($slider = false, $slides = false){
		...
		return array(
			...
			'slide_id'	=> $id,
			'slide_ids'	=> $slide_ids,
			'title'		=> RevLoader::esc_html($slider->get_title()),
			'alias'		=> $slider->get_alias(),
			'source'	=> RevLoader::esc_html($type),
	...
	public function get_specific_posts($gal_ids = array()){
		...
		if(!empty($gal_ids) && $gal_ids[0] !== ''){
			$posts	= $gal_ids;
			$posts	= RevLoader::apply_filters('revslider_set_posts_list_gal', $posts, $this->get_id());
			$is_gal	= true;
		}else{
			if(isset($gal_ids[0])){
				unset($gal_ids[0]);
			...
			$posts = RevLoader::apply_filters('revslider_set_posts_list', $posts, $this->get_id());
		}
	...
	public function get_posts_by_id($ids, $slider_id, $is_gal, $additional = array()){
		...
		$query	= RevLoader::apply_filters('revslider_get_posts', $query, $slider_id);
	...
	public function get_posts_by_category($slider_id, $cat_id, $sort_by = 'ID', $direction = 'DESC', $max_posts = -1, $post_types = 'any', $taxonomies = 'category', $addition = array(), $type = ''){
		$a = RevLoader::apply_filters('revslider_get_posts_by_category', array('slider_id' => $slider_id, 'cat_id' => $cat_id, 'sort_by' => $sort_by, 'direction' => $direction, 'max_posts' => $max_posts, 'post_types' => $post_types, 'taxonomies' => $taxonomies, 'addition' => $addition, 'type' => $type), $this);
	...
	public function get_tax_by_cat_id($cat_ids){
		...
		if(!empty($cat_ids)){
			foreach($cat_ids as $cat){
				...
				$pos = strrpos($cat, '_');
				if($pos === false) $this->throw_error(RevLoader::__('Wrong category format', 'revslider'));
