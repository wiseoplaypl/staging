...
class RevSliderObjectLibrary extends RevSliderFunctions {
	...
	public function _get_list($force = false){
		...
		$last_check	= RevLoader::get_option('revslider-library-check');

		if($last_check == false){ //first time called
			$last_check = 1296001;
			RevLoader::update_option('revslider-library-check', time());
		}
		...
		if(time() - $last_check > 1296000 || $force == true){ //30 days
			...
			$code = ($validated == 'false') ? '' : RevLoader::get_option('revslider-code', '');
			$hash = RevLoader::get_option('revslider-library-hash', '');
			$rattr = array(
			...
			if(!RevLoader::is_wp_error($request)){
				if($response = RevLoader::maybe_unserialize($request['body'])){
					if('actual' != $response){
						...
						if(is_array($library)){
							if(isset($library['hash'])) RevLoader::update_option('revslider-library-hash', $library['hash']);
	...
	public function get_object_handle_by_id($id){
		$handle	 = '';
		$full	 = RevLoader::get_option('rs-library', array());
	...
	public function _get_object_thumb($object_handle, $type, $download = false){
		...
		if($validated == 'false' && !in_array($type, $this->allowed_types, true)){
			return array('error' => RevLoader::__('Plugin not activated', 'revslider'));
		}
		
		// Check folder permission and define file location
		if($_download && $download === true && RevLoader::wp_mkdir_p($this->upload_dir['basedir'].$path)){
			//track
			...
			if(!is_file($file)){
				...
				if($curl !== false){
					if($validated == 'false' && !in_array($type, $this->allowed_types, true)){
						$error = RevLoader::__('Plugin not activated', 'revslider');
					}else{
						$rslb	= new RevSliderLoadBalancer();
						$code	= ($validated == 'false') ? '' : RevLoader::get_option('revslider-code', '');
						$rattr	= array(
						...
						$url = 'http://revapi.smartdatasoft.net/v6/call/object.php?'. http_build_query($rattr);
						$image_data = RevLoader::wp_remote_post($url, array(
								'user-agent' => RevLoader::get_bloginfo('url'),
								'body'     => '',
								'timeout'  => 45
						));
						...
						if(!RevLoader::is_wp_error($image_data) && isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200'){
							$image_data = rtrim($image_data['body'], "1");
							...
							$image_data = false;
							$error = RevLoader::__('Error downloading object', 'revslider');
						}
					}
				}else{
					//cant download file
	...
	public function _get_object_layers($object_id){
		...
		if(intval($object_id) > 0){
			...
			$error = RevLoader::__('Error downloading layers', 'revslider');
			return array('error' => $error);
		}
		...
		if($curl !== false){
			...
			if($validated == 'false' && !in_array($type, $this->allowed_types)){
				...
				$url = 'http://revapi.smartdatasoft.net/v6/call/object.php?'. http_build_query($rattr);
				$layers_data = RevLoader::wp_remote_post($url, array(
						'user-agent' => RevLoader::get_bloginfo('url'),
						'body'     => '',
						'timeout'  => 45
				));
				...
				if(!RevLoader::is_wp_error($layers_data) && isset($layers_data['body']) && isset($layers_data['response']) && isset($layers_data['response']['code']) && $layers_data['response']['code'] == '200'){

					$layers_data = rtrim($layers_data['body'], "1");
					...
					$layers_data = false;
					$error = RevLoader::__('Error downloading layers data', 'revslider');
				}
			}
		}else{
			//cant download file
		...
		$error = ($layers_data === false && $error == '') ? RevLoader::__('Error downloading layers data', 'revslider') : $error;
		...
		if(!empty($data)){
			foreach($data as $k => $v){
				...
				if(!empty($svg_source)){
					...
					$t = explode('./uploads/revslider/', $svg_source);
	...
	public function load_objects_with_svg(){
		...
		$online	= RevLoader::get_option('rs-library', array());
	...
	public function create_image_dimensions($handle, $force = false){
		...
		$image = new ResizeImage($image_path);
		...
		if(!RevLoader::is_wp_error($image) && $imgsize !== false) {
			...
			foreach($this->sizes as $size){
				...
				if(!file_exists($upload_directory.$modified_file_name_without_ending.'.'.$file_ending) || $force){
					...
					$image->saveImage($upload_directory.$modified_file_name_without_ending.'.'.$file_ending);
				}
			}
	...
	*/
	...
	public function get_favorites(){
		return RevLoader::get_option('rs_obj_favorites', array());
	}
	...
	public function save_favorites($favourites){
		RevLoader::update_option('rs_obj_favorites', $favourites);
	}
	...
	public function get_svg_sets_url(){
		...
		return RevLoader::apply_filters('revslider_get_svg_sets', $svg_sets);
	}
	...
	public function get_svg_sets_full(){
		...
		return RevLoader::apply_filters('revslider_get_svg_sets_full', $svg);
	}
	...
	public function get_font_icons(){
		...
		return RevLoader::apply_filters('revslider_get_font_icons', $font_icons);
	}
	...
	public function get_font_tags(){
		...
		return RevLoader::apply_filters('revslider_get_font_tags', $tags);
	}
