...
class RevSliderNavigation extends RevSliderFunctions {
	...
	public function get_all_navigations($defaults = true, $raw = false, $old = false){
		...
		if($defaults){
			...
			$default_presets = RevLoader::get_option('revslider-nav-preset-default', array());
	...
	public function delete_navigation($nav_id = 0){
		...
		if(!isset($nav_id) || intval($nav_id) == 0) return RevLoader::__('Invalid ID', 'revslider');
		
		$response = $wpdb->delete($wpdb->prefix.RevSliderFront::TABLE_NAVIGATIONS, array('id' => $nav_id));
		if($response === false) return RevLoader::__('Navigation could not be deleted', 'revslider');
	...
	public static function get_default_navigations(){
		...
		return RevLoader::apply_filters('revslider_mod_default_navigations', $navigations);
	}
	...
	public function add_preset($data){
		...
		foreach($navs as $nav){
			if($nav['id'] == $data['navigation']){ //found the navigation, get ID and update settings
				...
				if(isset($nav['factory']) && $nav['factory'] == true){
						...
						foreach($nav['presets'] as $prkey => $preset){
							if($prkey == $data['handle']){
								if(!isset($preset['editable'])){
									return RevLoader::__("Can't modify a default preset of default navigations", 'revslider');
								}
							}
						}
					}
						...
						foreach($default_presets[$nav['id']] as $prkey => $preset){
							if($prkey == $data['handle']){
								if($data['do_overwrite'] === false || $data['do_overwrite'] === 'false'){
									return RevLoader::__('Preset handle already exists, please choose a different name', 'revslider');
								}
								
								$default_presets[$nav['id']][$prkey] = array(
									'name'		=>  RevLoader::esc_attr($data['name']),
									//'handle'	=>  RevLoader::esc_attr($data['handle']),
									'type'		=>  RevLoader::esc_attr($data['type']),
									'values'	=> $data['values'],
									'editable'	=> true
								);
					...
					RevLoader::update_option('revslider-nav-preset-default', $default_presets);
					
					//return RevLoader::__('Can\'t add a preset to default navigations', 'revslider');
				}else{
		...
		return RevLoader::__('Navigation not found, could not add preset', 'revslider');
	...
	public function delete_preset($data){
		...
		foreach($navs as $nav){
			...
			if(isset($nav['factory']) && $nav['factory'] == true){
				...
				if(!empty($default_presets) && isset($default_presets[$nav['id']])){
					...
					return RevLoader::__('Can\'t delete default preset of default navigations', 'revslider');
				}
				return RevLoader::__('Preset not found in default navigations', 'revslider');
			}else{
				if(isset($nav['presets'])){
		...
		return RevLoader::__('Navigation not found, could not delete preset', 'revslider');
	}
