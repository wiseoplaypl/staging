...
class RevSliderSlide extends RevSliderFunctions {
	...
	public function get_settings(){
		return RevLoader::apply_filters('revslider_slide_get_settings', RevLoader::apply_filters('revslider_slide_getSettings', $this->settings, $this), $this);
	}
	...
	public function get_params(){
		return RevLoader::apply_filters('revslider_slide_get_params', RevLoader::apply_filters('revslider_slide_getParams', $this->params, $this), $this);
	}
	...
	public function get_layers(){
		return RevLoader::apply_filters('revslider_get_layers', RevLoader::apply_filters('revslider_getLayers', $this->layers, $this), $this);
	}
	...
	public function exist_by_id($slideid){
		...
		try{
			if(strpos($slideid, 'static_') !== false){
				...
				$this->validate_numeric($slide_id, RevLoader::__('Slide ID', 'revslider'));
	...
	public function init_by_slide(RevSliderSlide $slide){
		$slide = RevLoader::apply_filters('revslider_slide_initBySlide', $slide, $this);
		$this->id			= 'template';
	...
	public function save_slide($slide_id, $data, $slider_id){
		...
		//track
		//RevLoader::do_action('revslider_slide_updateSlideFromData_post', false, $data, $this);
		if(class_exists('RsBackupBase')){
			RevLoader::check_add_new_backup($data,$this);
		}
		return true;
	}
	...
	public function delete_slide_by_id($slide_id){
		...
		RevLoader::do_action('revslider_slide_deleteSlide', $slide_id);
	...
	//  */
	// public function init_by_post_data_($data, RevSliderSlide $template, $slider_id){
	...
	private function set_layers_by_post($post, $slider_id)
	{
			...
			if($source_type == 'woocommerce' || $source_type == 'woo' || 'woo_specific' == $source_type){
			...
			$price_display = Product::getTaxCalculationMethod((int) Context::getcontext()->cookie->id_customer);
			if (!$price_display) {
					$productprice = Tools::displayPrice($post["price"], Context::getContext()->currency);
			} else {
					$productprice = Tools::displayPrice($post["price_tax_exc"], Context::getContext()->currency);
			}

			$price_display = Product::getTaxCalculationMethod((int) Context::getcontext()->cookie->id_customer);
			if (!$price_display) {
					$regular_product_price = Tools::displayPrice($post["price_without_reduction"], Context::getContext()->currency);
			} else {
					$regular_product_price = Tools::displayPrice($post["price_without_reduction_without_tax"], Context::getContext()->currency);
			}

			$post_id = $post["id_product"];
			$countdown = $this->setCountDown($post);
			// $imgsrc = $this->setImageSrc($post);
			$title = $this->get_val($post, "name");
			//$excerpt_limit = $this->get_slider_param($slider_id, "excerpt_limit", 55, 'numeric');
			$excerpt_limit = (int) $excerpt_limit;
			$description = Tools::substr($post["description"], $excerpt_limit);
			$description_short = $post["description_short"];
			$link = $post["link"];
			$date_add = $post["date_add"];
			$date_upd = $post["date_upd"];
			$default_category = $post["default_category"];
			$ps_stock = $post["quantity"];
			$ps_stock_quantity = $post["quantity"];

			$link_obj         = new Link();
			//$add_cart_url = $link->getAddToCartURL( $post_id , "add=1&amp;id_product=" . $post_id);
			$addtocart = $link_obj->getPageLink('cart', false, null, "add=1&amp;id_product=" . $post_id, false);
			//$addtocart = 'addtocartlink';
			$product = new Product((int) $post_id, false, Context::getContext()->language->id);
			$img = $product->getCover($product->id);
			$home_default_img_url = $link_obj->getImageLink(isset($product->link_rewrite) ? $product->link_rewrite : $product->name, (int)$img['id_image'], 'home_default');
			if ( count( $img ) > 0 ) {
				$image     = new \Image( $img['id_image'] );
				$main_img_url = _PS_BASE_URL_SSL_ . _THEME_PROD_DIR_ . $image->getExistingImgPath() . '.jpg';
			}
				 ...
				 $actions = $this->get_val($layer, array('actions', 'action'), array());
					if(!empty($actions)){
						foreach($actions as $a_k => $action){					
							...
							if(!empty($ilink)){
								$ilink = str_replace(array('%link%', '{{link}}'), $link, $ilink);
								$ilink = str_replace(array('%ps_add_to_cart%', '{{ps_add_to_cart}}'), $addtocart, $ilink);
								$ilink = str_replace(array('%wc_add_to_cart%', '{{wc_add_to_cart}}'), $addtocart, $ilink);
					...
					if($this->get_val($layer, 'type', 'text') === 'image' && $this->get_val($layer, array('media', 'imageFromStream'), false) === true){
						$featured_image_url = $main_img_url;
					...
					$text = str_replace(array('%cover_image_url_thumbnail%', '{{cover_image_url_thumbnail}}'), $home_default_img_url, $text);
					$text = str_replace(array('%cover_image_url_original_size%', '{{cover_image_url_original_size}}'), $main_img_url, $text);
					$text = str_replace(array('%short_description%', '{{short_description}}'), $description_short, $text);
					$text = str_replace(array('%excerpt%', '{{excerpt}}'), $description_short, $text);
					$text = str_replace(array('%description%', '{{description}}'), $description, $text);
					$text = str_replace(array('%link%', '{{link}}'), $link, $text);
					$text = str_replace(array('%ps_add_to_cart%', '{{ps_add_to_cart}}'), $addtocart, $text);
					$text = str_replace(array('%countdown%', '{{countdown}}'), $countdown, $text);
					//$text = str_replace("%imgsrc%", $main_img_url, $text);
					$text = str_replace(array('%ps_stock%', '{{ps_stock}}'), $ps_stock, $text);
					$text = str_replace(array('%ps_stock_quantity%', '{{ps_stock_quantity}}'), $ps_stock_quantity, $text);
					$text = str_replace(array('%date%', '{{date}}'), $date_add, $text);
					$text = str_replace(array('%date_modified%', '{{date_modified}}'), $date_upd, $text);
					$text = str_replace(array('%ps_price%', '{{ps_price}}'), $productprice, $text);
					$text = str_replace(array('%ps_full_price%', '{{ps_full_price}}'), $productprice, $text);
					$text = str_replace(array('%wc_full_price%', '{{wc_full_price}}'), $productprice, $text);
					$text = str_replace(array('%ps_regular_price%', '{{ps_regular_price}}'), $regular_product_price, $text);
					$text = str_replace(array('%ps_categories%', '{{ps_categories}}'), $default_category, $text);
					...
					foreach ($arrMatches as $matched) {
							foreach ($matched as $match) {
									...
									$meta = str_replace("%", "", $meta);
									$meta = str_replace('_REVSLIDER_', '-', $meta);
									if (@RevsliderPrestashop::getIsset($post[$meta]) && !empty($post[$meta])) {
											$metaValue = $post[$meta];
	...
	private function set_layers_by_post_1($post, $slider_id){
		$post = RevLoader::apply_filters('revslider_slide_setLayersByPostData_pre', $post, $slider_id, $this);
		...
		$attr = RevLoader::apply_filters('revslider_slide_setLayersByPostData_post', $attr, $post, $slider_id, $this);
		
		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){
				$text = $this->get_val($layer, 'text');
				$text = RevLoader::apply_filters('revslider_mod_meta', $text, $post['ID'], $post); //option to add your own filter here to modify meta to your likings
...
// end start countdown JS
	...
	public function init_by_post_data($data, RevSliderSlide $template, $slider_id){
		...
		$this->id = $post_id;
		$this->params["title"] = RevSliderFunctions::getVal($data, "name");													
		$status = $data["active"];
		...
		if(!in_array($this->get_val($this->params, array('bg', 'type'), 'trans'), array('trans', 'solid'), true)){
			if($this->get_val($this->params, array('bg', 'imageFromStream'), false) === true){ //if image is choosen, use featured image as background // && $this->get_val($this->params, array('bg', 'type')) == 'image'
				...
				$slider_img_size_type = $revSlider->getSliderImgSizeTypeSettings($slider_id);
		
				if (!empty($post_id)) {
						$this->set_image_by_image_id($post_id, $slider_img_size_type);
				}
	 ...
	 */
	private function set_image_by_image_id($post_id , $img_type = ''){
		$id			= RevLoader::apply_filters('revslider_slide_setImageByImageID', $post_id, $this);
		$resolution = $this->get_val($this->params, array('bg', 'imageSourceType'), 'full');

		//track
		// $prdid_image = Product::getCover($id);
		// $id = $prdid_image['id_image'];
		// 	if (sizeof($prdid_image) > 0) {
		// 			$prdimage = new Image($prdid_image['id_image']);
		// 			$prdimage_url = _PS_BASE_URL_SSL_ . _THEME_PROD_DIR_;
		...
		$product = new Product((int) $post_id, false, Context::getContext()->language->id);
		$img = $product->getCover($product->id);
		$prdimage_url = $link_obj->getImageLink($product->link_rewrite, $img['id_image']);
        $prdimage_url = 'http://'. $prdimage_url;

		$this->image_id		= $post_id;
		//$this->image_url	= $this->get_url_attachment_image($id, $resolution);
		$this->image_url	= $prdimage_url;
		$this->image_url	= (RevLoader::is_ssl()) ? str_replace('http://', 'https://', $this->image_url) : $this->image_url;
		//$this->image_thumb	= $this->get_url_attachment_image($id, 'medium');
		$this->image_thumb	= $prdimage_url;
		if(empty($this->image_url)) return(false);
		//$this->params["background_type"] = "image"; //for 7.1
	...
	public function get_excerpt_by_id($id, $limit = 55){
		...
		return RevLoader::apply_filters('revslider_getExcerptById', $excerpt, $post, $limit);
	}
	...
	public function set_post_data($text, $attr, $post_id){
		...
		$text = RevLoader::apply_filters('rev_slider_insert_meta', $text, $post_id);
		...
		if(RevSliderWooCommerce::woo_exists()){
			...
			if($product !== false){
				...
				if(strpos($text, 'wc_add_to_cart_button') !== false){
					...
					$ajax_cart_en	= RevLoader::get_option('woocommerce_enable_ajax_add_to_cart') == 'yes' ? true : false;
					...
					if($ajax_cart_en){
						...
						if($wc_is_localized === false){ //load it only one time
							wp_localize_script('wc-add-to-cart', 'wc_add_to_cart_params', RevLoader::apply_filters('wc_add_to_cart_params', array(
								'ajax_url'			=> WC()->ajax_url(),
								'ajax_loader_url'	=> RevLoader::apply_filters('woocommerce_ajax_loader_url', $assets_path . 'images/ajax-loader@2x.gif'),
								'i18n_view_cart'	=> RevLoader::__('View Cart', 'woocommerce'),
								'cart_url'			=> get_permalink(wc_get_page_id('cart')),
								'is_cart'			=> is_cart(),
								'cart_redirect_after_add' => RevLoader::get_option('woocommerce_cart_redirect_after_add')
							)));
							$wc_is_localized = true;
						}
					}
					
					$wc_add_to_cart_button = RevLoader::apply_filters(
						'woocommerce_loop_add_to_cart_link',
						sprintf('<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="button %s product_type_%s">%s</a>',
							esc_url($product->add_to_cart_url()),
							 RevLoader::esc_attr($pr_id),
							 RevLoader::esc_attr($product->get_sku()),
							$product->is_purchasable() ? 'add_to_cart_button' : '',
							 RevLoader::esc_attr($pr_type),
							RevLoader::esc_html($product->add_to_cart_text())
						),
						$product
					);
				}
	...
	public function init_by_stream_data($data, $template, $slider_id, $sourcetype, $additions){
		$a = RevLoader::apply_filters('revslider_slide_initByStreamData', array('post_data' => $data, 'template' => $template, 'slider_id' => $slider_id, 'sourcetype' => $sourcetype, 'additions' => $additions), $this);
		...
		switch($a['sourcetype']){
			...
			break;
			default:
				$return = RevLoader::apply_filters('revslider_slide_initByStreamData_sourceType', false, $a, $this);
				
				if($return === false) $this->throw_error(RevLoader::__('Source must be from Stream', 'revslider'));
			break;
		}
	...
	private function init_by_facebook($slider_id, $additions){
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByFacebook_pre', $this->post_data, $slider_id, $additions, $this);
		...
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByFacebook_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_twitter($slider_id, $additions){
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByTwitter_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if(in_array($this->get_val($this->params, array('bg', 'type')), array('html5', 'trans', 'image', 'streamtwitter', 'streamtwitterboth'), true)){ //if image is choosen, use featured image as background
			...
			if($img == ''){
				...
				if(is_array($image_url_array) && isset($image_url_array[0])){
					$img = (RevLoader::is_ssl()) ? $this->get_val($image_url_array[0], 'media_url_https') : $this->get_val($image_url_array[0], 'media_url');
				}
			}
		...
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByTwitter_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_instagram($slider_id, $additions = array()){
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByInstagram_pre', $this->post_data, $slider_id, $this);
		...
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByInstagram_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_flickr($slider_id, $additions){
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByFlickr_pre', $this->post_data, $slider_id, $this);
		$this->id		 = $this->get_val($this->post_data, 'id');
		$this->set_param('title', $this->get_val($this->post_data, 'title'));
		$this->set_param(array('publish', 'state'), 'published');
		...
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByFlickr_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_youtube($slider_id, $additions){
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByYoutube_pre', $this->post_data, $slider_id, $additions, $this);
		...
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByYoutube_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_vimeo($slider_id, $additions){
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByVimeo_pre', $this->post_data, $slider_id, $additions, $this);
		...
		$this->post_data = RevLoader::apply_filters('revslider_slide_initByVimeo_post', $this->post_data, $slider_id, $additions, $this);
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false){
		...
		$text = RevLoader::apply_filters('revslider_slide_set_stream_data_pre', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
		...
		return RevLoader::apply_filters('revslider_slide_set_stream_data_post', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
	}
	...
	private function set_layers_by_stream($slider_id, $stream_type, $additions = array()){
		$a				= RevLoader::apply_filters('revslider_slide_setLayersByStreamData_pre', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
		...
		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){
				$text = $this->get_val($layer, 'text');
				$text = RevLoader::apply_filters('revslider_mod_stream_meta', $text, $slider_id, $stream_type, $this->post_data); //option to add your own filter here to modify meta to your likings
		...
		$a = RevLoader::apply_filters('revslider_slide_setLayersByStreamData_post', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
	...
	public function return_stream_data($stream_type, $additions = array()){
		$img_sizes = $this->get_all_image_sizes($stream_type);
		$attr = array();
		$attr = RevLoader::apply_filters('revslider_slide_return_stream_data_pre', $attr, $stream_type, $additions, $img_sizes);
		...
		switch($stream_type){
			...
			case 'instagram':
				...
				$attr1 = array(
					...
					'date'		=>  RevLoader::date_i18n(RevLoader::get_option('date_format').' '. RevLoader::get_option('time_format'), $this->get_val($this->post_data, 'taken_at_timestamp', false)),
		...
		return RevLoader::apply_filters('revslider_slide_return_stream_data_post', $attr, $stream_type, $additions, $img_sizes);
	}
	...
	public function get_facebook_timeline_image(){
		...
		return RevLoader::apply_filters('revslider_slide_get_facebook_timeline_image', $return, $object_id, $picture, $this);
	}
	...
	public function save_layers(){
		...
		$this->layers	= RevLoader::apply_filters('revslider_slide_saveLayers', $this->layers, $this->static_slide, $this);
	...
	public function save_params(){
		...
		$table = ($this->static_slide) ? $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES : $wpdb->prefix . RevSliderFront::TABLE_SLIDES;
		$this->params = RevLoader::apply_filters('revslider_slide_saveParams', $this->params, $this->static_slide, $this);
	...
	public function save_settings(){
		...
		$table = ($this->static_slide) ? $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES : $wpdb->prefix . RevSliderFront::TABLE_SLIDES;
		$this->settings = RevLoader::apply_filters('revslider_slide_save_settings', $this->settings, $this->static_slide, $this);
	...
	public function get_children(){
		...
		return RevLoader::apply_filters('revslider_slide_getArrChildren', $this->children, $this);
	}
	...
	public function create_slide($slider_id, $obj = '', $static = false, $id = false){
		...
		$insert	= RevLoader::apply_filters('revslider_slide_createSlide', $insert, $slider_id, $static, $this);
	...
	public function init_by_data($slide){
		$slide = RevLoader::apply_filters('revslider_slide_init_by_data', RevLoader::apply_filters('revslider_slide_initByData', $slide));
		...
		$image_url			= (RevLoader::is_ssl()) ? str_replace('http://', 'https://', $image_url) : $image_url;
		...
		if(!empty($slides)){
			...
			foreach($slides as $key => $slide){
				...
				if($this->get_val($slide, 'static', false) === true){
					$slides[$key]['title'] = RevLoader::__('Global Layers', 'revslider');
	...
	public function get_params_for_export(){
		...
		return RevLoader::apply_filters('revslider_slide_getParamsForExport', RevLoader::apply_filters('revslider_slide_getParamsForExport', $params, $this), $this);
	}
	...
	public function get_layers_for_export(){
		...
		return RevLoader::apply_filters('revslider_slide_get_layers_for_export', RevLoader::apply_filters('revslider_slide_getLayersForExport', $layers, $this), $this);
	}
	...
	public function get_image_attributes($layouttype){
		...
		return RevLoader::apply_filters('revslider_slide_get_image_attributes', array(
			'type'	=> $class,
			'src'	=> $thumb,
			'style'	=> $style
		), $this);
	...
	public function get_used_fonts($full = false){
		...
		return RevLoader::apply_filters('revslider_slide_getUsedFonts', $fonts, $this);
	}
	 ...
	 * @before: RevSliderSlide::setImageByImageID();
	 */
	private function set_image_by_image_id_($id){
		$id			= RevLoader::apply_filters('revslider_slide_setImageByImageID', $id, $this);
		$resolution = $this->get_val($this->params, array('bg', 'imageSourceType'), 'full');
	...
	public function set_image_by_id($imageID, $size = 'full'){
		$a = RevLoader::apply_filters('revslider_slide_setImageByID', array('imageID' => $imageID, 'size' => $size), $this);
	...
	public function get_categories_html($cat_ids, $tax = null, $post_id = ''){
		...
		if(!empty($errors)){
			...
			$this->throw_error(RevLoader::__('retrieving categories error: '.RevLoader::esc_html($err)));
		}
		...
		foreach($categories as $category){
			...
			$name = $category['name'];
			$list.= ($sep == true) ? ',' : '';
			$list.= (!empty($link)) ? '<a href="' . esc_url($link) . '" title="' .  RevLoader::esc_attr(sprintf(RevLoader::__('View all posts in %s', 'revslider'), $category['name'])) .'" '. $rel .'>'. $name .'</a>' : $name;
			$sep  = true;
		}
	...
	public function get_content_path(){
		if(RevLoader::is_multisite()){
			$path = (!defined('WP_CONTENT_DIR')) ? RS_PLUGIN_URL : RS_PLUGIN_URL; //FIX FOR PHP5
		}else{
			$path = (!defined('WP_CONTENT_DIR')) ? RS_PLUGIN_URL : RS_PLUGIN_URL; //FIX FOR PHP5
			//commented
