...
class RevSliderFunctions extends RevSliderData {
	...
	public function get_addition($key = ''){
		$additions = (array)RevLoader::get_option('revslider-additions', array());
	...
	public function validate_numeric($val, $fn = 'Field'){
		...
		if(!is_numeric($val))
			$this->throw_error($fn.RevLoader::__(' should be numeric', 'revslider'));
	}
	...
	public function validate_not_empty($val, $fn = 'Field'){
		if(empty($val) && is_numeric($val) == false)
			$this->throw_error($fn.RevLoader::__(' should not be empty', 'revslider'));
	}
	...
	public function get_image_id_by_url($image_url){
		...
		if($image_url !== ''){
			...
			if(0 == $attachment_id){ //get it the old school way
				$upload_dir_paths = RevLoader::wp_upload_dir();
	...
	public static function get_base_url(){
		if(RevLoader::is_multisite() == false){ //without multisite
			$url = RevLoader::content_url().'/';
		}else{	//for multisite
	...
	public static function esc_attr_deep($value){
		$value = is_array($value) ? array_map(array('RevSliderFunctions', 'esc_attr_deep'), $value) :  RevLoader::esc_attr($value);
		...
		if(trim($image) !== ''){
			...
			if(strpos($image, 'http') !== false){
				...
				if(strpos($image, 'img/cms') !== false){
					...
					$save_dir = _PS_ROOT_DIR_ . 'img/'.end($exp);
			...
			if(strpos($image, 'revsliderprestashop') !== false){

				$explode_image = explode('revsliderprestashop',$image);
			...
			if($cms_img !== ''){
				$image = _PS_BASE_URL_SSL_.__PS_BASE_URI__ . $cms_img;
			}
		}
		return $image;
	}
	...
	public function import_media($file_url, $folder_name){
		...
		if(@fclose(@fopen($file_url, 'r'))){ //make sure the file actually exists
			...
			if($_s_dir !== false){
				$s_dir = (! RevLoader::is_multisite()) ? 'uploads/'.$_s_dir : $_s_dir;
				$s_dir = str_replace('//', '/', $s_dir);
			}else{
				$art_dir = (!RevLoader::is_multisite()) ? 'uploads/'.$art_dir : $art_dir;
				$s_dir = str_replace('//', '/', $art_dir.$folder_name.$filename);
			}
