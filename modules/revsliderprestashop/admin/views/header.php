...
?>
		...
		<?php
			RevLoader::do_action('admin_enqueue_scripts');
			RevLoader::wp_print_styles();
			RevLoader::wp_print_head_scripts();
		?>



<div class="toplevel_page_revslider" >
...
$rs_image_meta_todo		 = RevLoader::get_option('rs_image_meta_todo', array());
...
if($rs_new_temp_counter === false){
	$_rs_tmplts			 = RevLoader::get_option('rs-templates', array());
...
//RevLoader::do_action('admin_enqueue_style');
...
<script type="text/javascript">
    ...
    // var test = window.location.origin + window.location.pathname + "?page=revslider";
    // alert(test);

    window.AdminControllerUrl = '<?php echo $context->link->getAdminLink( 'AdminRevsliderSliders' ); //track echo admin_url('admin.php?page=revslider'); ?>';
	...
	RVS.ENV.activated		= '<?php echo (RevLoader::get_option('revslider-valid', 'false')) == 'true' ? 'true' : 'false'; ?>';
	RVS.ENV.activated		= RVS.ENV.activated == 'true' || RVS.ENV.activated == true ? true : false;
	RVS.ENV.nonce			= '<?php echo RevLoader::wp_create_nonce('revslider_actions'); ?>';
	...
	RVS.ENV.wp_plugin_url 	= '<?php echo RS_PLUGIN_URL; ?>';

	RVS.ENV.admin_url		= '<?php echo $context->link->getAdminLink( 'AdminRevsliderSliders' ); //track echo admin_url('admin.php?page=revslider'); ?>';

	RVS.ENV.revision		= '<?php echo RS_REVISION; ?>';
	RVS.ENV.updated			= <?php echo (version_compare(RS_REVISION, $rs_show_updated, '>')) ? 'true' : 'false'; ?>;
	RVS.ENV.latest_version	= '<?php echo RevLoader::get_option('revslider-latest-version', RS_REVISION); ?>';	
...
<?php
RevLoader::do_action('revslider_header_content', $rsaf);
?>
...
if(!empty($rs_backend_fonts)){
	foreach($rs_backend_fonts as $rs_bf){
		if($rs_bf['type'] === 'custom' && isset($rs_bf['url']) && isset($rs_bf['backend']) && $rs_bf['backend'] === true){
			echo '<link href="'.RevLoader::esc_html($rs_bf['url']).'" rel="stylesheet" property="stylesheet" media="all" type="text/css" >'."\n";
		}
	}
}
?>
...
<div id="waitaminute" class="_TPRB_">
	<div class="waitaminute-message"><i class="eg-icon-emo-coffee"></i><br><?php RevLoader::_e('Please Wait...', 'revslider'); ?></div>
</div>
