						...
						if(!file_exists($file) || isset($temp['push_image'])){
							if($curl !== false){
								...
								do{
									...
									$image_data = RevLoader::wp_remote_post($url.'/'.$temp['img'], array(
								...
								$count = 0;
								do{
									$image_data = @file_get_contents($url.'/'.$temp['img']); // Get image data
									if($image_data == false){
										$rslb->move_server_list();
										$url = $rslb->get_url('templates', 0, true);
									}
								...
								if(!file_exists($file) || isset($reload[$key])){ //update, so load again
									if($curl !== false){
										...
										do{
											...
											$image_data = RevLoader::wp_remote_post($url.'/'.$tvalues['img'], array(
												'user-agent' => 'revphp; '. RevLoader::get_bloginfo('url'),
												'body'		 => array(
													'item'		=> urlencode(RS_PLUGIN_SLUG),
													'version'	=> urlencode(RS_REVISION)
										...
										$count = 0;
										do{
											$image_data = @file_get_contents($url.'/'.$tvalues['img']); // Get image data
											if($image_data == false){
												$rslb->move_server_list();
												$url = $rslb->get_url('templates', 0, true);
											}
