 ...
 */
 ...
 //http://localhost/wp-themes/revolution/wp-content/uploads/revslider/ken-burns-hero/hero2.jpg
 //slide bg image
 //localhost/wp-themes/revolution/wp-content/uploads/revslider/ken-burns-hero/deskimage.jpg
			...
			foreach($this->used_images as $file => $val){
				...
				if(strpos($file, 'http') !== false){
					...
					if(is_file($upload_dir.$checkpath)){
						if(!$this->usepcl){
							$return = $this->zip->addFile($upload_dir.$checkpath, $add_checkpath);
						}else{
					    ...
					    if (strpos($checkpath, 'img/cms') !== false) {
						$add_check_path = explode('/', $checkpath);
						$add_check_path_ext = end($add_check_path);
						$custom_path = _PS_ROOT_DIR_ . '/img/cms/' . $add_check_path_ext;
							if(is_file($custom_path)){
								$this->zip->addFile($custom_path, 'img/cms/'. $add_check_path_ext);
							}
						}
