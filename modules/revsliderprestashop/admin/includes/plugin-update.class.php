...
class RevSliderPluginUpdate extends RevSliderFunctions {
	...
	public function __construct(){
		$this->revision = RevLoader::get_option('revslider_update_version', '6.0.0');
	...
	public function get_version(){
		$real_version = RevLoader::get_option('revslider_update_version', 1.0);
	...
	public function set_version($set_to){
		...
		RevLoader::update_option('revslider_update_version', $set_to);
	...
	public function slider_need_update_checks(){
		$finished = RevLoader::get_option('revslider_update_revision_current', '1.0.0');
	...
	public function upgrade_next_slider(){
		...
		RevLoader::update_option('revslider_update_revision_current', $this->revision);
	...
	/**
	 * remove the settings from the table and use them from now on with get_option / RevLoader::update_option
	 * @since 5.0
	 */
	public function check_settings_table(){
		...
		if($wpdb->get_var("SHOW TABLES LIKE '" . $wpdb->prefix . RevSliderFront::TABLE_SETTINGS . "'") == $wpdb->prefix . RevSliderFront::TABLE_SETTINGS){
			...
			if(isset($result['general'])){
				RevLoader::update_option('revslider-global-settings', $result['general']);
			}
		}
	}
	...
	public function add_general_settings($sliders = false){
		...
		if(!empty($sliders) && is_array($sliders)){
			$fonts = RevLoader::get_option('tp-google-fonts', array());
	 ...
	 * @since: 6.0.0
	 **/
	public function migrate_layer_to_6_0($layer, $blank, $slide, $slider){
		$color_picker		= new RSColorpicker();
		...
		$ml['timeline'] = array(
			...
			'frames' => array(
				'frame_0' => $this->default_frame(
					array(
						'fid' => 'frame_0',
						'alias' => RevLoader::__('Anim From', 'revslider'),
				...
				'frame_1' => $this->default_frame(
					array(
						'fid' => 'frame_1',
						'alias' => RevLoader::__('Anim To', 'revslider'),
	...
	private function change_global_addon_settings_to_6_0(){
		...
		if(function_exists('is_plugin_active')){
			...
			if(is_plugin_active('revslider-maintenance-addon/revslider-maintenance-addon.php')){
				...
				$settings = RevLoader::get_option('revslider_maintenance_addon');
				if($settings !== false){
					$options = array();
					parse_str($settings, $options);
					if(isset($options['revslider-maintenance-addon-active']) && intval($options['revslider-maintenance-addon-active'])){
						RevLoader::update_option('revslider_maintenance_enabled', 1);
					}
				}
	...
	public function change_global_settings_to_6_2_0(){
		$global = maybe_unserialize(RevLoader::get_option('revslider-global-settings', '')); //get the old structure as serialized
		...
		if($_->get_param('wb_enable', false) !== false){
			$obj['revslider-whiteboard-addon'] = array(
				...
				'writehand' => array(
					...
					'source' => ($_->get_param('wb_writehand_source') === '1') ? RS_PLUGIN_URL .'addons/revslider-whiteboard-addon/'.'assets/images/write_right_angle.png' : $_->get_param('wb_writehand_source_custom', RS_PLUGIN_URL .'addons/revslider-whiteboard-addon/'.'assets/images/write_right_angle.png'),
				),
				'movehand' => array(
					...
					'source' => ($_->get_param('wb_movehand_source') === '1') ? RS_PLUGIN_URL .'addons/revslider-whiteboard-addon/'.'assets/images/hand_point_right.png' : $_->get_param('wb_movehand_source_custom', RS_PLUGIN_URL .'addons/revslider-whiteboard-addon/'.'assets/images/hand_point_right.png'),
				),
			);
		}
	...
	*/
	...
	public function default_frame($o = array(), $overwrite_with = array()){
		...
		$frame = array(
			'alias' => $this->get_val($o, 'alias', RevLoader::__('On Stage', 'revslider')),
			'filter' => array(
				'use' => $fuse,
				'grayscale' => $gs,
				'brightness' => $bs,
