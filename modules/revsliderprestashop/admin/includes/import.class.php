...
class RevSliderSliderImport extends RevSliderSlider {
	...
	public function import_slider($update_animation = true, $exact_filepath = false, $is_template = false, $single_slide = false, $update_navigation = true, $install = true){
		...
		try{
			...
			RevLoader::delete_files($this->remove_path);
		...
		RevLoader::do_action('revslider_slider_imported', $this->real_slider_id);
	...
	private function unzip_slider($exact_filepath = false){
		...
		if($exact_filepath !== false){
			...
			switch($error){
				...
				case UPLOAD_ERR_NO_FILE:
					$this->throw_error(RevLoader::__('No file sent.', 'revslider'));
				case UPLOAD_ERR_INI_SIZE:
				case UPLOAD_ERR_FORM_SIZE:
					$this->throw_error(RevLoader::__('Exceeded filesize limit.', 'revslider'));
				default:
				break;
			}
		...
		if(file_exists($path) == false)
			$this->throw_error(RevLoader::__('Import file not found', 'revslider'));
	...
	public function set_slider_data_raw(){
		global $wp_filesystem;
		$this->slider_raw_data = (file_exists($this->download_path.'slider_export.txt')) ? file_get_contents($this->download_path.'slider_export.txt') : '';
		if($this->slider_raw_data == ''){
			...
			if(!empty($dirs)){
				foreach($dirs as $dir){				
					if($dir !== '.' && $dir !== '..' && is_dir($this->download_path . $dir)){
						...
						$this->slider_raw_data = (file_exists($dir.'slider_export.txt')) ? file_get_contents($dir.'slider_export.txt') : '';
						if($this->slider_raw_data != '') {
			...
			if($this->slider_raw_data == '') $this->throw_error(RevLoader::__('slider_export.txt does not exist!', 'revslider'));
		}
	}
	...
	public function set_animations($update){
		...
		$animations		 = (file_exists($this->download_path.'custom_animations.txt')) ? file_get_contents($this->download_path.'custom_animations.txt') : '';
	...
	public function set_dynamic_css_v5(){
		...
		$dynamic	= (file_exists($this->download_path.'dynamic-captions.css')) ? file_get_contents($this->download_path.'dynamic-captions.css') : '';
	...
	public function set_dynamic_css_v6(){
		...
		$styles = (file_exists($this->download_path.'styles.txt')) ? file_get_contents($this->download_path.'styles.txt') : '';
	...
	public function set_navigations($update_navigation){
		...
		$navigations		= (file_exists($this->download_path.'navigation.txt')) ? file_get_contents($this->download_path.'navigation.txt') : '';
	...
	public function check_template(){
		...
		$uid_check = (file_exists($this->download_path.'info.cfg')) ? file_get_contents($this->download_path.'info.cfg') : '';
		
		if($this->is_template !== false){
			if($uid_check != $this->is_template){
				return array('success' => false, 'error' => RevLoader::__('Please select the correct zip file, checksum failed!', 'revslider'));
			}
	...
	public function process_slider_raw_data_pre_6(){
		...
		if(empty($this->slider_data)){
			...
			//$wp_filesystem->delete($this->remove_path, true);
			RevLoader::delete_files($this->remove_path);
			$this->throw_error(RevLoader::__('Wrong export slider file format! Please make sure that the uploaded file is either a zip file with a correct slider_export.txt in the root of it or an valid slider_export.txt file.', 'revslider'));
		}
	...
	public function process_slide_data_pre_6(){
		...
		foreach($this->slides_data as $slide_key => $slide){
					...
					if($this->import_zip === true){ //we have a zip, check if exists
						...
						if(isset($layer['type']) && $layer['type'] == 'svg'){
							if(isset($layer['svg']) && isset($layer['svg']->src)){
								//track replace /plugins/revslider/ to /
								$pos = strpos($layer['svg']->src, 'addon');
								if ($pos !== false) {
									$layer['svg']->src = RevLoader::content_url(). str_replace("plugins","addons", $layer['svg']->src);
								}else{
									$layer['svg']->src = RevLoader::content_url(). str_replace("/plugins/revslider/","/", $layer['svg']->src);
								}
							}
						}
					}
			...
			$d = RevLoader::apply_filters('revslider_importSliderFromPost_modify_data', $d, 'normal', $this->download_path);
				...
				foreach($layers as $layer_key => $layer){
					...
					if($this->import_zip === true){ //we have a zip, check if exists
						...
						if($layer_type == 'svg'){
							...
							if(!empty($svg)) $layer['svg']['source'] = RevLoader::content_url().$svg;
						}
					}
			...
			foreach($static_slide as $slide){
				...
				if(!empty($layers)){
					foreach($layers as $layer_key => $layer){
						...
						if(isset($layer['type']) && $layer['type'] == 'svg'){
							if(isset($layer['svg']) && isset($layer['svg']->src)){
								$layer['svg']->src = RevLoader::content_url().$layer['svg']->src;
							}
						}
				...
				$d = RevLoader::apply_filters('revslider_importSliderFromPost_modify_data', $d, 'static', $this->download_path);
		...
		if($this->is_template !== false){ //duplicate the slider now, as we just imported the "template"
			if($single_slide !== false){ //add now one Slide to the current Slider
				...
				if($changed){
					...
					//$wp_filesystem->delete($this->remove_path, true);
					RevLoader::delete_files($this->remove_path);
					return array('success' => false, 'error' => RevLoader::__('could not find correct Slide to copy, please try again.', 'revslider'), 'sliderID' => $this->slider_id);
				}
	...
	public function import_media_from_zip($image){
		...
		if($image !== '' && strpos($image, 'http') === false){
			if($this->import_zip === true){ //we have a zip, check if exists
				$exists = file_exists($this->download_path.'images/'.$image);
				if(!$exists){
					//echo '<p>'.esc_attr($image).RevLoader::__(' not found!', 'revslider').'</p>';
				}else{
