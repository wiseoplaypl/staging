...
class WPPost
{
    ...
    public static function getInstance(UId $uid, array &$postarr = [])
    {
        ...
        if (in_array($uid->id_type, [UId::REVISION, UId::TEMPLATE, UId::THEME])) {
            if (!$self->template_type = &$obj->type) {
                $self->template_type = (new $objectModel($uid->id))->type;
            }
        } elseif (UId::CONTENT === $uid->id_type || UId::CATEGORY_ADDITIONAL === $uid->id_type) {
        ...
        if (property_exists($obj, 'content')) {
            ...
            UId::CATEGORY_ADDITIONAL === $uid->id_type
                ? $self->post_content = &$obj->additional_description
...
function wp_update_post($postarr = [], $wp_error = false)
{
    ...
    try {
        $post->uid->id_shop && $post->_obj->id_shop_list = [$post->uid->id_shop];

        if ($post->_obj instanceof \Category) {
            // Fix: Category groups would lose after save
            $post->_obj->groupBox = $post->_obj->getGroups();
        } elseif ($post->_obj instanceof \CMSCategory) {
            // Fix: Save CMS Category description as richtext
            \Closure::bind(function () {
                $this->def['fields']['description']['type'] = \CMSCategory::TYPE_HTML;
            }, $post->_obj, $post->_obj)->__invoke();
        }
...
function wp_get_post_revisions($post_id, $args = null)
{
    ...
    $query = new \DbQuery();
    $query->select($fields = !empty($args['fields']) && 'ids' === $args['fields'] ? '`id_ce_revision`' : '*');
    $query->from('ce_revision');
    $query->where('`active` = 1 AND `parent` = ' . pSQL($parent));
    $query->orderBy('`date_upd` DESC');
    empty($args['posts_per_page']) || $query->limit($args['posts_per_page']);
