...
abstract class CoreXSettingsXBaseXManager
{
    ...
    protected function printEditorTemplateContent($name)
    {
        ?>
        <#
        const tabs = elementor.config.settings[<?php echo json_encode($name); ?>].tabs;

        if ( Object.values( tabs ).length > 1 ) { #>
            <div class="elementor-panel-navigation">
            <# _.each( tabs, function( tabTitle, tabSlug ) {
                $e.bc.ensureTab( 'panel/' + <?php echo json_encode($name); ?> + '-settings', tabSlug ); #>
        ...
        <# } #>
        <div id="elementor-panel-<?php escape($name); ?>-settings-controls"></div>
        <?php
    }
    ...
    private function getEditorTemplate()
    {
        ...
        ob_start(); ?>
        <script type="text/template" id="tmpl-elementor-panel-<?php escape($name); ?>-settings">
