...
abstract class CoreXFilesXCSSXBase extends BaseFile
{
    ...
    public function enqueue()
    {
        ...
        if (self::CSS_STATUS_INLINE === $meta['status']) {
            ...
            if ('elementor-post-' . \CreativeElements::getPreviewUId(false) === $handle) {
                ...
                $time = $meta['time'];
                !did_action('elementor/frontend/after_enqueue_styles') ? add_action('elementor/frontend/after_enqueue_styles', function () use ($handle, $time) {
                    wp_enqueue_style($handle, $this->getUrl(), $this->getEnqueueDependencies(), $time);
                }, 1) : wp_enqueue_style($handle, $this->getUrl(), $this->getEnqueueDependencies(), $time);
            }
        }
            ...
            if (ControlsManager::ICONS === $control['type']) {
                ...
                if ('classic' === $control['of_type']) {
                    ...
                    if (!empty($values[$prefix . '_image_tablet']['url'])) {
                        ...
                        $preload['media'] = '(min-width:' . (int) $min . 'px)';

                        $max = $min - 1;
                        $preload_tablet = [
                            'as' => 'image',
                            'href' => Helper::getMediaLink($values[$prefix . '_image_tablet']['url']),
                            'media' => '(max-width:' . (int) $max . 'px)',
                        ];
                        $this->preloads[] = &$preload_tablet;
                    }
                    if (!empty($values[$prefix . '_image_mobile']['url'])) {
                        $min = \Configuration::get('elementor_viewport_md');
                        isset($preload['media']) || $preload['media'] = '(min-width:' . (int) $min . 'px)';
                        isset($preload_tablet) && $preload_tablet['media'] .= ' and (min-width:' . (int) $min . 'px)';
