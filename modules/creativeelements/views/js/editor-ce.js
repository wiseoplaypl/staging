...
$(window).on('elementor:init', function onElementorInit() {
	...
	elementor.channels.editor.on('section:activated', function onSectionActivated(sectionName, editor) {
		var view = editor.getOption('editedElementView'),
			widgetType = view.model.get('widgetType');

		if ('flip-box' === widgetType) {
			// init flip box back
			var isSideB = ~['section_b', 'section_style_b', 'section_style_button'].indexOf(sectionName),
				$backLayer = view.$el.find('.elementor-flip-box-back');

			view.$el.toggleClass('elementor-flip-box--flipped', isSideB);
			...
			// init search results
			view.$el.find('.elementor-search__products').css({
			...
			view.$el.find('[role=alert]').attr('hidden', 'section_messages_style' !== sectionName ? '' : null);
		} else if ('login-form' === widgetType || 'register-form' === widgetType) {
			view.$el.find('[role=alert]').attr('hidden', 'section_messages_style' !== sectionName ? '' : null);
			view.$el.find('.elementor-field-type-email').toggleClass('elementor-error', 'section_messages_style' === sectionName)
		} else if ('password-form' === widgetType) {
			view.$el.find('[role=alert]').attr('hidden', 'section_messages_style' !== sectionName ? '' : null);
			'section_reset_content' === sectionName && view.$el.removeClass('ce-password--change');
			'section_change_content' === sectionName && view.$el.addClass('ce-password--change');
		} else if ('skip-links' === widgetType) {
			view.$el.find('.elementor-active').length || view.$el.find('a:first').addClass('elementor-active');
		}
	});

	elementor.channels.editor.on('change:editSettings', function onEditSettingsChange(model, view) {
		// Handle Skip Links
		if (model.changed.activeItemIndex && 'skip-links' === view.model.attributes.widgetType) {
			view.$el.find('a').eq(model.changed.activeItemIndex - 1).addClass('elementor-active').siblings().removeClass('elementor-active');
		}
	});
...
$(function onReady() {
	...
	elementor.channels.editor.on('ceThemeStyle:EditButton', function editButton(button) {
		var name = button.model.get('name');
		$e.route("panel/page-settings/advanced");
		$('.elementor-control-section_' + name).click();
	});
	...
	elementor.once('document:loaded', function onceDocumentLoaded() {
		...
		if ('kit' === doc.type) {
			const $html = $(elementor.$previewContents[0].documentElement);
			...
			elementor.$previewElementorEl.on('click', 'img', function onClickKitImage() {
				if (!$html.hasClass('elementor-editor-preview')) return;
				'kit_settings' === elementor.panel.currentView.currentPageName || $e.route("panel/global/style");
			...
			elementor.$previewElementorEl.on('click', '.elementor-button', function onClickKitButton() {
				if (!$html.hasClass('elementor-editor-preview')) return;
				const view = elementor.panel.currentView;
				'page_settings' === view.currentPageName && 'advanced' === view.currentPageView.activeTab || $e.route("panel/page-settings/advanced");

				$(this).closest('.elementor-button-primary').length && $('.elementor-control-section_primary_button:not(.elementor-open)').click() ||
				$(this).closest('.elementor-button-secondary').length && $('.elementor-control-section_secondary_button:not(.elementor-open)').click();
			...
			elementor.$previewElementorEl.on('click', '.btn, .btn-primary, .btn-secondary', function onClickKitBtn() {
				...
				$(this).hasClass('btn-primary') && $('.elementor-control-section_primary_btn:not(.elementor-open)').click() ||
				$(this).hasClass('btn-secondary') && $('.elementor-control-section_secondary_btn:not(.elementor-open)').click() ||
				$('.elementor-control-section_btn:not(.elementor-open)').click();
			});
...
$(document).on('heartbeat-error', function onHeartbeatError(e, xhr) {
	if (xhr.getResponseHeader('login') || xhr.responseText?.indexOf('submit_login') >= 0) {
		$.fancybox.isOpen || $.fancybox({
			type: 'iframe',
			href: baseAdminDir + (parseInt(_PS_VERSION_) < 9
				? 'index.php?controller=AdminLogin&redirect=AdminCEEditor'
				: '?controller=AdminCEEditor'
			),
			autoSize: false,
			width: 600,
			height: 800,
...
$('#elementor-panel').on('change.ls', '.ls-selector select', function onChangeSlider() {
	...
	null === title || $.post(ls.url, {
		...
		if (id) {
			var option = '#' + id + ' - ' + title;
			elementor.widgetsCache['ps-widget-LayerSlider'].controls.slider.options[id] = option;
			$('.ls-selector select')
