...
class ElementSection extends ElementBase
{
    ...
    protected function registerControls()
    {
        ...
        $this->startControlsTabs('tabs_tabs_style', ['separator' => 'before']);
    ...
    protected function contentTemplate()
    {
        ...
        <# if ( settings.background_video_link ) {
            ...
            view.addRenderAttribute( 'background-video-container', {
            ...
            <nav {{{ view.getRenderAttributeString( 'tabs' ) }}}>
                ...
                <# view.model.get('elements').models.forEach( function ( column, i ) { #>
                    <li class="menu-item menu-item-type-column" role="presentation"><#
                        var colSettings = column.get('settings'),
                            colId = colSettings.get( '_element_id' ) || 'ce-tabpanel-' + column.id; #>
                        <a class="elementor-item{{ i ? '' : ' elementor-item-active' }}" id="ce-tab-{{ column.id }}" href="#ce-action=select"
                            role="tab" aria-selected="{{ i ? 'false' : 'true' }}" aria-controls="{{ colId }}">
                            {{{ elementor.helpers.renderIcon( view, colSettings.get( 'tab_icon' ), { 'aria-hidden': 'true' }, 'i' ) }}}
    ...
    public function beforeRender()
    {
        ...
        if ($settings['tabs']) {
            ...
            if ('none' !== $settings['pointer']) {
                $animation_type = ModulesXThemeXWidgetsXNavMenu::getPointerAnimationType($settings['pointer']);
        ...
        <<?php Utils::printValidatedHtmlTag($this->getHtmlTag()); ?> <?php $this->printRenderAttributeString('_wrapper'); ?>>
        <?php if ('video' === $settings['background_background'] && $settings['background_video_link']) {
            $this->addRenderAttribute('background-video-container', [
                'class' => 'elementor-background-video-container',
                'aria-hidden' => 'true',
            ]);
            if (!$settings['background_play_on_mobile']) {
            ...
            <?php if (Embed::getVideoProperties($settings['background_video_link'])) { ?>
                <div class="elementor-background-video-embed"></div>
            <?php } else { ?>
                <video class="elementor-background-video-hosted elementor-html5-video" muted playsinline<?php echo $settings['background_play_once'] ? '' : ' loop', $settings['background_lazy_load'] ? ' preload="none"' : ' autoplay'; ?>></video>
            <?php } ?>
            </div>
        <?php } ?>
        ...
        empty($settings['shape_divider_bottom']) || $this->printShapeDivider('bottom'); ?>
            <div class="elementor-container elementor-column-gap-<?php escape($settings['gap']); ?>">
            <?php if ($settings['tabs']) {
                $first = Helper::getFirstTabIndex($this); ?>
                <nav <?php $this->printRenderAttributeString('tabs'); ?> aria-label="<?php esc_attr_e('Tabs'); ?>">
                    <ul class="elementor-nav ce-scrollbar-x--auto" role="tablist">
                <?php foreach ($this->getChildren() as $i => $column) { ?>
                    <?php if ($this->render_tabs[$i]) {
                        $column_id = $column->getSettings('_element_id') ?: 'ce-tabpanel-' . $column->getId(); ?>
                        <li class="menu-item menu-item-type-column" role="presentation">
                            <a class="elementor-item<?php $first === $i && print ' elementor-item-active'; ?>" id="ce-tab-<?php escape($column->getId()); ?>" href="#ce-action=select"
                                role="tab" aria-selected="<?php echo $first === $i ? 'true' : 'false'; ?>" aria-controls="<?php escape($column_id); ?>">
                                <?php IconsManager::renderIcon($column->getSettings('tab_icon'), ['aria-hidden' => 'true']); ?>
    ...
    public function afterRender()
    {
        ...
        </<?php Utils::printValidatedHtmlTag($this->getHtmlTag()); ?>>
        <?php
    }
    ...
    protected function addRenderAttributes()
    {
        ...
        if ('slideshow' === $this->getSettings('background_background')
            ...
            $this->addRenderAttribute('_wrapper', 'style', 'background-image: url' .
                '("' . Helper::getMediaLink($slideshow[0]['image']['url']) . '");');
            $this->addScriptDepends('swiper');
            $this->addStyleDepends('swiper');
        }
    }
    ...
    private function printShapeDivider($side)
    {
        ...
        <div class="elementor-shape elementor-shape-<?php escape($side); ?>" aria-hidden="true" data-negative="<?php echo $negative ? 'true' : 'false'; ?>">
