...
class ElementColumn extends ElementBase
{
    ...
    protected function contentTemplate()
    {
        ...
        <#
        setTimeout( () => {
            if ( !view._parent || !view._parent.model.get( 'settings' ).get( 'tabs' ) ) {
                return;
            }
            const elementID = settings._element_id || 'ce-tabpanel-' + id;
            view.renderCustomElementID = function () {
                const index = view.$el.attr( 'id', elementID ).index();
                view._parent.$el.find( '[role=tab]' ).eq( index ).attr( 'aria-controls', elementID );
            };
            view.$el.attr( {
                id: elementID,
                role: 'tabpanel',
                'aria-labelledby': 'ce-tab-' + id,
                'data-col': 100
            } );
    ...
    public function beforeRender()
    {
        ...
        if ($section && $section->getSettings('tabs')) {
            $this->removeRenderAttribute('_wrapper', 'class', 'elementor-col-' . $settings['_column_size']);
            $this->addRenderAttribute('_wrapper', 'class', 'elementor-col-100');
            ...
            $settings['_element_id']
                || $this->addRenderAttribute('_wrapper', 'id', 'ce-tabpanel-' . $this->getId());
            $this->addRenderAttribute('_wrapper', [
                'role' => 'tabpanel',
                'aria-labelledby' => 'ce-tab-' . $this->getId(),
            ]);
        }
    ...
    protected function addRenderAttributes()
    {
        ...
        $this->addRenderAttribute('_wrapper', 'class', [
            'elementor-column',
            'elementor-col-' . $this->getSettings('_column_size'),
