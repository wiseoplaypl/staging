...
class Frontend extends App
{
    ...
    public function init()
    {
        ...
        if ($this->_has_elementor_in_page) { // never happens
    ...
    public function registerStyles()
    {
        ...
        */
        wp_register_style(
            'e-transitions',
            $this->getCssAssetsUrl('transitions', 'views/css/conditionals/'),
            [],
            _CE_VERSION_
        );
        ...
        $version = $has_custom_breakpoints ? null : _CE_VERSION_;

        wp_register_style(
            'elementor-frontend',
            $this->getFrontendFileUrl("frontend$direction_suffix$min_suffix.css", $has_custom_breakpoints),
            [],
            $version
        );

        foreach (Plugin::$instance->widgets_manager->widgetsWithStyles() as $widget_name) {
            wp_register_style(
                "widget-$widget_name",
                _CE_ASSETS_URL_ . "css/widget-$widget_name$min_suffix.css",
                ['elementor-frontend'],
                _CE_VERSION_
            );
        }

        foreach (Plugin::$instance->widgets_manager->widgetsWithResponsiveStyles() as $widget_name) {
            wp_register_style(
                "widget-$widget_name",
                $this->getFrontendFileUrl("widget-$widget_name$min_suffix.css", $has_custom_breakpoints),
                ['elementor-frontend'],
                $version
            );
        }
    ...
    public function wpFooter()
    {
        if (!$this->_has_elementor_in_page) {
            // Fix: insert kit to all pages
            empty(Helper::$actions['elementor/frontend/after_enqueue_styles'][1]) || call_user_func(reset(Helper::$actions['elementor/frontend/after_enqueue_styles'][1]));
    ...
    protected function getInitSettings()
    {
        ...
        $settings = [
            'environmentMode' => [
                ...
                'isScriptDebug' => Utils::isScriptDebug(),
            ],
            'is_rtl' => is_rtl(),
            'breakpoints' => Responsive::getBreakpoints(),
            'responsive' => [
                'hasCustomBreakpoints' => Responsive::hasCustomBreakpoints() ? (int) $GLOBALS['context']->shop->id : false,
            ],
            'version' => _CE_VERSION_,
            'urls' => [
                'assets' => '//' . \Tools::getMediaServer($assets_path = _MODULE_DIR_ . 'creativeelements/views/') . $assets_path,
            ],
            'i18n' => [
                'close' => __('Close', 'Shop.Theme.Global'),
                'zoom' => __('Zoom'),
                'fullscreen' => __('Fullscreen'),
                'prevSlideMessage' => __('Previous', 'Shop.Theme.Global'),
                'nextSlideMessage' => __('Next', 'Shop.Theme.Global'),
                'paginationBulletMessage' => __('Go To') . ' {{index}}',
            ],
