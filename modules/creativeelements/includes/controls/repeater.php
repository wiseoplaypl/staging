...
class ControlRepeater extends BaseDataControl
{
    ...
    {
        if (!$settings || empty($control_data['fields'])) {
            return $settings;
        }
        ...
        foreach ($settings as &$item) {
            foreach ($control_data['fields'] as $field) {
                ...
                if (method_exists($control_obj, __FUNCTION__)) {
                    $item[$field['name']] = $control_obj->onImport($item[$field['name']], $field);
                }
            }
        }
     ...
     * @return array Control settings
     */
    public function onExport($settings, &$control_data)
    {
        if (!$settings || empty($control_data['fields'])) {
            return $settings;
        }
        ...
        foreach ($settings as &$item) {
            foreach ($control_data['fields'] as $field) {
                ...
                if (method_exists($control_obj, __FUNCTION__)) {
                    $item[$field['name']] = $control_obj->onExport($item[$field['name']], $field);
                }
            }
        }
    ...
    public function contentTemplate()
    {
        ...
        <div class="elementor-control-repeater elementor-control-tag-area">
            <label class="elementor-control-dynamic-switcher-wrapper">
                <span class="elementor-control-title">{{{ data.label }}}</span>
            </label>
        ...
        <# } else if ( itemActions.add ) { #>
            <div class="elementor-button-wrapper">
                <button class="elementor-button elementor-button-default elementor-repeater-add" type="button">
                    <i class="eicon-plus" aria-hidden="true"></i><?php _e('Add Item'); ?>
                </button>
            </div>
        <# } #>
        </div>
