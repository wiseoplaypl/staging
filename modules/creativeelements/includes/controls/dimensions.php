                ...
                <ul class="elementor-control-dimensions">
                    <?php foreach ($dimensions as $dimension_key => $dimension_title) { ?>
                        ...
                        <li class="elementor-control-dimension">
                            <input id="<?php escape($control_uid); ?>" type="number" data-setting="<?php escape($dimension_key); ?>" placeholder="<#
                                if ( _.isObject( data.placeholder ) ) {
                                    if ( ! _.isUndefined( data.placeholder[<?php echo json_encode($dimension_key); ?>] ) ) {
                                        print( data.placeholder[<?php echo json_encode($dimension_key); ?>] );
                                    }
                                } else {
                                    print( data.placeholder );
                                } #>"
                                <# if ( -1 === _.indexOf( allowed_dimensions, <?php echo json_encode($dimension_key); ?> ) ) { #>disabled<# } #>
                            />
                            <label for="<?php escape($control_uid); ?>" class="elementor-control-dimension-label"><?php echo $dimension_title; ?></label>
                        </li>
                    <?php } ?>
                    <li>
