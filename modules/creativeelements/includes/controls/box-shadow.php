    ...
    public function contentTemplate()
    {
        ...
        <?php foreach ($this->getSliders() as $slider_name => $slider) { ?>
            ...
            <div class="elementor-shadow-slider elementor-control-type-slider">
                <label for="<?php escape($control_uid); ?>" class="elementor-control-title">
                    <?php echo $slider['label']; ?>
                </label>
                <div class="elementor-control-input-wrapper">
                    <div class="elementor-slider" data-input="<?php escape($slider_name); ?>"></div>
                    <div class="elementor-slider-input elementor-control-unit-2">
                        <input id="<?php escape($control_uid); ?>" data-setting="<?php escape($slider_name); ?>" type="number" min="<?php escape($slider['min']); ?>" max="<?php escape($slider['max']); ?>">
                    </div>
                </div>
            </div>
        <?php } ?>
