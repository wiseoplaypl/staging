...
<?php if ((count($langs = \Language::getLanguages(true, false)) > 1 || \Shop::isFeatureActive()) && ($uid = get_the_ID()) && $uid->id_type !== UId::TEMPLATE) { ?>
    <div id="elementor-panel-footer-lang" class="elementor-panel-footer-tool elementor-toggle-state">
        ...
        <?php if (\Shop::isFeatureActive() && count($shops = \Shop::getShops()) > 1) {
            ...
            <form class="elementor-panel-footer-sub-menu" id="ce-context-wrapper" name="context" method="post">
                <select name="setShopContext" id="ce-context">
                    <option value=""><?php _e('All shops', 'Admin.Global') . (!$active_group ? $star : ''); ?></option>
                    <?php foreach ($group_ids as $id_group) { ?>
                        <?php $active = !$active_shop && $id_group == $active_group ? $star : ''; ?>
                        <option value="g-<?php echo (int) $id_group; ?>"<?php $active && print ' selected'; ?>><?php echo $tab1 . $groups[$id_group] . $active; ?></option>
                        <?php foreach ($shop_ids as $id_shop) { ?>
                            <?php if ($shops[$id_shop]['id_shop_group'] == $id_group) { ?>
                                <?php $active = $id_shop == $active_shop ? $star : ''; ?>
                                <option value="s-<?php echo (int) $id_shop; ?>"<?php $active && print ' selected'; ?>><?php echo $tab2 . $shops[$id_shop]['name'] . $active; ?></option>
                            <?php } ?>
                        <?php } ?>
                    <?php } ?>
                </select>
        ...
        <?php } ?>
            <div class="elementor-panel-footer-sub-menu" id="ce-langs" data-lang="<?php echo (int) $uid->id_lang; ?>" data-built='<?php echo json_encode(UId::getBuiltList($uid->id, $uid->id_type)); ?>'>
                <?php foreach ($langs as &$lang) { ?>
                    <div class="elementor-panel-footer-sub-menu-item ce-lang" data-lang="<?php echo (int) $lang['id_lang']; ?>" data-shops='<?php echo json_encode(array_keys($lang['shops'])); ?>'>
    ...
    <?php foreach ($scheme_fields as $option_name => $option) { ?>
        <div class="elementor-panel-scheme-typography-item elementor-control elementor-control-type-select">
            ...
            <?php if ('select' === $option['type']) { ?>
                <select name="<?php escape($option_name); ?>" class="elementor-panel-scheme-typography-item-field">
                <?php foreach ($option['options'] as $field_key => $field_value) { ?>
                    <option value="<?php escape($field_key); ?>"><?php echo $field_value; ?></option>
                <?php } ?>
                </select>
            <?php } elseif ('font' === $option['type']) { ?>
                ...
                <?php foreach (Fonts::getFontGroups() as $group_type => $group_label) { ?>
                    <optgroup label="<?php escape($group_label); ?>">
                    <?php foreach (Fonts::getFontsByGroups([$group_type]) as $font_title => $font_type) { ?>
                        <option value="<?php escape($font_title); ?>"><?php echo $font_title; ?></option>
                    <?php } ?>
                    </optgroup>
                <?php } ?>
                </select>
            <?php } elseif ('text' === $option['type']) { ?>
                <input name="<?php escape($option_name); ?>" class="elementor-panel-scheme-typography-item-field">
            <?php } ?>
            </div>
        </div>
    <?php } ?>
...
<script type="text/template" id="tmpl-elementor-control-dynamic-switcher">
    <div class="elementor-control-dynamic-switcher elementor-control-unit-1" data-tooltip="<?php esc_attr_e('Dynamic Tags'); ?>">
