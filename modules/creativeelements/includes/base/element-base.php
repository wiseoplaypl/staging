    ...
    public function getRenderData()
    {
        ...
        if (!empty($settings['__dynamic__'])) {
            ...
            foreach ($settings['__dynamic__'] as $name => &$value) {
                ...
                $dynamic = array_merge($controls[$name]['dynamic'], Plugin::$instance->controls_manager->getControl($controls[$name]['type'])->getSettings('dynamic') ?: []);

                if (!empty($dynamic['property'])) {
                    unset($settings[$name][$dynamic['property']]);
                } else {
                    unset($settings[$name]);
                }
            }
