...
class WidgetsManager
{
    ...
    {
        // Set translator to preview language
        $uid = get_the_ID();
        $uid->id_lang && Helper::$translator = $GLOBALS['context']->getTranslatorFromLocale((new \Language($uid->id_lang))->locale);

        $documents = Plugin::$instance->documents;
        $documents->switchToDocument($documents->get($uid));
     ...
     */
    public function widgetsWithStyles()
    {
        return [
            'counter',
            'divider',
     ...
     */
    public function widgetsWithResponsiveStyles()
    {
        return [
            'accordion',
            'alert',
