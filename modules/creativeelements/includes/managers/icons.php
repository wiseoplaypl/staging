...
class IconsManager
{
    ...
    private static function renderSvgIcon($value, array $attributes = [])
    {
        if (empty($value['url'])) {
            return '';
        }

        $svg = SvgHandler::getInlineSvg($value['url']);

        if ($attributes && $svg) {
            $svg = preg_replace('/(<svg\b)([^>]*>)/i', '$1 ' . Utils::renderHtmlAttributes($attributes) . '$2', $svg, 1);
        }
    ...
    public static function renderIcon($icon, $attributes = [], $tag = 'i')
    {
        ...
        // handler SVG Icon
        if ('svg' === $icon['library']) {
            $output = self::renderSvgIcon($icon['value'], $attributes);
        } else {
    ...
    public static function getBcIcon(array &$settings, $bc_name, $attributes = [], $tag = 'i')
    {
        ...
        return 'svg' === $icon['library']
            ? self::renderSvgIcon($icon['value'], $attributes)
