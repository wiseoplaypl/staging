...
class WidgetImageGallery extends WidgetBase
{
    ...
    {
        return ['widget-image-gallery', 'e-transitions'];
    }
    ...
    protected function registerControls()
    {
        ...
        $this->addResponsiveControl(
            ...
            [
                ...
                'size_units' => ['px', 'em'],
                'selectors' => [
                    '{{WRAPPER}}.ce-image-gallery--layout-grid .ce-image-gallery' => 'gap: {{SIZE}}{{UNIT}}',
        ...
        $this->addControl(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .ce-gallery-icon, {{WRAPPER}} .ce-gallery-icon img' => 'transition: {{SIZE}}ms;',
                ],
                'render_type' => 'template',
            ]
        );
    ...
    protected function render()
    {
        ...
        <div class="ce-image-gallery ce-scrollbar-x--auto">
        <?php foreach ($gallery as $i => &$item) { ?>
            <?php if (!empty($item['image']['url'])) {
                $style = [];
                empty($item['cols']) || $style[] = '--ce-col-span: ' . (int) $item['cols'] . ';';
                empty($item['rows']) || $style[] = '--ce-row-span: ' . (int) $item['rows'] . ';';
                empty($item['cols_tablet']) || $style[] = '--ce-col-span-tablet: ' . (int) $item['cols_tablet'] . ';';
                empty($item['rows_tablet']) || $style[] = '--ce-row-span-tablet: ' . (int) $item['rows_tablet'] . ';';
                empty($item['cols_mobile']) || $style[] = '--ce-col-span-mobile: ' . (int) $item['cols_mobile'] . ';';
                empty($item['rows_mobile']) || $style[] = '--ce-row-span-mobile: ' . (int) $item['rows_mobile'] . ';';
                ($link = $this->getLinkUrl($item, $settings['gallery_link'])) && $this->addLinkAttributes($link_i = "link_$i", $link);
                $caption = $link && '' !== $item['caption'] ? "<a {$this->getRenderAttributeString('link')} {$this->getRenderAttributeString($link_i)} tabindex=\"-1\">$item[caption]</a>" : $item['caption'];
                ?>
                <figure class="ce-gallery-item<?php escape($animated_class); ?>"<?php $style && print Utils::renderHtmlAttributes([' style' => $style]); ?>>
                    <div class="ce-gallery-icon<?php escape($image_animated_class); ?>">
                    ...
                    <?php if (!empty($settings['overlay'])) { ?>
                        <div class="ce-gallery-overlay elementor-animated-item--<?php escape($overlay_animation); ?>"></div>
                    <?php } ?>
                    </div>
                    <figcaption class="ce-gallery-caption elementor-animated-item--<?php escape($caption_animation); ?>"
                        <?php $item['description'] && print 'data-description="' . esc_attr($item['description']) . '"'; ?>><?php echo $caption; ?></figcaption>
                </figure>
            <?php } ?>
        <?php } ?>
        </div>
    ...
    protected function contentTemplate()
    {
        ...
        <# gallery.length && gallery.forEach(function (item) { #>
            <# if (item.image && item.image.url) {
                var style = [];
                item.cols > 1 && style.push('--ce-col-span: ' + parseInt(item.cols));
                item.rows > 1 && style.push('--ce-row-span: ' + parseInt(item.rows));
                item.cols_tablet > 1 && style.push('--ce-col-span-tablet: ' + parseInt(item.cols_tablet));
                item.rows_tablet > 1 && style.push('--ce-row-span-tablet: ' + parseInt(item.rows_tablet));
                item.cols_mobile > 1 && style.push('--ce-col-span-mobile: ' + parseInt(item.cols_mobile));
                item.rows_mobile > 1 && style.push('--ce-row-span-mobile: ' + parseInt(item.rows_mobile));
                #>
                <figure class="ce-gallery-item {{ animatedClass }}" style{{{ style.length ? '="' + style.join('; ') + '"' : '' }}}>
                    ...
                    <figcaption class="ce-gallery-caption elementor-animated-item--{{ settings.caption_animation }}"
                        ...
                        if (link) {
                            #><a {{{ view.getRenderAttributeString('link') }}} href="{{ link.url }}" tabindex="-1"><#
                        }
                        print(item.caption);
                        if (link) {
                            #></a><#
