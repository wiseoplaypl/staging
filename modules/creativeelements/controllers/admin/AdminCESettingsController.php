...
class AdminCESettingsController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->fields_options['general'] = [
            'tab' => $this->l('General'),
            'class' => 'general' === $this->sub_tab ? '' : 'hidden',
            'icon' => 'icon-cog',
            ...
            'fields' => [
                ...
                'elementor_dashboard_widget' => [
                    'title' => $this->l('Show Dashboard Widget'),
                    'type' => 'bool',
                    'defaultValue' => self::zeroSafeDefault('elementor_dashboard_widget', 1),
                    'validation' => 'isBool',
                    'cast' => 'intval',
                    'desc' => $this->l('Dashboard widget provides quick access to recently edited content, news and updates for the module.'),
                ],
                ...
                'elementor_max_revisions' => [
                    ...
                    'cast' => 'intval',
                    'form_group_class' => 'list-group-item-text',
                    'desc' => $this->l('Sets the maximum number of revisions per content.'),
                ],
                'elementor_clear_revisions' => [
                    'no_multishop_checkbox' => true,
                    'title' => '',
                    'type' => 'button',
                    'class' => 'btn-default',
                    'icon' => 'icon-trash',
                    'label' => $this->l('Clear Revisions'),
                    'js' => '//' . $this->trans('Are you sure?', [], 'Admin.Notifications.Warning'),
                    'desc' => $this->l('Delete all revisions above the limit.'),
                ],
            ...
            'buttons' => [
                [
                    ...
                    'class' => $btn_class = (int) _PS_VERSION_ < 8 ? 'btn btn-default pull-right' : 'btn btn-primary pull-right',
                ],
            ],
        ];
        ...
        $this->fields_options['style'] = [
            'tab' => $this->l('Style'),
            'class' => 'style' === $this->sub_tab ? '' : 'hidden',
            'icon' => 'icon-adjust',
        ...
        $this->fields_options['advanced'] = [
            ...
            'class' => 'advanced' === $this->sub_tab ? '' : 'hidden',
            'icon' => 'icon-cogs',
        ...
        $this->fields_options['experiments'] = [
            ...
            'class' => 'experiments' === $this->sub_tab ? '' : 'hidden',
            'icon' => 'icon-flask',
    ...
    public function initModal()
    {
        ...
        $this->modals[] = [
            ...
            'modal_content' => CESmarty::sprintf(
                ...
                $this->l('https://old-url.com'),
                $this->l('https://new-url.com'),
    ...
    public function initPageHeaderToolbar()
    {
        ...
        $this->page_header_toolbar_btn['replace-url'] = [
            ...
            'js' => Shop::getContextShopID()
                ? "$('#modal_replace_url').modal()"
                : "alert(`{$this->trans('Choose the source shop', [], 'Admin.Advparameters.Feature')}`)",
        ];
    ...
    public function initContent()
    {
        ...
        if ((int) _PS_VERSION_ > 8) {
            ...
            $this->initHeader();
            // Shop context fix
            $this->context->smarty->tpl_vars['page_header_toolbar_btn']->value['replace-url']['js'] = Shop::getContextShopID()
                ? "$('#modal_replace_url').modal()"
                : "alert(`{$this->trans('Choose the source shop', [], 'Admin.Advparameters.Feature')}`)";
    ...
    public function ajaxProcessClearRevisions()
    {
        try {
            $limit = (int) Configuration::get('elementor_max_revisions');
            $count = CERevision::reduceRevisions($limit);

            CE\wp_send_json_success(sprintf(CE\__('%d Revisions Deleted'), $count));
        } catch (Exception $ex) {
            CE\wp_send_json_error(CE\__('An error occurred'));
        }
    }
