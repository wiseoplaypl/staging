...
{
    const HELP_URL = 'https://docs.webshopworks.com/creative-elements/87-widgets/premium-widgets/335-creative-slider-widget';
    ...
    protected function getSliderOptions()
    {
        ...
        try {
            $sliders = \Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                'SELECT `id`, `name` FROM ' . _DB_PREFIX_ . 'layerslider WHERE `flag_hidden` = 0 AND `flag_deleted` = 0 LIMIT 200'
            );
        } catch (\Exception $ex) {
            $sliders = [];
        }
    ...
    protected function patchInitScript(&$slider, $id)
    {
        ...
        $slider .= "<script>window['eval']($('#layerslider_$id$suffix').prev().html()?.match(/{([^]*)}/)?.pop())</script>";
    }
    ...
    protected function getPromo()
    {
        ...
        <a href="<?php escape($demo); ?>" target="_blank" id="ls-btn-demo" class="elementor-button elementor-button-default"><?php _e('Live Demo'); ?></a
        ><a href="<?php escape($more); ?>" target="_blank" id="ls-btn-more" class="elementor-button elementor-button-default"><?php _e('Read More'); ?></a>
        <?php
        return ob_get_clean();
    }
