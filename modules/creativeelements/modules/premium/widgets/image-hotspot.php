...
{
    const HELP_URL = 'https://docs.webshopworks.com/creative-elements/87-widgets/premium-widgets/329-image-hotspot-widget';
    ...
    protected function registerControls()
    {
        ...
        $this->addResponsiveControl(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .elementor-image-hotspot-content' => 'width: {{SIZE}}{{UNIT}}',
                ],
            ]
        );
        ...
        $this->addControl(
            ...
            [
                ...
                'size_units' => ['px', '%'],
                'selectors' => [
                    '{{WRAPPER}} .elementor-image-hotspot-content' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
    ...
    protected function render()
    {
        ...
        $this->addRenderAttribute('title', 'class', 'elementor-image-hotspot-title');
        ...
        <?php foreach ($settings['hotspots'] as $i => $item) {
            ...
            if (!empty($item['link']['url'])) {
                ...
                $icon_tag = 'div';
                $this->addRenderAttribute("icon-$i", 'tabindex', 0);
            }

            if ('' !== $item['title']) {
                $this->setRenderAttribute('title', 'id', $title_id = "elementor-image-hotspot__title-$id-$i");
                $this->addRenderAttribute("icon-$i", 'aria-labelledby', $title_id);
            } ?>
            <div class="elementor-image-hotspot-wrapper elementor-repeater-item-<?php escape($item['_id']); ?>">
                <<?php Utils::printValidatedHtmlTag($icon_tag); ?> <?php $this->printRenderAttributeString("icon-$i"); ?>><?php echo $icon; ?></<?php Utils::printValidatedHtmlTag($icon_tag); ?>>
                <div class="elementor-image-hotspot-content" role="tooltip">
                    <?php '' !== $item['title'] && print "<$title_tag {$this->getRenderAttributeString('title')}>$item[title]</$title_tag>"; ?>
    ...
    protected function contentTemplate()
    {
        ...
        var icon = elementor.helpers.getBcIcon( view, settings, 'icon' ),
            iconClass = 'elementor-icon' + ( settings.icon_animation ? ' elementor-animation-' + settings.icon_animation : '' ),
            titleClass = 'elementor-image-hotspot-title' + ( settings.title_display ? ' ce-display-' + settings.title_display : '' );
        #>
        <div class="elementor-image-hotspot">
            <img class="elementor-image" src="{{ elementor.helpers.getMediaLink( settings.image.url ) }}">
        <#  _.each( settings.hotspots, function( item, i ) { #>
            <div class="elementor-image-hotspot-wrapper elementor-repeater-item-{{{item._id}}}">
                <# var iconTag = item.link.url ? 'a' : 'div'; #>
                <{{{ iconTag }}} class="{{ iconClass }}" tabindex="0" aria-labelledby="elementor-image-hotspot__title-{{ `${id}-${i}` }}"
                    <# if (item.link.url) { #>href="{{ item.link.url }}"<# } #>>
                    {{{ icon }}}
                </{{{ iconTag }}}>
                <div class="elementor-image-hotspot-content" role="tooltip">
                <# if (item.title) { #>
                    <{{{ elementor.helpers.validateHTMLTag(settings.title_size) }}} class="{{ titleClass }}" id="elementor-image-hotspot__title-{{ `${id}-${i}` }}">
                        {{{item.title}}}
