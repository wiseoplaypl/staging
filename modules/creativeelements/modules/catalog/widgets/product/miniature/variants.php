...
class ModulesXCatalogXWidgetsXProductXMiniatureXVariants extends ProductVariants
{
    ...
    protected function render()
    {
        ...
        ?>
        <div class="ce-product-variants ce-product-variants__patterns" role="list" aria-label="<?php esc_attr_e('Product Variations'); ?>">
        <?php for ($i = 0; $i < $count && $i < $limit; ++$i) {
            $variant = $product['main_variants'][$i]; ?>
            <a class="ce-product-variants__pattern ce-product-variants__<?php echo $variant['texture'] ? 'texture' : 'color'; ?>" <?php echo Utils::renderHtmlAttributes([
                'href' => $variant['url'],
                'title' => $variant['name'],
                'role' => 'listitem',
                'style' => $variant['texture'] ? 'background-image:url' . "($variant[texture])" : 'background-color:' . $variant['html_color_code'],
            ]); ?>><span class="screen-reader-text"><?php echo $variant['name']; ?></span></a>
        <?php } ?>
        <?php if ($count > $limit) { ?>
            <span class="ce-product-variants__pattern ce-product-variants__count">+<?php echo $count - $limit; ?></span>
        <?php } ?>
        </div>
        <?php
    }
    ...
    protected function renderSmarty()
    {
        ...
        ?>
        <div class="ce-product-variants ce-product-variants__patterns" role="list" aria-label="{CE\__('Product Variations')}">
        {foreach $product.main_variants as $i => $variant}
            {if $i < <?php echo (int) $limit; ?>}
                <a class="ce-product-variants__pattern ce-product-variants__{if $variant.texture}texture{else}color{/if}" <?php echo Utils::renderHtmlAttributes([
                    'href' => '{$variant.url}',
                    'title' => '{$variant.name}',
                    'role' => 'listitem',
                    'style' => 'background-{if $variant.texture}image:url' .
                        '({$variant.texture}){else}color:{$variant.html_color_code}{/if}',
                ]); ?>><span class="screen-reader-text">{$variant.name}</span></a>
            {else}
                <span class="ce-product-variants__pattern ce-product-variants__count">+{$product.main_variants|count - <?php echo (int) $limit; ?>}</span>
                {break}
            {/if}
        {/foreach}
        </div>
