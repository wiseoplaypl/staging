...
class ModulesXCatalogXWidgetsXProductXMiniatureXAddToCart extends ProductAddToCart
{
    ...
    protected function registerControls()
    {
        ...
        $this->addControl(
            ...
            [
                ...
                'type' => ControlsManager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} a.elementor-button-combinations:not(#e):hover, {{WRAPPER}} a.elementor-button-combinations:not(#e):focus' => 'color: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->addControl(
            ...
            [
                ...
                'type' => ControlsManager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} a.elementor-button-combinations:hover, {{WRAPPER}} a.elementor-button-combinations:focus' => 'background-color: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->addControl(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} a.elementor-button-combinations:hover, {{WRAPPER}} a.elementor-button-combinations:focus' => 'border-color: {{VALUE}};',
                ],
            ]
        );
        ...
        if ($settings['combinations_action']) {
            ...
            $this->addRenderAttribute('button', [
                ...
                '{if $product.attributes && $product.quantity_all_versions}' .
                    'href="' . ('quickview' === $settings['combinations_action'] ? '#ce-action=quickview' : '{$product.url}') . '"' .
                '{else}' .
                    'href="#ce-action=' . esc_attr($action) . '{if $product.minimal_quantity > 1}&amp;qty={$product.minimal_quantity}{/if}"' .
                    '{if !$product.add_to_cart_url} aria-disabled="true"{/if}' .
                '{/if}' => [],
            ]);
            ...
            $this->setSettings(
                'text',
                '{if $product.attributes && $product.quantity_all_versions}' .
                    ($settings['combinations_text'] ?: ('quickview' === $settings['combinations_action'] ? '{CE\__("Quick View")}' : '{CE\__("View Full Details")}')) .
                '{else}' .
                    ($settings['text'] ?: ('add-to-cart' === $settings['button_type'] ? '{CE\__("Add to Cart")}' : '{CE\__("Buy Now")}')) .
                '{/if}'
            );
        } else {
            $this->addRenderAttribute('button', [
            ...
            '' === $settings['text'] && $this->setSettings(
                'text',
                'add-to-cart' === $settings['button_type'] ? '{CE\__("Add to Cart")}' : '{CE\__("Buy Now")}'
            );
        }
        WidgetButton::render();
        echo '{/if}';
    ...
    {
        ?>
        <span class="elementor-button-icon elementor-align-icon-<?php escape($this->getSettings('icon_align')); ?>">
