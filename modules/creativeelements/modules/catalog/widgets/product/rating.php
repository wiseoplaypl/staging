...
{
    const HELP_URL = 'https://docs.webshopworks.com/creative-elements/89-widgets/product-widgets/370-product-rating-widget';
    ...
    {
        return ['widget-star-rating', 'widget-product'];
    }
    ...
    protected function registerControls()
    {
        ...
        $this->addControl(
            ...
            [
                ...
                'placeholder' => Helper::$translator->trans('Read user reviews', [], 'Modules.Productcomments.Shop'),
                'condition' => [
                    'show_comments_number!' => '',
                ],
            ]
        ...
        $this->addPostCommentStyleControls();
    }
    ...
    protected function addPostCommentControls()
    {
        ...
        _CE_ADMIN_ && !\Configuration::get('PRODUCT_COMMENTS_ALLOW_GUESTS') && $this->addControl(
            'post_comment_description',
            [
                'raw' => __('You need to be [1]logged in[/1] or [2]create an account[/2] to post your review.', 'Modules.Productcomments.Shop', [
                    '[1]' => '<strong>',
                    '[/1]' => '</strong>',
                    '[2]' => '<strong>',
                    '[/2]' => '</strong>',
        ...
        $this->addControl(
            ...
            [
                'label' => __('Text'),
                'type' => ControlsManager::TEXT,
                'placeholder' => Helper::$translator->trans('Write your review', [], 'Modules.Productcomments.Shop'),
                'condition' => [
                    'show_post_comment!' => '',
                ],
            ]
    ...
    protected function addPostCommentStyleControls()
    {
        ...
        $this->addGroupControl(
            GroupControlBoxShadow::getType(),
            [
                'name' => 'post_comment_box_shadow_hover',
    ...
    protected function render()
    {
        ...
        if (empty($productcomments->active) || $productcomments->author !== 'PrestaShop') {
            return print '<div class="alert alert-warning" role="alert">' . sprintf(__('%s module (%s) must be installed!'), __('Product Comments'), 'productcomments') . '</div>';
        }
        ...
        $this->rating = \Tools::ps_round($vars['average_grade'], 1);
        $has_link = $this->getName() === 'product-rating' && \Tools::getValue('action') !== 'quickview';
        $settings = $this->getSettingsForDisplay();
        ?>
        <div class="ce-product-rating">
            <?php parent::render(); ?>
        <?php if ($this->rating && $settings['show_average_grade']) { ?>
            <span class="ce-product-rating__average-grade"><?php echo $this->rating; ?></span>
        <?php } ?>
        ...
        <?php if ($vars['post_allowed'] && !empty($settings['show_post_comment'])) { ?>
            <a class="elementor-button elementor-button--post-comment elementor-size-sm" href="#product-comments-list-header">
                ...
                <?php if ($pc_icon = IconsManager::getBcIcon($settings, 'post_comment_icon')) { ?>
                    <span class="elementor-button-icon elementor-align-icon-left" aria-hidden="true"><?php echo $pc_icon; ?></span>
                <?php } ?>
