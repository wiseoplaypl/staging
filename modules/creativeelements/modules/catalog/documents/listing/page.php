...
class ModulesXCatalogXDocumentsXListingXPage extends ThemePageDocument
{
    ...
    protected function getPermalinkUrl($id_lang, $id_shop, array $args, $relative = true)
    {
        ...
        switch ($preview) {
            ...
            case 'supplier':
                ...
                $url = Helper::$link->getSupplierLink($id_supplier, null, $id_lang, $id_shop, $relative);
                break;
            case 'search':
    ...
    protected function renderBlocks(&$elements_data)
    {
        ...
        foreach ($elements_data as &$element_data) {
            if (isset($element_data['widgetType']) && strpos($widget_type = $element_data['widgetType'], 'listing') === 0) {
                ...
                } elseif ('listing-description' === $widget_type && !empty($element_data['settings']['show_first'])) {
    ...
    protected function registerPreviewSettings()
    {
        ...
        if (!$display_suppliers = \Configuration::get('PS_DISPLAY_SUPPLIERS')) {
            unset($preview_options['supplier']);
        }
        ...
        $this->addControl(
            ...
            [
                'show_label' => false,
                'label_block' => true,
                'type' => $display_manufacturers ? SelectManufacturer::CONTROL_TYPE : ControlsManager::HIDDEN,
                'select2options' => [
                    'allowClear' => false,
                ],
                'options' => [
        ...
        $this->addControl(
            ...
            [
                'show_label' => false,
                'label_block' => true,
                'type' => $display_suppliers ? SelectSupplier::CONTROL_TYPE : ControlsManager::HIDDEN,
                'select2options' => [
                    'allowClear' => false,
                ],
                'options' => [
