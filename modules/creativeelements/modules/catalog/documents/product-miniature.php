...
class ModulesXCatalogXDocumentsXProductMiniature extends ProductDocument
{
    ...
    {
        Plugin::$instance->frontend->registerStyles();

        $wrapper_tag = $this->getSettings('content_wrapper_html_tag');
        $container = $this->getContainerAttributes();

        if ($css_classes = $this->getSettings('css_classes')) {
            $container['class'] .= " $css_classes";
        } else {
            $container['class'] .= '{if !empty($productClasses)} {$productClasses|escape}{/if}';
        }
        $uid = $this->getMainId();
        $article = [
        ...
        <<?php Utils::printValidatedHtmlTag($wrapper_tag); ?> <?php echo Utils::renderHtmlAttributes($container); ?>>
        ...
        <?php
        foreach (Helper::$head_styles as $handle => &$style) {
            if ('elementor-frontend' !== $handle) {
                echo '{CE\wp_enqueue_style(' . var_export($handle, true) . ")}\n";
            }
        }
    }
