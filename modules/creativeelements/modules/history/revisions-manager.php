    ...
    public static function getRevisions($post_id = 0, $query_args = [], $parse_result = true)
    {
        ...
        foreach ($posts as $revision) {
            ...
            if ($revision->ID === $post->ID) {
                $type = 'current';
                // Tweak: Display revision author instead of original author
                $revision->post_author = $current->post_author;
                // Fix for CMS pages
                $revision->post_modified || $revision->post_modified = $current->post_modified;
