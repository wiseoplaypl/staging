...
class ModulesXThemeXWidgetsXLanguageSelector extends WidgetBase
{
    ...
    const HELP_URL = 'https://docs.webshopworks.com/creative-elements/88-widgets/site-widgets/362-language-selector-widget';
    ...
    protected function render()
    {
        ...
        <nav <?php $this->printRenderAttributeString('main-menu'); ?> aria-label="<?php esc_attr_e('Language Selector'); ?>"><?php $this->selectorList($menu, 0, $ul_class); ?></nav>
        <?php
    }
    ...
    protected function selectorList(array &$nodes, $depth = 0, $ul_class = '')
    {
        ...
        <?php foreach ($nodes as &$node) { ?>
            <li class="menu-item menu-item-type-lang menu-item-lang-<?php echo (int) $node['id_lang'], $node['current'] ? ' current-menu-item' : '', !empty($node['children']) ? ' menu-item-has-children' : ''; ?>">
                ...
                <?php if ($this->lang_flag) {
                    $alt = !$this->lang_name ? explode(' (', $node['name'])[0] : (!$this->lang_code ? $node['iso_code'] : __('Flag')); ?>
                    <img class="elementor-langs__flag" src="<?php escape(Helper::getMediaLink('img/l/' . (int) $node['id_lang'] . '.jpg')); ?>" alt="<?php escape($alt); ?>" width="16" height="11">
                <?php } ?>
