...
class ModulesXThemeXWidgetsXNavMenu extends WidgetBase
{
    ...
    const HELP_URL = 'https://docs.webshopworks.com/creative-elements/88-widgets/site-widgets/359-nav-menu-widget';
    ...
    protected function registerToggleStyleSection(array $args = [])
    {
        ...
        $this->addControl(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} a.elementor-menu-toggle:not(#e)' => 'color: {{VALUE}}',
                ],
            ]
        );
        ...
        $this->addControl(
            ...
            [
                ...
                'type' => ControlsManager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} a.elementor-menu-toggle:not(#e):hover' => 'color: {{VALUE}}',
                ],
            ]
        );
    ...
    protected function render()
    {
        ...
        if ('dropdown' !== $settings['layout']) {
            ...
            <nav <?php $this->printRenderAttributeString('main-menu'); ?> aria-label="<?php escape($menu_name); ?>"><?php echo $menu_html; ?></nav>
            <?php
        }
        ...
        <a href="javascript://toggle" class="elementor-menu-toggle" role="button" aria-label="<?php esc_attr_e('Menu'); ?>" aria-expanded="false">
            <i class="ceicon ceicon-menu" aria-hidden="true"></i>
        </a>
        <nav class="elementor-nav--dropdown elementor-nav__container" aria-label="<?php escape($menu_name); ?>"><?php echo str_replace('menu-1-', 'menu-2-', $menu_html); ?></nav>
        <?php
    }
    ...
    {
        ?>
        <ul class="<?php echo $depth ? 'sub-menu elementor-nav--dropdown' : esc_attr($ul_class) . '" id="menu-1-' . esc_attr($this->getId()); ?>">
        <?php foreach ($nodes as &$node) { ?>
            <li class="menu-item menu-item-type-<?php echo esc_attr($node['type']), ' menu-item-' . esc_attr($node['page_identifier']), $node['current'] ? ' current-menu-item' : '', $node['children'] ? ' menu-item-has-children' : ''; ?>">
                <a class="<?php echo $depth ? 'elementor-sub-item' : 'elementor-item', strpos($node['url'], '#') !== false ? ' elementor-item-anchor' : '', $node['current'] ? ' elementor-item-active' : ''; ?>" href="<?php escape($node['url']); ?>"<?php $node['open_in_new_window'] && print ' target="_blank"'; ?>>
    ...
    {
        $id_category = $GLOBALS['context']->controller instanceof \ProductController || $GLOBALS['context']->controller instanceof \CategoryController ? $GLOBALS['cookie']->last_visited_category : 0;
        ?>
        <ul class="<?php echo $depth ? 'sub-menu elementor-nav--dropdown' : esc_attr($ul_class) . '" id="menu-1-' . esc_attr($this->getId()); ?>">
        <?php foreach ($nodes as &$node) {
            $current = $node['id'] == $id_category; ?>
            <li class="menu-item menu-item-type-category menu-item-category-<?php echo (int) $node['id'], $current ? ' current-menu-item' : '', $node['children'] ? ' menu-item-has-children' : ''; ?>">
                <a class="<?php echo $depth ? 'elementor-sub-item' : 'elementor-item', $current ? ' elementor-item-active' : ''; ?>" href="<?php escape($node['link']); ?>">
                <?php if ($this->show_category_thumbs && \Tools::file_exists_cache(_PS_CAT_IMG_DIR_ . (int) $node['id'] . '-0_thumb.jpg')) { ?>
                    <img class="cat-menu" src="<?php escape(Helper::$link->getCatImageLink('', (int) $node['id'] . '-0_thumb')); ?>" alt="">
                <?php } ?>
                    <?php echo $node['name']; ?>
                <?php if ($this->indicator && $node['children']) { ?>
                    <span class="sub-arrow <?php escape($this->indicator); ?>" aria-hidden="true"></span>
    ...
    protected function psLinkList(array &$nodes, $depth = 0, $ul_class = '')
    {
        ...
        <ul class="<?php echo $depth ? 'sub-menu elementor-nav--dropdown' : esc_attr($ul_class) . '" id="menu-1-' . esc_attr($this->getId()); ?>">
        <?php foreach ($nodes as &$node) { ?>
            <li class="menu-item menu-item-type-link menu-item-<?php echo esc_attr($node['id']), !empty($node['links']) ? ' menu-item-has-children' : ''; ?>">
    ...
    protected function prestaBlogCategories(array &$nodes, $depth = 0, $ul_class = '')
    {
        ...
        <?php foreach ($nodes as &$node) {
            ...
            <li class="menu-item menu-item-type-prestablog-category menu-item-prestablog-category-<?php echo (int) $node['id'], $current ? ' current-menu-item' : '', $node['children'] ? ' menu-item-has-children' : ''; ?>">
                <a class="<?php echo $depth ? 'elementor-sub-item' : 'elementor-item', $current ? ' elementor-item-active' : ''; ?>" href="<?php escape($url); ?>">
                    <?php echo $node['title']; ?>
                <?php if ($this->indicator && $node['children']) { ?>
                    <span class="sub-arrow <?php escape($this->indicator); ?>" aria-hidden="true"></span>
                <?php } ?>
