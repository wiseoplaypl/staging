	    	...
	    	<ol class="">
				...
				<li>
					<a href="{smartblog::GetSmartBlogLink('smartblog')}">
						<span>{l s='All Post' d='ModulesSmartblogPosts'}</span>
					</a>
				</li>
				{if $title_category != ''}
...
{block name='page_content'}
	{capture name=path}
		<a href="{smartblog::GetSmartBlogLink('smartblog')|escape:'htmlall':'UTF-8'}">{l s='All Blog News' d='ModulesSmartblogPosts'}</a>
	...
	{/capture}
				...
				<div itemprop="articleBody">
								...
								<div class="post_meta clearfix">
									{if $smartshowauthor ==1} 
										<div class="meta_author">
											...
											<span>{if $smartshowauthorstyle != 0}{$firstname} {$lastname}{else}{$lastname} {$firstname}{/if}</span>
										</div>
									{/if}
									<div class="meta_date">
									...
									<div class="meta_comment">
										...
										{if $countcomment != ''}{$countcomment|escape:'htmlall':'UTF-8'}{else}{l s='0' d='ModulesSmartblogPosts'}{/if}
										{l s=' Comments' d='ModulesSmartblogPosts'}
									</div>
								</div>
								<a title="" style="display:none" itemprop="url" href="#"></a>
							</div>

							<div class="title_block h5">{$meta_title|escape:'htmlall':'UTF-8'}</div>
					...
					{if $tags != ''}
						<div class="sdstags-update">
							<span class="tags"><b>{l s='Tags:' d='ModulesSmartblogPosts'} </b>
		...
		{if $posts_previous != '' || $posts_next != ''}
		<div id="product_comments_block_tab">
			<ul class="footer_links clearfix">
				{foreach from=$posts_previous item="post"}
					{if isset($post.id_smart_blog_post)}
						<li>
							<a title="{l s='Prevoius Post' d='ModulesSmartblogPosts'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" class="btn btn-default button button-small"><span><i class="fa fa-angle-double-left"></i> {l s='Prevoius Post' d='ModulesSmartblogPosts'}</span></a>
						</li>
					{/if}
				{/foreach}
				{foreach from=$posts_next item="post"}
					{if isset($post.id_smart_blog_post)}
						<li class="pull-right">
							<a title="{l s='Next Post' d='ModulesSmartblogPosts'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" class="btn btn-default button button-small"><span>{l s='Next Post' d='ModulesSmartblogPosts'} <i class="fa fa-angle-double-right"></i> </span></a>
						</li>
					{/if}
				{/foreach}
			</ul>
		...
		{if $countcomment != ''}
			<div id="articleComments">
				<h3><span>{l s=' Comments' d='ModulesSmartblogPosts'}</span></h3>
				<div id="comments">
		...
		{if ($enableguestcomment==0) && isset($is_looged) && $is_looged==''}
			<section class="page-product-box">
				<h3 class="page-product-heading"><span>{l s='Comments' d='ModulesSmartblogPosts'}</span></h3>
			    {l s='Log in or register to post comments' d='ModulesSmartblogPosts'}
			</section>
		{else}
			{if Configuration::get('smartenablecomment') == 1}
				{if $comment_status == 1}
					<div class="smartblogcomments" id="respond">
				    	<h4 class="comment-reply-title" id="reply-title"><span>{l s='Leave a Reply' d='ModulesSmartblogPosts'}</span>
				    		<small style="float:right;">
				        		<a style="display: none;" href="/wp/sellya/sellya/this-is-a-post-with-preview-image/#respond" id="cancel-comment-reply-link" rel="nofollow">{l s='Cancel Reply' d='ModulesSmartblogPosts'}</a>
				            </small>
				        </h4>
						<div id="commentInput">
							<form action="" method="post" id="commentform">
								...
								<table>
									<tbody>
										...
										{else}
											<tr class="col-sm-6">
												<td>
													<span class="required">*</span> <b>{l s='Name:'  d='ModulesSmartblogPosts'} </b>
												</td>
												<td>
													<input type="text" tabindex="1" class="inputName form-control grey" value="" name="name" required>
												</td>
											</tr>
        									<tr class="col-sm-6">
												<td>
													<span class="required">*</span> <b>{l s='E-mail:' d='ModulesSmartblogPosts'} </b><span class="note hidden-xs-up">{l s='(Not Published)'  d='ModulesSmartblogPosts'}</span>
												</td>
												<td>
													<input type="text" tabindex="2" class="inputMail form-control grey" value="" name="mail" required>
												</td>
											</tr>
											<tr class="col-sm-12">
												<td>&nbsp;&nbsp;&nbsp;<b>{l s='Website:'  d='ModulesSmartblogPosts'} </b><span class="note"> {l s='(Site url with'  d='ModulesSmartblogPosts'}http://)</span></td>
												<td><input type="text" tabindex="3" value="" name="website" class="form-control grey"></td>
											</tr>
										{/if}	
										<tr class="col-sm-12">
											<td><span class="required">*</span> <b> {l s='Comment:'  d='smartblog'}</b></td>
											<td><textarea tabindex="4" class="inputContent form-control grey" rows="8" cols="50" name="comment" required></textarea></td>
										</tr>
										{if Configuration::get('smartcaptchaoption') == '1'}
											<tr class="col-sm-12">
												<td><b>{l s='Capcha' d='ModulesSmartblogPosts'}</b></td>
												<td>
													<img src="{$modules_dir|escape:'htmlall':'UTF-8'}smartblog/classes/CaptchaSecurityImages.php?width=100&height=40&characters=5" class="comment-capcha-image">
								...
								<div class="right">
							        <div class="submit mb-md-20">
										<button type="submit" name="addComment" id="submitComment" class="btn btn-primary">{l s='Submit' d='ModulesSmartblogPosts'}</button>
									</div>
								</div>
    						</form>
						</div>
