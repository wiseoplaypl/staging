...
{block name='page_content'}
    {capture name=path}<a href="{smartblog::GetSmartBlogLink('smartblog')|escape:'htmlall':'UTF-8'}">{l s='All Blog News' d='ModulesSmartblogTagresult'}</a>
         {if $title_category != ''}
        <span class="navigation-pipe"></span>{$keyword|escape:'htmlall':'UTF-8'}{/if}{/capture}
     
        {if $postcategory == ''}
                 <p class="error">{l s='No Post in This Tag' d='ModulesSmartblogTagresult'}</p>
        {else}
