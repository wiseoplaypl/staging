<div id="product-details" data-product="{$product.embedded_attributes|json_encode}" class="clearfix js-product-details">
        ...
        <div class="product-manufacturer {if isset($manufacturer_image_url)} float-right{/if}">
            {if isset($manufacturer_image_url)}
                ...
                <a href="{$product_brand_url}">
                    <img src="{$manufacturer_image_url}" class="img-fluid  manufacturer-logo" alt="{$product_manufacturer->name}" loading="lazy"/>
                </a>
            {else}
                <label class="label">{l s='Brand' d='Shop.Theme.Catalog'}</label>
                <span>
